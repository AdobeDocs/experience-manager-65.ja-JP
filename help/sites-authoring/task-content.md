...
タスクは、コンテンツに関して実行する作業項目を表します。 タスクが割り当てられると、ワークフローインボックスに表示されます。タスク項目は、 **タイプ** 列。
...
* **タスクタイル：** プロジェクトの全体的な進行状況はタスクタイルに表示され、プロジェクトの詳細ページに表示されます。

* **タスクリスト：** タスクタイルをクリックすると、タスクのリストが表示されます。 このリストには、プロジェクトに関連するすべてのタスクの詳細情報が含まれます。

両方のオプションは、ワークフロータスクと、タスクタイル内で直接作成したタスクを一覧表示します。

### タスクタイル {#task-tile}

プロジェクトに関連タスクがある場合、タスクタイルはプロジェクト内に表示されます。 タスクタイルには、プロジェクトの現在のステータスが表示されます。 このステータスは、ワークフロー内の既存タスクに基づいています。ワークフローの進行に伴って今後作成されるタスクは含まれていません。タスクタイルには、次の情報が表示されます。
...
特定のタスクの詳細を表示するには、インボックスで、タスクをタップまたはクリックして選択し、タップまたはクリックします **開く** 」と入力します。
...
1. プロジェクト内で、 **タスク** タイル
1. タイルの右上にある下向きの山形をタップまたはクリックし、「 」を選択します。 **タスクを作成**.
1. 内 **タスクを追加** ウィンドウで、タスクの詳細（優先度、担当者、期限など）を指定します。
...
タスクを完了するには、タスクを選択し、 **完了** 」と入力します。 タスクに情報を追加して、「**完了**」をクリックします。詳しくは、[インボックス](/help/sites-authoring/inbox.md)を参照してください。
