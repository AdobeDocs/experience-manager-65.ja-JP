...
すべてのスライドを表示するか、オプション **これを再度表示しない**&#x200B;にチェックを入れない限り、コンソールにアクセスすると、概要が再開されます。
...
* パンくずリスト
  ...
  パンくずリストはパネルの中央に表示され、常に現在選択されている項目の説明が表示されます。パンくずリストを使用すると、特定のコンソール内で移動できます。 Sites コンソールでは、web サイトのレベル間を移動できます。
...
* [表示](/help/sites-authoring/basic-handling.md#viewingandselectingyourresourcescardlistcolumn)
  ...
  表示アイコンは、AEM ツールバーの右端にあります。 現在のビューも示すように、その外観は変化します。 例えば、デフォルトの&#x200B;**列ビュー** ビューでは、次のように表示されます。
...
* **ページエディター**
  ...
  ページを初めて編集する場合は、一連のスライドでページエディターが紹介されます。
...
* カード表示では、現在のレベルの各項目の情報カードが表示されます。 次のような情報が提供されます。
   ...
   * ページのタイトル。
   * 重要な日付（最終編集日、最終公開日など）。
   * ページがロックされている場合、非表示になっている場合、またはライブコピーの一部である場合。
