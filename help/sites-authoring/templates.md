...
* インスタンスの設定に応じて、AEMで次の情報が提供されます。 [2 つの基本的なタイプのテンプレート](/help/sites-authoring/templates.md#editable-and-static-templates). これは、実際に[テンプレートを使用してページを作成する](#using-a-template-to-create-a-page)方法に影響を与えませんが、作成できるテンプレートの種類と、ページがそのテンプレートにどのように関連するかには影響します。
...
>[!NOTE]
>
>初期コンテンツは、コンテンツを作成するための出発点となるコンポーネントおよびページレイアウトを用意するためのものです。そのままの状態で実際のコンテンツとなるものではありません。このため、初期コンテンツは翻訳できません。
>
