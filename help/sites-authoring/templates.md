...
* テンプレートの作成には共同作業が必要です。 この理由から、[ロール](#roles)がそれぞれのタスクに示されます。

* インスタンスの設定に応じて、AEMで次の情報が提供されます。 [2 つの基本的なタイプのテンプレート](/help/sites-authoring/templates.md#editable-and-static-templates). これは、実際の [テンプレートを使用したページの作成](#using-a-template-to-create-a-page)作成できるテンプレートのタイプと、ページとテンプレートとの関係は変わりません。

### roles {#roles}

を使用したテンプレートの作成 **テンプレートコンソール** そして **テンプレートエディター** では、次の役割間の共同作業が必要です。
...
* [編集可能なテンプレート](/help/sites-authoring/templates.md#creatingandmanagingnewtemplates)
   ...
   * 新しいページが作成されると、ページとテンプレートの間に動的な接続が維持されます。 つまり、テンプレート構造やロックされたコンテンツに対する変更は、そのテンプレートで作成されたすべてのページに反映されます。 ロック解除された（つまり、初期の）コンテンツに対する変更は反映されません。
...
* 静的テンプレート
   ...
   * 静的テンプレートの編集は開発者専用のタスクなので、開発者ドキュメントを参照してください。 [ページテンプレート — 静的](/help/sites-developing/page-templates-static.md) を参照してください。
...
テンプレートを使用して [ページの作成](/help/sites-authoring/managing-pages.md#creating-a-new-page) 静的テンプレートと編集可能テンプレートの間には、目に見える違いはなく、表示もありません。 ページの作成者にとって、この処理は透過的です。
...
* パブリッシュ環境で利用できるようにするために[テンプレートを公開します。](#publishingatemplateauthor)
...
1. **テンプレートタイプ**&#x200B;を選択してから、「**次へ**」を選択します。
   ...
   >[!NOTE]
   >
   >テンプレートタイプは、定義済みのテンプレートレイアウトであり、テンプレートのためのテンプレートであると見なすことができます。これらは、開発者またはシステム管理者が事前定義します。詳しくは、開発者向けドキュメントを参照してください。 [ページテンプレート — 編集可能](/help/sites-developing/page-templates-editable.md#template-type).
...
1. 「**作成**」を選択します。確認メッセージが表示されます。「 **開く** 開始する [テンプレートの編集](#editingatemplate) または **完了** をクリックして、テンプレートコンソールに戻ります。
...
>[!CAUTION]
>
>テンプレートを有効にすると、テンプレート作成者がテンプレートの更新を開始したときに警告が表示されます。 これはテンプレートが参照されている可能性があり、そのため変更がテンプレートを参照しているページに影響を及ぼす可能性があることをユーザーに通知するためのものです。

1. **テンプレートコンソール**&#x200B;で、テンプレートを選択します。
1. ツールバーから「**有効**」または「**無効**」を選択して、確認ダイアログで再び選択します。
1. これで、次の場合にテンプレートを使用できます。 [ページの作成](/help/sites-authoring/managing-pages.md#creating-a-new-page)を使用する必要があります。 [テンプレートの編集](#editingatemplate) 自分の要件に従って
...
* [構造](#editingatemplatestructure)
  ...
  コンポーネントがロックされているとコンテンツは追加できますが、ページ作成者はこれを編集できません。コンポーネントのロックを解除して、 [初期コンテンツ](#editingatemplateinitialcontent).
...
* **ポリシーおよびプロパティ（一般）**
  ...
  利用できるプロパティは選択されたコンポーネントに依存します。例えば、テキストコンポーネントの場合、プロパティは、コピーと貼り付けのオプション、書式設定のオプション、段落スタイルなどのオプションを定義します。
  ...
  **メイン**&#x200B;タブでは、コンポーネントの最も重要な設定が定義されます。
  ...
  1 つの設定で複数の設定を実行できる場合は、 **追加** ボタンをクリックして別の設定を追加します。
  ...
  *機能*
  ...
  >[!NOTE]
  >
  >[リッチテキストエディターを実装するコンポーネントのコンテンツポリシー](/help/sites-administering/rich-text-editor.md#main-pars-header-206036638) は、UI 設定を通じて RTE で使用可能になったオプションに対してのみ定義できます。 [](/help/sites-administering/rich-text-editor.md#main-pars_header_206036638) [](/help/sites-administering/rich-text-editor.md#main-pars_header_206036638)
...
* **ポリシーおよびプロパティ（レイアウトコンテナ）**
  ...
  *デフォルトのコンポーネント*
  ...
  クリック **マッピングを追加** ：まったく新しいコンポーネントと MIME タイプマッピングを追加します。

  リスト内のコンポーネントを選択し、 **タイプを追加** をクリックして、既にマッピングされているコンポーネントに追加の MIME タイプを追加します。 「**削除**」アイコンをクリックして、MIME タイプを削除します。
...
* **コンポーネントのロック解除/ロック**
  ...
  これには、ロック解除コンテナコンポーネントも含まれるので、**初期コンテンツ**&#x200B;モードまたは結果ページのいずれかに、さらにコンポーネントを追加できます。ロック解除前にコンテナにコンポーネント/コンテンツを既に追加している場合は、には表示されなくなります。 **構造** モードでは表示されますが、 **初期コンテンツ** モード。 **構造モード**&#x200B;では、コンテナコンポーネント自体だけがその&#x200B;**許容コンポーネント**&#x200B;のリストとともに表示されます。
