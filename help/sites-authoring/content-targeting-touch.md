...
>[!NOTE]
>
>Adobe Target でキャンペーンを作成すると、`thirdPartyId` というプロパティが各キャンペーンに割り当てられます。 Adobe Target でキャンペーンを削除しても、thirdPartyId は削除されません。 `thirdPartyId` を異なるタイプ（AB、XT）のキャンペーンに再利用することはできません。また、手動で削除することはできません。 この問題を回避するには、各キャンペーンに一意の名前を付けます。 キャンペーン名は、異なるキャンペーンタイプでは再利用できません。
>
...
さらに、すべての Adobe Target アクティビティの目標と指標を管理し、Adobe Target オーディエンスを管理できます。 Adobe Targetのアクティビティレポート（A/B テストの勝者の変換を含む）も含まれています。
...
1. アクティビティの名前を入力します。
   ...
   >[!NOTE]
   >
   >アクティビティを作成し、ページまたはその親にAdobe Target クラウド設定がアタッチされている場合、AEMはAdobe Targetを自動的にエンジンと見なします。
...
1. [ターゲット](#diagramtargetconfiguringtheaudiences)：各エクスペリエンスのターゲットとなるオーディエンスを指定します。 特定のオーディエンスをターゲットにすることができます。 A/B テストを利用すれば、エクスペリエンスに対するトラフィックの割合を判断できます。
...
ターゲットモードでは、ターゲットコンポーネントの境界線は青で、ドロップターゲットメッセージはターゲットの性質を示します。
...
[ターゲティングプロセス](/help/sites-authoring/content-targeting-touch.md#the-targeting-process-create-target-and-goals-settings)のターゲット設定手順には、作成手順で操作したエクスペリエンスとオーディエンスのマッピングを含みます。 ターゲットページには、各エクスペリエンスがターゲットとするオーディエンスが表示されます。 各エクスペリエンスのオーディエンスを指定または変更できます。 Adobe Targetを利用している場合は、特定のエクスペリエンスに対するオーディエンスのトラフィックの割合をターゲティングできるA/B テストを実施することもできます。
...
1. （オプション）「**編集**」をクリックして、キーワードを入力し、目的のセグメントを検索します。
1. 「**OK**」をクリックします。
1. オーディエンスのトラフィックを各エクスペリエンスにルーティングする方法を設定するには、「割合」を入力します。 合計数は 100 にする必要があります。
...
[ターゲティングプロセス](/help/sites-authoring/content-targeting-touch.md#the-targeting-process-create-target-and-goals-settings)の目標と設定手順では、ブランドアクティビティの動作を設定します。 アクティビティの開始時間と終了時間、およびアクティビティの優先度を指定します。 さらに、目標を追跡することもできます。 具体的には、アクティビティで何を測定するかを決めることができます。
