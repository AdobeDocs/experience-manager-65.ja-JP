...
キャンペーンを作成する場合、セグメント化を考えることが重要になります。通常は、キャンペーンを開始する前にセグメントをあらかじめ定義しておく必要があります。
...
一致するセグメントに基づいて、訪問者のニーズと関心に合ったコンテンツをターゲットに設定できます。
...
**ユーザー** - Web サイトに登録してアカウントのプロファイルを受け取る訪問者です。プロファイルを生成するために、メールアドレスや性別など、追加の識別情報を提供します。 コミュニティの活動や購入パターンなどの追加情報も収集できます。 プロファイル情報に基づいて、人口統計プロファイルを作成することができます。
...
* 参照元の web サイトに対する関心。使用した検索用語やクリックした広告など。
* 他のサイトへの関心。Spyjax などのツールを使用して特定されます。
* 訪問者のロイヤルティ。訪問期間、訪問頻度。
...
例えば、人口統計学的データ（次のようなツールから取得）と参照 URL （行動特性として）を組み合わせる [Google広告プランナー](https://www.google.com/adplanner/)）を使用すると、サイト所有者は訪問者の人口統計学的特性を導き出すことができます。
...
>[!NOTE]
>
>参照： [セグメント化](/help/sites-administering/campaign-segmentation.md) Adobe Experience Managerのセグメントについて詳しくは、こちらを参照してください。
