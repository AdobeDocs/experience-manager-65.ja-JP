...
* **ブランディングのカスタマイズ**
   ...
   * **ヘッダーテキストのカスタマイズ：** ヘッダーに表示するテキストを指定して、デフォルトの **Adobe Experience Managerのテキストを置き換えます** 。

   * **ロゴのカスタマイズ：** ヘッダーにロゴとして表示する画像を指定します。 Digital Asset Management(DAM)で画像をアップロードし、その画像をフィールドで参照します。
