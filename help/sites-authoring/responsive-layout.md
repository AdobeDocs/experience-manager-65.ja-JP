...
これにより、レスポンシブグリッド内にコンポーネントを配置できる段落システムが提供されます。 このグリッドでは、デバイスやウィンドウのサイズと形式に応じてレイアウトを並べ替えることができます。このコンポーネントは、 [**レイアウト** mode](/help/sites-authoring/responsive-layout.md#defining-layouts-layout-mode)：デバイスに応じたレスポンシブレイアウトを作成および編集できます。
...
* [**レイアウトコンテナ**](#adding-a-layout-container-and-its-content-edit-mode)&#x200B;コンポーネント
  ...
  このコンポーネントは、 [コンポーネントブラウザー](/help/sites-authoring/author-environment-tools.md#components-browser) およびは、レスポンシブグリッド内にコンポーネントを追加および配置できるグリッド段落システムを提供します。 ページ上のデフォルトの段落システムとしても設定できます。
...
エミュレーターを使用する場合、エミュレーションとレイアウトの定義用の特定のデバイスを選択すると、関連するブレークポイントもハイライト表示されます。レイアウトの変更は、ブレークポイントが適用される他のデバイス（アクティブなブレークポイントマーカーの左側、次のブレークポイントマーカーの前に配置されるすべてのデバイス）に適用できます。
...
1. エミュレートする特定のデバイスを選択するには、次のいずれかを実行します。
   ...
   * エミュレーターツールバーのデバイスインジケーターをクリックします。
...
* コンテンツコンポーネントをクリックすると、ツールバーで次の操作が可能になります。
...
* In **レイアウト** モードでは、 **ここにコンポーネントをドラッグ** をクリックして、コンポーネント全体を選択します。 これにより、このモードのツールバーが表示されます。
   ...
   * **ブレークポイントレイアウトを元に戻す** - デフォルトのレイアウトに戻します。これは、カスタマイズされたレイアウトが適用されないことを意味します。
