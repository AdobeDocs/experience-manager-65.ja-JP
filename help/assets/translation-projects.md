...
翻訳ワークフローを実行して編集済みバージョンの元のアセットで言語コピーを更新すると、翻訳済みアセットが承認されるまで、既存の言語コピーが保持されます。 [!DNL Adobe Experience Manager Assets] は、新しく翻訳されたアセットを一時的な場所に保存し、アセットを明示的に承認した後で既存の言語コピーを更新します。 ユーザーがアセットを承認しないと、言語コピーは変更されません。
