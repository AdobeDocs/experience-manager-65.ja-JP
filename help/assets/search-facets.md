...
description: で検索ファセットを作成、変更、使用する方法 [!DNL Adobe Experience Manager].
contentOwner: AG
...
企業全体での [!DNL Adobe Experience Manager Assets] には、多くのアセットを保存する能力があります。 の汎用検索機能のみを使用する場合は、適切なアセットの検索が困難で時間がかかることがあります。 [!DNL Experience Manager].
...
フルテキスト検索の場合は、 **[!UICONTROL Fulltext]** の述語がフォームに含まれている。 「プロパティの述語」を使用すると、ユーザーが指定した 1 つのプロパティと一致するアセットが検索されます。「オプションの述語」を使用すると、特定のプロパティについて 1 つ以上の値と一致するアセットが検索されます。「日付の範囲の述語」を追加すると、指定した期間内に作成されたアセットが検索されます。

1. 次をクリック： [!DNL Experience Manager] ロゴをクリックし、に移動します。 **[!UICONTROL ツール]** > **[!UICONTROL 一般]** > **[!UICONTROL 検索Forms]**.
1. 次の [!UICONTROL 検索Forms] ページ、選択 **[!UICONTROL アセット管理者の検索レール]**&#x200B;を選択し、「 **[!UICONTROL 編集]** ![編集アイコン](assets/do-not-localize/aemassets_edit.png).

   >[!NOTE]
   >
   >事前設定済みの [!DNL Assets] 以前のバージョンの管理者の検索レールでは、次の手順を実行します。
   >
   >1. に移動します。 `/conf/global/settings/dam/search/facets/assets/jcr:content/items` CRXDE 内で使用します。
   >1. を削除します。 `type` ノード。
   >1. パスから `/libs/settings/dam/search/facets/assets/jcr:content/items`、ノードをコピーします。 `asset`, `directory`, `typeor`, `excludepaths`、および `searchtype` を、手順 1 で説明したパスに追加します。
   >1. 変更内容を保存します。
...
1. 「[!UICONTROL 設定]」タブで、その述語のラベル、プレースホルダーテキストおよび説明を入力します。述語に関連付けるメタデータプロパティの有効な名前を指定します。ヘッダーのラベル ( [!UICONTROL 設定] 「 」タブに、選択した述語のタイプが表示されます。
...
1. の検索パネルに移動します。 [!DNL Assets] ユーザーインターフェイス。 プロパティの述語がパネルに追加されています。
1. 検索するアセットの説明をテキストボックスに入力します。例えば、 `Adobe`. 検索を実行すると、説明が一致するアセットが `Adobe` が検索結果に表示されます。
...
CRXDE リポジトリのオプション用のノード構造を手動で作成する代わりに、対応するキーと値のペアを指定することで JSON ファイルでオプションを定義できます。 JSON ファイルのパスを「**[!UICONTROL プロパティ名]**」フィールドに指定します。例えば、キーと値のペア `image/bmp`、`image/gif`、`image/jpeg`、`image/png` を定義し、次の JSON ファイルのサンプルに示すように、それらの値を指定します。内 **[!UICONTROL プロパティ名]** フィールドに、このファイルの CRXDE パスを指定できます。
...
1. 次をクリック： [!DNL Experience Manager] ロゴをクリックし、に移動します。 **[!UICONTROL ツール]** > **[!UICONTROL 一般]** > **[!UICONTROL 検索Forms]**.
1. 次の **[!UICONTROL 検索Forms]** ページ、選択 **[!UICONTROL アセット管理者の検索レール]**&#x200B;を選択し、「 **[!UICONTROL 編集]**.
...
1. 次のいずれかの操作を行います。
   ...
   * 次をクリック： `+` 「オプション」フィールドの横の記号を使用して、フィルターパネルに入力するオプションの表示テキストと値を指定します。 別のオプションを追加するには、 `+` 記号をクリックし、手順を繰り返します。
...
複数値プロパティの述語では、複数の値でアセットを検索できます。[!DNL Assets] に複数の製品の画像があり、各画像のメタデータには製品の SKU 番号が含まれているとします。この述語を利用すれば、複数の SKU 番号で製品の画像を検索できます。

1. 次をクリック： [!DNL Experience Manager] ロゴをクリックし、に移動します。 **[!UICONTROL ツール]** > **[!UICONTROL 一般]** > **[!UICONTROL 検索Forms]**.
1. 検索Formsページで、「 **[!UICONTROL アセット管理者の検索レール]**、クリック **[!UICONTROL 編集]** ![編集アイコン](assets/do-not-localize/aemassets_edit.png).
...
1. のフィルターパネルに移動します。 [!DNL Assets] ユーザーインターフェイス。 **[!UICONTROL 複数値プロパティ]**&#x200B;の述語がパネルに追加されています。
1. 「複数値」フィールドに、複数の値を区切り文字で区切って検索します。述語は、指定した値とテキストが完全に一致するものを返します。

## タグ用述語の追加 {#adding-a-tags-predicate}

タグの述語を使用すると、タグに基づいたアセットの検索が実行できます。 デフォルトでは、[!DNL Assets] は指定されたタグに基づいて、1 つまたは複数のタグと一致するアセットを検索します。言い換えれば、検索クエリは指定したタグを用いて OR 演算を実行します。ただし、「すべてのタグに一致」オプションを使用すれば、すべての指定したタグを含むアセットを検索することも可能です。

1. 次をクリック： [!DNL Experience Manager] ロゴをクリックし、に移動します。 **[!UICONTROL ツール]** > **[!UICONTROL 一般]** > **[!UICONTROL 検索Forms]**.
1. 「Formsを検索」ページで、「 **[!UICONTROL アセット管理者の検索レール]** 次に、 **[!UICONTROL 編集]** ![編集アイコン](assets/do-not-localize/aemassets_edit.png).
...
| [!UICONTROL パスブラウザー] | 事前設定済みのルートパスのフォルダーおよびサブフォルダー内のアセットを検索するための検索用述語。 | <ul><li>プレースホルダー</li><li>ルートパス</li><li>説明</li></ul> |
...
| [!UICONTROL ファイルサイズ] | サイズに基づいてアセットを検索するための検索用述語。スライダーベースの述語で、設定可能なノードからスライダーのオプションを選択します。デフォルトのオプションは、CRXDE リポジトリの/libs/dam/options/predicates/filesize で定義されます。 ファイルサイズはバイト単位で示します。 | <ul><li>ラベル</li><li>プロパティ名</li><li>パス</li><li>説明</li></ul> |
...
デフォルトでは、ロックアイコン ![閉じたアイコンをロック](assets/do-not-localize/lock_closed_icon.svg) 次の値より前 **[!UICONTROL アセット管理者の検索レール]** 内 **[!UICONTROL 検索Forms]** ページ。 検索フォームページのオプションに対するロックアイコンは、デフォルト設定がそのままになっており、カスタマイズされていないことを示します。アイコン ![閉じたアイコンをロック](assets/do-not-localize/lock_closed_icon.svg) が表示されなくなるのは、デフォルトのフォームが変更されたことを示す検索ファセットをフォームに追加した場合です。

![ロックアイコン](assets/locked_admin_rail.png)

デフォルトの検索ファセットを復元するには、次の手順を実行します。

1. **[!UICONTROL 検索フォーム]**&#x200B;ページで「**[!UICONTROL アセット管理者の検索レール]**」を選択します。
1. クリック **[!UICONTROL 削除]** ![deleteoutline](assets/do-not-localize/deleteoutline.png) 」と入力します。
1. 確認ダイアログで、 **[!UICONTROL 削除]** をクリックして、カスタムの変更を削除します。

   検索ファセットに対するカスタムの変更を削除した後、ロックアイコン ![閉じたアイコンをロック](assets/do-not-localize/lock_closed_icon.svg) 前に再現 **[!UICONTROL アセット管理者の検索レール]** 内 **[!UICONTROL 検索Forms]** ページ。
...
| アクション | 権限 |
| ------------------- | ---------------------------------------------------------------- |
| [!UICONTROL 編集] | の読み取りおよび書き込み権限 `/apps` CRXDE のノード |
| [!UICONTROL 削除] | の読み取り、書き込みおよび削除権限 `/apps` CRXDE のノード |
| [!UICONTROL プレビュー] | の読み取り、書き込みおよび削除権限 `/var/dam/content` CRXDE のノード。 また、`/apps` の読み取りおよび書き込み権限 |
