---
title: スマートイメージング
description: スマートイメージングでは、各ユーザーに固有の閲覧特性を適用して、ユーザーのエクスペリエンス用に自動的に最適化された適切な画像を提供することで、より良いパフォーマンスとエンゲージメントをもたらします。
...
## スマートイメージングの利点 {#what-are-the-key-benefits-of-smart-imaging}
...
+++### サポートされている画像形式を教えてください。{#what-image-formats-are-supported}
...
スマートイメージングを使用するには、会社のDynamic Media ClassicまたはExperience Manager版Dynamic Media アカウントに、ライセンスの一部としてAdobeバンドルの CDN （Content Delivery Network）が含まれている必要があります。

+++

+++### アカウントのスマートイメージングを有効にするには、どうすればよいですか？{#what-is-the-process-for-enabling-smart-imaging-for-my-account}
...
TTL を調整するには：
   ...
   1. **Adobe Experience ManagerのDynamic Mediaの場合：**
      1. [ 次の手順 ](/help/assets/dm-publish-settings.md#common-thumbnail-attributes-tab) に従います。
      1. **[!UICONTROL 有効期限]** の値を 24 時間以上に設定します。

+++

+++### アカウントでスマートイメージングを有効にするには、どうしたらよいですか？{#when-can-i-expect-my-account-to-be-enabled-with-smart-imaging}
...
+++### Chrome Web ブラウザーに fmt=tif の URL があります。 しかし、要求が ImageServer エラーで失敗します。なぜでしょうか。{#fmt-tif}
...
+++### スマートイメージングでは、画質（% 単位）の出力設定は自動的に調整されますか？それとも、手動で調整した設定がすべての画像に適用されますか？ 何の範囲だ？{#percent-quality}
...
+++### スマートイメージングで置き換えられるのは、JPEG画像のみですか？ WebP や PNG などをリクエストした場合はどうなりますか？{#replace-request}
