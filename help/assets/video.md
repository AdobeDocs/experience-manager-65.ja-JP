...
description: ビデオのエンコーディング、ビデオへのマルチオーディオおよびマルチキャプションの追加、ビデオサムネールの追加に関するベストプラクティスなど、Dynamic Mediaでビデオを操作する方法について説明します。
mini-toc-levels: 3
...
>
>Dynamic Mediaでの現在の既知のビデオ再生の問題 *Experience Manager6.5.9.0 のみ*:
>
...
1. 次のいずれかの操作を行って、**Dynamic Media ビデオを公開します**。
   ...
   * [ビデオにキャプションを追加する](#adding-captions-to-video).
...
* iPhone、iPad、Android™、BlackBerry®、Windows Phone など、Web およびデスクトップおよびモバイルデバイスにアダプティブビデオセットを配信します。
...
Dynamic Media では、MP4 H.264 ビデオのモバイルビデオ再生がサポートされています。このビデオ形式をサポートする BlackBerry®デバイスは、次の場所で見つけることができます。 [BlackBerry®でサポートされるビデオ形式](https://support.blackberry.com/kb/articleDetail?ArticleNumber=000005482).
...
DM ゲートウェイサービスの URL が常に HTTPS も使用するので、Experience Manager 6.3 以降ではビデオが HTTPS（DASH または HLS） でストリーミングされるようになりました。このデフォルトの動作はユーザーに影響しません。つまり、ブラウザーでサポートされていない場合を除き、ビデオストリーミングは常に HTTPS 上で行われます（次の表を参照）。 したがって、次のようになります。
...
>[!IMPORTANT]
>
>*ビデオに DASH を使用するには、まずアカウントの Adobe テクニカルサポートが DASH を有効にする必要があります。詳しくは、 [Dynamic Mediaアカウントで DASH を有効にする](#enable-dash).
...
![Dynamic Mediaビデオソリューションのアーキテクチャ。](assets/chlimage_1-427.png)
...
Adobeサポートケースを作成して DASH を有効にすると同時に、マルチサブタイトルとマルチオーディオトラックのサポートを自動的に有効にするメリットも得られます。 有効化後、以降にアップロードするすべてのビデオは、ビデオへのマルチサブタイトルおよびマルチオーディオトラックの追加をサポートする新しいバックエンドアーキテクチャで処理されます。

>[!IMPORTANT]
>
>Dynamic Media アカウントでのマルチサブタイトルおよびマルチオーディオトラックのサポートの有効化&#x200B;*前*&#x200B;にアップロードしたビデオは、[再処理する必要があります](/help/assets/processing-profiles.md#reprocessing-assets)。このビデオ再処理手順は、マルチサブタイトルとマルチオーディオトラック機能を使用できるようにするために必要です。再処理後も、ビデオ URL は引き続き通常どおり動作し、再生されます。
...
1. 開始日 **Dynamic Media for DASH の設定** -Experience Managerから、に移動します。 **[!UICONTROL ツール]** > **[!UICONTROL 運用]** > **[!UICONTROL Web コンソール]**.

1. 次から： **[!UICONTROL Adobe Experience Manager Web コンソールの設定]** ページで、名前までスクロールします。 *AEM Assets Dynamic Mediaビデオの高度なストリーミング機能フラグ*.

1. 名前の左側で、DASH を有効（オン）にするチェックボックスを選択します。

1. 「**[!UICONTROL 保存]**」を選択します。

1. 今すぐ [Admin Consoleを使用して、新しいサポートケースの作成を開始します。](https://helpx.adobe.com/jp/enterprise/using/support-for-experience-cloud.html).
...
ビデオレポートの正常動作のために、Dynamic Media Cloud Services の設定時に、レポートスイート ID が自動的に作成されます。そのときに、そのレポートスイート ID がパブリッシュサーバーにプッシュされ、アセットのプレビューの際に URL のコピー機能で使用できるようになります。ただし、この機能を使用するには、公開サーバーが既に設定されている必要があります。パブリッシュサーバーがセットアップされていない場合でも、公開してビデオレポートを確認することはできます。ただし、その際には Dynamic Media クラウド設定に戻って「**[!UICONTROL OK]**」を選択する必要があります。
...
![Dynamic Media の「サブタイトルとオーディオトラック」タブとアップロードされた.VTT サブタイトルファイルと、ビデオ用にアップロードされた .MP3 オーディオトラックファイルを示す表。](assets-dm/msma-subtitle-audiotracks-tab.png)
...
すべての [Dynamic Media でサポートされるビデオ形式](/help/assets/assets-formats.md)と Dynamic Media *Video_360* ビューアを除くすべての Dynamic Media ビデオビューアは、マルチサブタイトルおよびマルチオーディオトラックとの使用がサポートされます。
...
* [Dynamic Media ビデオプロファイルは、ビデオが取り込まれるフォルダーに適用されます](/help/assets/video-profiles.md#applying-a-video-profile-to-folders)。
...
>[!IMPORTANT]
>
>Dynamic Media アカウントでマルチサブタイトルおよびマルチオーディオトラックのサポートを有効にする&#x200B;*前*&#x200B;にアップロードしたビデオは[再処理する必要があります](/help/assets/processing-profiles.md#reprocessing-assets)。このビデオ再処理手順は、マルチサブタイトルとマルチオーディオトラック機能を使用できるようにするために必要です。再処理後も、ビデオ URL は引き続き通常どおり動作し、再生されます。

**ビデオにマルチサブタイトルとマルチオーディオトラックを追加するには、次の手順に従います。**

1. 既にビデオプロファイルが割り当てられた[フォルダーにプライマリビデオをアップロード](/help/assets/managing-video-assets.md#upload-and-preview-video-assets)には、ています。
...
1. ツールバーで、プロパティアイコン（中に「i」がある円）を選択します。
   ![ビデオのサムネール画像の上にチェックマークが付き、ツールバーで「プロパティを表示」がハイライト表示されている、選択されたビデオアセット。](assets-dm/msma-selectedasset-propertiesbutton.png)*カード表示で選択したビデオアセット。*
1. ビデオのプロパティページで、「**[!UICONTROL サブタイトルとオーディオトラック]**」タブを選択します。

   >[!TIP]
   >「**[!UICONTROL サブタイトルとオーディオトラック]**」タブが表示されない場合、次の 2 つのいずれかを意味します。
   >
   >* 選択したビデオが存在するフォルダーには、ビデオプロファイルが割り当てられていません。その場合は、 [フォルダーへのビデオプロファイルの適用](/help/assets/video-profiles.md#applying-video-profiles-to-specific-folders).
   >* または、ビデオを Dynamic Media で再処理する必要があります。その場合は、 [フォルダー内のアセットの再処理](/help/assets/processing-profiles.md#reprocessing-assets).
   >
   >上記のいずれかのタスクを完了したら、次の手順に戻ります。

   ![プロパティページの「サブタイトルとオーディオトラック」タブ。](assets-dm/msma-audiotracks.png)*ビデオのプロパティページの「サブタイトルとオーディオトラック」タブ。*
...
>[!CAUTION]
>
>公開または非公開のビデオにサブタイトルファイルまたはオーディオファイルをアップロードすると、ビデオを&#x200B;[*再処理*](/help/assets/processing-profiles.md#reprocessing-assets)&#x200B;するとファイルは削除されます。ビデオの元のオーディオのみがそのまま残ります。このような場合は、サブタイトルファイルとオーディオトラックファイルをビデオに再度アップロードする必要があります。
...
![「サブタイトル」フィールドおよび「オーディオトラック」フィールドでハイライト表示されているステータス列。](assets-dm/msma-lifecycle-status.png)*アップロードされた各サブタイトルおよびオーディオトラックファイルのライフサイクルステータス。*
...
**デフォルトとして設定**&#x200B;ダイアログボックスで、「**[!UICONTROL 置き換える]**」を選択します。
   ...
   ![選択したオーディオトラックファイル名と、「デフォルトとして設定」ボタンがハイライト表示されたオーディオトラックの見出し。](assets-dm/msma-defaultaudiotrack.png)*ビデオのデフォルトのオーディオトラックの設定。*
...
字幕ファイルとオーディオトラックファイルをビデオにアップロードして処理した後、Dynamic Mediaビデオビューア（または必要に応じて他のビューアタイプ）を使用して、様々なトラックをプレビューできます。 プレビューを使用すると、ビデオが顧客に対してどのように表示され、サウンドが期待どおりに動作しているかを確認できます。
...
1. [ビューアー] リストで、ビデオプレビューに使用するビューアーを選択します。例として、次のスクリーンショットは **[!UICONTROL ビデオ]**&#x200B;ビューアーが選択されています。
   ...
   ![[ビューアー] ドロップダウンリストでのビデオビューアーの選択。](assets-dm/msma-dmviewerselected.png)
...
1. 右下付近のボリュームアイコンの左側にある吹き出しアイコンを選択し、聞きたい、または見たいオーディオまたはサブタイトルを選択します。必要に応じて、[サブタイトル] で [**[!UICONTROL オフ]**] を選択して、サブタイトルやキャプションを表示しないようにします。
   ...
   ![ビデオビューアのオーディオおよびサブタイトルのポップアップリスト。](assets-dm/msma-selectaudiosubtitle.png)*ビデオ再生用のオーディオとサブタイトルを選択するユーザーのシミュレーション。*

1. 再生を開始するには、ビデオの「**[!UICONTROL 再生]**」ボタンを選択します。
左下に [**[!UICONTROL URL]**] および [**[!UICONTROL 埋め込み]**] ボタンがあります。これらのボタンを使用して、[ビデオの URL を Web アプリケーションにリンクする](/help/assets/linking-urls-to-yourwebapplication.md)か、[Web ページにビデオを埋め込みます](/help/assets/embed-code.md)。
...
| `manifestType` | オプション。パラメータは DASH または HLS にすることができます。 渡されない場合は、デフォルトで DASH に設定されます。 |
