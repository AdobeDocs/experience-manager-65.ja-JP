...
description: ビデオのエンコーディング、ビデオへの複数のオーディオトラックとキャプショントラックの追加、ビデオサムネールなど、Dynamic Mediaでビデオを操作する方法を説明します。
mini-toc-levels: 3
...
DM ゲートウェイサービスの URL が常に HTTPS も使用するので、Experience Manager 6.3 以降ではビデオが HTTPS（DASH または HLS） でストリーミングされるようになりました。このデフォルトの動作はユーザーに影響しません。つまり、ブラウザーでサポートされていない場合を除き、ビデオストリーミングは常に HTTPS 上で行われます（以下の表を参照）。したがって、次のようになります。
...
>[!IMPORTANT]
>
>*ビデオに DASH を使用するには、まずアカウントの Adobe テクニカルサポートが DASH を有効にする必要があります。詳しくは、[Dynamic Media アカウントで DASH を有効にする](#enable-dash)を参照してください。
...
### Dynamic Media アカウントで DASH、複数キャプションおよびオーディオトラックのサポートを有効にする {#enable-dash}
...
アカウントで DASH を有効にするAdobeサポートケースを作成すると同時に、複数のキャプションとオーディオトラックのサポートを自動的に有効にするというメリットもあります。 イネーブルメント後、アップロードするすべてのビデオは、ビデオへの複数のキャプショントラックとオーディオトラックの追加をサポートする新しいバックエンドアーキテクチャで処理されます。

>[!IMPORTANT]
>
>アップロードしたビデオ *次の前* Dynamic Media アカウントで複数のキャプションおよびオーディオトラックのサポートを有効にする [再処理する必要があります](/help/assets/processing-profiles.md#reprocessing-assets). このビデオの再処理手順は、複数のキャプションとオーディオトラック機能を使用するために必要です。 再処理後も、ビデオ URL は引き続き通常どおり動作し、再生されます。

**Dynamic Media アカウントで DASH、複数キャプション、複数音声トラックのサポートを有効にするには：**
...
1. 次に、[Admin Console を使用して、新しいサポートケースの作成を開始します](https://helpx.adobe.com/jp/enterprise/using/support-for-experience-cloud.html)。
...
1. 次のいずれかを実行できます。
   ...
   * [複数のキャプションおよびオーディオトラックの追加](#add-msma) ビデオに追加します。
...
Dynamic Mediaの複数のキャプションおよびオーディオトラック機能を使用すると、プライマリビデオに複数の字幕とオーディオトラックを簡単に追加できます。 この機能により、グローバルなオーディエンスがビデオにアクセスできるようになります。1 つの公開済みプライマリビデオを複数の言語でグローバルオーディエンスに向けてカスタマイズし、様々な地理的地域のアクセシビリティガイドラインに従うことができます。また、作成者は、ユーザーインターフェイスの 1 つのタブからサブタイトルとオーディオトラックを管理することもできます。

![Dynamic Media の「サブタイトルとオーディオトラック」タブと、アップロードされた .VTT サブタイトルファイルおよびビデオ用にアップロードされた .MP3 オーディオトラックファイルを示すテーブル。](assets-dm/msma-subtitle-audiotracks-tab.png)

プライマリビデオに複数のキャプショントラックとオーディオトラックを追加する場合に考慮すべきユースケースには、次のものがあります。
...
すべて [Dynamic Mediaでサポートされるビデオ形式](/help/assets/assets-formats.md) およびDynamic Mediaを除くすべてのDynamic Media ビデオビューア *Video_360* ビューア – 複数のキャプションおよびオーディオトラックでの使用がサポートされています。

Dynamic Media アカウントで複数のキャプションやオーディオトラックを使用するには、Adobeカスタマーサポートから有効（オン）にする機能を切り替える必要があります。

### ビデオへの複数のキャプショントラックとオーディオトラックの追加 {#add-msma}
...
* [Dynamic Media アカウントで複数のキャプションとオーディオトラックが有効になっている](#enable-dash).
...
1. ビデオのプロパティページで、「**[!UICONTROL 字幕とオーディオトラック]**」タブを選択します。
   ...
   >* または、ビデオを Dynamic Media で再処理する必要がある。この場合は、[フォルダー内のアセットを再処理](/help/assets/processing-profiles.md#reprocessing-assets)を参照してください。
   >
...
>[!CAUTION]
>
>公開または非公開のビデオにサブタイトルファイルまたはオーディオファイルをアップロードした場合、ビデオを&#x200B;[*再処理*](/help/assets/processing-profiles.md#reprocessing-assets)&#x200B;するとファイルは削除されます。ビデオの元のオーディオのみがそのまま残ります。このような場合は、サブタイトルファイルとオーディオトラックファイルをビデオに再度アップロードする必要があります。
...
![「サブタイトル」フィールドと「オーディオトラック」フィールドがハイライト表示されているステータス列。](assets-dm/msma-lifecycle-status.png)*アップロードされた各サブタイトルおよびオーディオトラックのファイルのライフサイクルステータス。*
...
**デフォルトとして設定**&#x200B;ダイアログボックスで、「**[!UICONTROL 置換]**」を選択します。
   ...
   ![選択したオーディオトラックファイル名と、ハイライトされた「デフォルトとして設定」ボタンが表示されたオーディオトラックの見出し。](assets-dm/msma-defaultaudiotrack.png)*ビデオのデフォルトオーディオトラックの設定*

1. 右上隅で「 **[!UICONTROL 保存して閉じる]**」を選択します。
1. ビデオを公開します。詳しくは、[アセットの公開](publishing-dynamicmedia-assets.md)を参照してください。

### 複数のサブタイトルとオーディオトラックがあるビデオのプレビュー{#preview-video-audio-subtitle}

サブタイトルファイルとオーディオトラックファイルをビデオにアップロードして処理した後、Dynamic Media ビデオビューアー（または必要に応じて他のビューアータイプ）を使用して、様々なトラックをプレビューできます。プレビューを使用すると、ビデオが顧客にどのように見え、どのように聞こえるかを確認し、期待どおりに動作していることを確認できます。
...
1. 右下隅付近のボリュームアイコンの左側にある吹き出しアイコンを選択し、聞きたい、見たい、またはその両方を行いたいオーディオまたはサブタイトルを選択します。必要に応じて、「サブタイトル」で&#x200B;**[!UICONTROL オフ]**&#x200B;を選択すると、サブタイトルやキャプションを非表示にすることもできます。
   ...
   ![ビデオビューアーのオーディオとサブタイトルのポップアップリスト。](assets-dm/msma-selectaudiosubtitle.png)*ビデオ再生用のオーディオとサブタイトルを選択するユーザーのシミュレーション。*
...
## ビデオにクローズドキャプションまたはサブタイトルを追加 {#adding-captions-to-video}

>[!IMPORTANT]
>
>Adobeでは、次のことをお勧めします [複数キャプションおよびオーディオトラック機能の有効化](#enable-dash) Dynamic Media アカウントに追加します。 そうすることで、最新の Dynamic Media バックエンドアーキテクチャと、キャプション、サブタイトル、オーディオトラックをビデオに追加するための簡素化されたワークフローを利用できるようになります。
