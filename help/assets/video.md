...
>[!IMPORTANT]
>
>アップロードしたビデオ *前* Dynamic Mediaアカウントでマルチサブタイトルおよびマルチオーディオトラックのサポートを有効にする [は再処理する必要があります](/help/assets/processing-profiles.md#reprocessing-assets). このビデオ再処理手順は、マルチサブタイトルとマルチオーディオトラック機能を使用できるようにするために必要です。 再処理後も、ビデオ URL は引き続き通常どおり動作し、再生されます。
...
1. ビデオのプロパティページで、「 **[!UICONTROL 字幕とオーディオトラック]** タブをクリックします。
   ...
   >[!TIP]
   >次の項目が表示されない場合、 **[!UICONTROL 字幕とオーディオトラック]** 「 」タブには、選択したビデオが存在するフォルダーが含まれ、ビデオプロファイルが割り当てられていないことを意味します。 [フォルダーへのビデオプロファイルの適用](/help/assets/video-profiles.md#applying-video-profiles-to-specific-folders)を選択し、次にこれらの手順に戻ります。

   ![「プロパティ」ページの「サブタイトルとオーディオトラック」タブ&#x200B;](assets-dm/msma-audiotracks.png)*ビデオのプロパティページの「サブタイトルとオーディオトラック」タブ。 空のオーディオトラックリストボックスで示されるように、ビデオに関連付けられたオリジナルのオーディオトラックがないことに注意してください。 ビデオの処理が完了すると、トラックが表示されます。*
...
| 処理 | 処理中です。 |
| 処理済み | 処理が完了しました。 |
| 公開済み | Dynamic Media に公開中. |
| 失敗 | 処理が完了しませんでした。 ファイルを削除して、もう一度アップロードしてみてください。 |
| 非公開     | 処理中ですが、まだDynamic Mediaに公開されていません。 |
...
1. 「ビューア」リストから **[!UICONTROL Video]** を選択します。

   ![「ビューア」ドロップダウンリストからのビデオビューアの選択。](assets-dm/msma-dmviewerselected.png)

1. 右下隅付近のボリュームアイコンの左側にある吹き出しアイコンを選択し、聞きたいオーディオまたはサブタイトル、またはその両方を選択します。 必要に応じて、「サブタイトル」で「 **[!UICONTROL オフ]** をクリックして、サブタイトルやキャプションを表示しないようにします。

   ![ビデオビューアのオーディオおよびサブタイトルポップアップリスト。](assets-dm/msma-selectaudiosubtitle.png)*ビデオ再生用のオーディオとサブタイトルを選択するユーザーのシミュレーション。*

1. ビデオの **[!UICONTROL 再生]** ボタンをクリックして再生を開始します。
次の点に注意してください。 **[!UICONTROL URL]** および **[!UICONTROL 埋め込み]** ボタンを使用します。 次のボタンを使用して、 [ビデオの URL を web アプリケーションにリンクする](/help/assets/linking-urls-to-yourwebapplication.md) または [Web ページにビデオを埋め込む](/help/assets/embed-code.md)、それぞれ。
