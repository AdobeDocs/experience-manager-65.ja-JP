...
>[!IMPORTANT]
>
>アップロードしたビデオ *前* Dynamic Mediaアカウントでのマルチサブタイトルおよびマルチオーディオトラックのサポートの有効化 [は再処理する必要があります](/help/assets/processing-profiles.md#reprocessing-assets). このビデオ再処理手順は、マルチサブタイトルとマルチオーディオトラック機能を使用できるようにするために必要です。 再処理後も、ビデオ URL は引き続き通常どおり動作し、再生されます。
...
1. 次のいずれかを実行できます。
   ...
   * 通常通り、[ビデオビューアープリセット](/help/assets/managing-viewer-presets.md#creating-a-new-viewer-preset)を作成します。
...
![Dynamic Mediaの「サブタイトルとオーディオトラック」タブ。アップロードされた.VTT サブタイトルファイルと、ビデオ用にアップロードされた.MP3 オーディオトラックファイルを示す表。](assets-dm/msma-subtitle-audiotracks-tab.png)
...
すべて [Dynamic Mediaでサポートされるビデオ形式](/help/assets/assets-formats.md) Dynamic Mediaを除くすべてのDynamic Mediaビデオビューア *ビデオ 360* viewer — マルチサブタイトルおよびマルチオーディオトラックでの使用がサポートされます。
...
* A [Dynamic Mediaビデオプロファイルは、ビデオが取り込まれるフォルダーに適用されます](/help/assets/video-profiles.md#applying-a-video-profile-to-folders).
...
1. [フォルダーにプライマリビデオをアップロード](/help/assets/managing-video-assets.md#upload-and-preview-video-assets) には、既にビデオプロファイルが割り当てられています。
...
1. ツールバーで、プロパティアイコン（円と「i」が入った円）を選択します。
   ![ビデオのサムネール画像の上にチェックマークが付いている、選択したビデオアセットと、ツールバーで「プロパティを表示」がハイライト表示されている。](assets-dm/msma-selectedasset-propertiesbutton.png)*カード表示で選択したビデオアセット。*
1. ビデオのプロパティページで、「 **[!UICONTROL 字幕とオーディオトラック]** タブをクリックします。
   ...
   >* または、ビデオをDynamic Mediaで再処理する必要があります。 その場合は、 [フォルダー内のアセットの再処理](/help/assets/processing-profiles.md#reprocessing-assets).
   >
   >上記のいずれかのタスクを完了したら、次の手順に戻ります。

   ![「プロパティ」ページの「サブタイトルとオーディオトラック」タブ&#x200B;](assets-dm/msma-audiotracks.png)*ビデオのプロパティページの「サブタイトルとオーディオトラック」タブ。*
...
![「字幕」フィールドと「オーディオトラック」フィールドで強調表示されているステータス列。](assets-dm/msma-lifecycle-status.png)*アップロードされた各サブタイトルおよびオーディオトラックファイルのライフサイクルステータス。*
...
Adobe Analytics の **デフォルトとして設定** ダイアログボックスで、次の項目を選択します。 **[!UICONTROL 置換]**.
   ...
   ![選択したオーディオトラックファイル名と、「デフォルトとして設定」ボタンがハイライトされた「オーディオトラック」見出し。](assets-dm/msma-defaultaudiotrack.png)*ビデオのデフォルトのオーディオトラックを設定する。*
...
1. プレビューページで、ページの左上隅付近にあるドロップダウンリストを選択し、「**[!UICONTROL ビューア]**」を選択します。
   ...
   ![「ビューア」オプションを表示するドロップダウンリスト。](assets-dm/msma-selectviewers.png)

1. 「ビューア」リストで、ビデオプレビューに使用するビューアを選択します。 例として、次のスクリーンショットは **[!UICONTROL ビデオ]** ビューアが選択されています。

   ![「ビューア」ドロップダウンリストからのビデオビューアの選択。](assets-dm/msma-dmviewerselected.png)
...
1. 右下隅付近のボリュームアイコンの左側にある吹き出しアイコンを選択し、聞きたいオーディオまたはサブタイトルを選択します。または、その両方を表示することもできます。 必要に応じて、「サブタイトル」で「 **[!UICONTROL オフ]** をクリックして、サブタイトルやキャプションを表示しないようにします。
   ...
   ![ビデオビューアのオーディオおよびサブタイトルポップアップリスト。](assets-dm/msma-selectaudiosubtitle.png)*ビデオ再生用のオーディオとサブタイトルを選択するユーザーのシミュレーション。*

1. 再生を開始するには、ビデオの&#x200B;**[!UICONTROL 再生]**&#x200B;ボタンを選択します。次の点に注意してください。 **[!UICONTROL URL]** および **[!UICONTROL 埋め込み]** ボタンを使用します。 次のボタンを使用して、 [ビデオの URL を web アプリケーションにリンクする](/help/assets/linking-urls-to-yourwebapplication.md) または [Web ページにビデオを埋め込む](/help/assets/embed-code.md)、それぞれ。
...
1 つのビデオまたはアダプティブビデオセットにチャプターマーカーを追加することで、長形式のビデオを見やすくし、移動しやすくすることができます。 ユーザーがビデオを再生する際、ビデオタイムライン（別名：ビデオスクラバー）のチャプターマーカーをクリックすると、関心のあるシーンに簡単に移動できます。または、新しいコンテンツ、デモおよびチュートリアルに即座にジャンプすることができます。
