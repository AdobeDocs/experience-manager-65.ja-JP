...
* JPGは 10:1 の比率で圧縮され、通常は画像ファイルサイズが小さくなります。 PNG は、画像に白い背景が含まれている場合などを除いて、約 2 : 1 の比率で圧縮されます。ただし、通常、PNG ファイルのサイズはJPGファイルよりも大きくなります。
...
Experience Manager を使用すれば、取得時、配信時またはその両方で画像をシャープにすることができます。ただし、通常は、両方のメソッドではなく、どちらか一方のメソッドのみを使用して画像をシャープにします。配信時に URL 上で画像をシャープニングすると、通常、最適な結果が得られます。
...
* このパラメータは、JPGのエンコーディング品質を制御します。 値を大きくすると、画質は高くなり、ファイルサイズは大きくなります。値を小さくすると、画質は低くなり、ファイルサイズは小さくなります。 このパラメータの範囲は 0 ～ 100 です。
