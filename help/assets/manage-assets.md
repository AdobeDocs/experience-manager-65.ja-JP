...
|  | Photoshop オプション | Adobe® Photoshop® ファイルからのテンプレート作成、レイヤーの維持、レイヤーの命名方法の指定、テキストの抽出、テンプレートへの画像のアンカー方法の指定をおこなうことができます。<br>テンプレートは [!DNL Experience Manager] ではサポートされていません。<br>[Photoshop アップロードオプションの設定](#setting-photoshop-upload-options)を参照してください。 |
...
>
>ただし、既存の冗長なユーザーグループ、未使用のユーザーグループおよび自動生成されたユーザーグループは、オーサーインスタンスにある JMX の `clean` メソッドを使用して、リポジトリからクリーンアップすることができます（`http://[server]:[port]/system/console/jmx/com.day.cq.dam.core.impl.team%3Atype%3DClean+redundant+groups+for+Assets`）。
