...
シャープの適用については、画像へ [のシャープの適用](/help/assets/assets/s7_sharpening_images.pdf)を参照してください。
...
後で変更した既存の画像プロファイルが既に存在するフォルダー内のアセットを再処理できます。 [処理プロファイルを編集した後のフォルダー内のアセットの再処理](processing-profiles.md#reprocessing-assets)を参照してください。
