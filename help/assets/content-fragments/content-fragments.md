...
description: Adobe Experience Manager(AEM) のコンテンツフラグメントを使用して、ヘッドレス配信に最適な、ページに依存しないコンテンツのデザイン、作成、キュレーション、使用を可能にする方法を説明します。
...
Adobe Experience Manager(AEM) を使用すると、コンテンツフラグメントを使用して、デザイン、作成、キュレーションおよび [ページに依存しないコンテンツを公開](/help/sites-authoring/content-fragments.md). 複数の場所や複数のチャネルで使用できるコンテンツを準備でき、ヘッドレス配信に最適です。
...
* 構造の範囲は次のとおりです。
   * 基本
      ...
      * ページオーサリングで使用する簡単なコンテンツを準備するために使用します。
   * 複合
      * テキスト、数値、ブール値、データ、時間など、様々なデータタイプの多くのフィールドの組み合わせ。
      * ページオーサリング用により構造化されたコンテンツを準備する場合、またはアプリケーションに配信する場合に使用します。
   * 入れ子
...
* データの複雑さを反映できるように、ネストされたコンテンツを作成します。
...
>[!NOTE]
>
>AEM 6.3 以前は、コンテンツフラグメントは、モデルの代わりにテンプレートを使用して作成されていました。 フラグメントを作成する際に、テンプレートは使用できなくなりましたが、そのようなテンプレートで作成されたフラグメントは引き続きサポートされます。
...
AEM コンテンツフラグメントを使用して構造化コンテンツを記述し、管理できます。構造化コンテンツは、テキスト、数値データ、ブール値、日付と時刻など、様々なコンテンツタイプを含むモデルで定義されます。
...
* Adobe Analytics で使用されている [コンテンツフラグメントコンポーネントを含むページエディター](/help/sites-authoring/content-fragments.md) （参照コンポーネント）:
...
* フラグメントに&#x200B;**アセットを挿入**（混在メディアフラグメント）
   ...
   * フラグメントの一部です ( [コンテンツフラグメントの構成要素](#constituent-parts-of-a-content-fragment)) をクリックします。
   * アセットの位置を定義します。
...
* **フラグメントの段落**
   ...
   * 個々のエンティティとして区切られた、多くの場合は複数行のテキストブロック。
...
* **バリエーション**
   ...
   * のコピーとして作成 **マスター**&#x200B;を編集する必要がありますが、その後、必要に応じて編集できます。バリエーション同士にはコンテンツの重複があります。
...
* 次に該当 [フラグメントのフロー内に追加される追加のコンテンツ](/help/sites-authoring/content-fragments.md#adding-in-between-content) ページ上で使用または参照された後。
