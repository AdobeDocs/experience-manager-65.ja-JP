...
1. [作成](#creating-a-content-fragment-model)、および [設定](#defining-your-content-fragment-model)、コンテンツフラグメントモデルに適用されます。
1. [コンテンツフラグメントモデルの有効化](#enabling-disabling-a-content-fragment-model) コンテンツフラグメントを作成する際に使用します。
...
* **レンダリング時の名前** フラグメント内のフィールドを実現／レンダリングするための様々なオプション。多くの場合、これにより、作成者にフィールドの 1 つのインスタンスが表示されるか、作成者が複数のインスタンスを作成できるかを定義できます。
...
1. モデルを選択し、次にツールバーの「**削除**」を選択します。
   ...
   >[!NOTE]
   >
   >モデルを参照している場合は、警告が表示されます。適切な対策を実施します。
