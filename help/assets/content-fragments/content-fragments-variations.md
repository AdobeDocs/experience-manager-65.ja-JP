...
**マスター** はコンテンツフラグメントの一部であり、定義上、このフラグメントにはコンテンツのマスターコピーが格納されます。一方、バリエーションには、そのコンテンツの個々の更新済みのカスタマイズ済みバージョンが格納されます。 プライマリを更新した場合、これらの変更はバリエーションにも関連するので、バリエーションに伝播させる必要があります。
