...
オプション **新しいコンテンツフラグメント** を使用すると、完全に新しいフラグメントを作成できます。 このために、コンテンツフラグメント作成ウィザードがエディターで開きます。
...
書式設定は行われず、フラグメントの段落シーケンスに追加されます。書式設定は、 [フラグメントがページで使用または参照されている](/help/sites-authoring/content-fragments.md).
