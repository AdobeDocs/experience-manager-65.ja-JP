...
例えば、バリエーションを操作する場合、メタデータを編集する場合、関連するコンテンツを管理する場合などです。
...
>
>ただし、このアクションは、同じ削除権限を他の&#x200B;*すべての* アセットタイプにも適用します。
...
* すべてのコンテンツフラグメントの`jcr:content` ノードについて：
...
`add`および`modify`権限は、CRXDE Liteで管理することも、User Management コンソールを使用して管理することもできます。
