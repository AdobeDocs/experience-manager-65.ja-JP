...
description: AEMとAdobe Creative cloudの統合に関するベストプラクティスを参照して、アセット転送ワークフローを合理化し、最大限の効率を達成してください。
contentOwner: AG
...
| 使用例 | Adobe Asset Link | AEM デスクトップアプリケーション | 備考／その他のソリューション |
|---|---|---|---|
...
| 使用 - アセットを開く | はい | 対応 - 任意のアプリに対して | [Webインターフェイスまたは](managing-assets-touch-ui.md#previewing-assets) Finderから開く |
...
| アップロード - 単一ファイル | 対応 - 現在のアクティブなドキュメントをアップロードします | はい | [Webインターフェイスを使用したアップロード](managing-assets-touch-ui.md#uploading-assets) |
| アップロード - 複数ファイル／階層フォルダー構造 | いいえ | はい | [Webインターフェイスを使用したアップロード](managing-assets-touch-ui.md#uploading-assets)；カス<br>タムスクリプティングまたはツール |
...
| その他 - 多数のアセットを移行する | いいえ | いいえ | [移行ガイド](assets-migration-guide.md) |
...
通常、社内ネットワークに接続されている社内のクリエイティブチームまたはエージェンシー/クリエイティブプロフェッショナルは、AEMログインを含むDAMインスタンスにアクセスできます。 外部の関係者（通常はクライアントのために働く代理店などの信頼できる組織）に直接アクセスを許可するように、AEM とネットワークインフラストラクチャを設定できます。これにより、外部の関係者がネットワークを介して（例えば、VPN または IP ホワイトリスト経由で）AEM にアクセスできるようになります。

このような場合、Adobe Asset Link または AEM Desktop App を使用すると、最終／承認済みアセットに簡単にアクセスしたり、クリエイティブレディアセットを DAM に保存したりできます。
...
説明は、デスクトップユーザーのワークフローの通常の部分として、操作的に（例えば、毎週、または各写真で）ファイルをアップロードすることを指します。 大規模なアセット移行については、ここでは説明しません。
