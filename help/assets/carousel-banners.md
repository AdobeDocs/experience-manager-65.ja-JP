...
1. [ホットスポットと画像マップの変数を識別します](#identifying-hotspot-and-image-map-variables)（ Experience Manager Assets と Dynamic Media の両方を使用するユーザーのみ）。
   ...
   >[!NOTE]
   >
   >Experience Manager Sites または Ecommerce のユーザーである場合、ビルトインの機能を使用して製品ページに移動し、製品カタログで既存の SKU （在庫管理単位）を調べることができます。 ホットスポットまたは画像マップの変数を手動で入力する必要はありません。 [eCommerce の設定](/help/commerce/cif-classic/administering/generic.md)に関する情報を参照してください。
   >
   >
