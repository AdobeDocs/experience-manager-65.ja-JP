...
1. カスタムフォームフィールドを含むプロジェクト内に、リンクされたフォルダーを作成します。カスタムフォームフィールドと、プロジェクトでリンクされたフォルダーを作成する必要がある対応する値。 この設定は、空のままにした場合は無視されます。フィールドに `CUSTOM FORMS: Create DAM Linked Folder` を選択して、値に `Yes` を入力します。
