...
MSM for [!DNL Assets]を使用するには、少なくとも[!DNL Experience Manager] 6.5 Service Pack 1をインストールします。 詳しくは、[最新のサービスパック](/help/release-notes/sp-release-notes.md)のリリースノートを参照してください。
...
## ライブコピーフォルダー{#bulk-actions}内のバルクアクション
...
## [!DNL Assets] {#extend-api}用にMSMを拡張

[!DNL Experience Manager] では、MSM Java API を使用して機能を拡張することができます。[!DNL Assets]の場合、拡張は[!DNL Sites]用のMSMと同じように機能します。 詳しくは、[MSM の拡張](/help/sites-developing/extending-msm.md)を参照してください。具体的な作業については、以下を参照してください。
...
>* MSMロックの設定は、ページプロパティ（タッチ操作対応UI）のMSM for [!DNL Assets]ではサポートされていません。
...
* サイトを作成ウィザードでの章の削除は、MSM for [!DNL Assets]ではサポートされていません。
* MSM for [!DNL Assets]では、ページプロパティ（タッチ操作対応UI）に対するMSMロックの設定はサポートされていません。
...
[!DNL Assets]のMSMの制限を次に示します。

* この使用例では、コンテンツフラグメント（CF）はサポートされていません。ライブコピーを作成しようとすると、CF は関係を除きそのままコピーされます。コピーされたCFは、時間内のスナップショットであり、元のCFが更新されても更新されません。
