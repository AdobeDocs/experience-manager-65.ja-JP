...
現在、コンテンツフラグメントの構造を定義するモデルは、HTTP API では公開されません。したがって、 *消費者* は、フラグメントのモデルについて（少なくとも 1 つ）把握する必要があります。ただし、ほとんどの情報はペイロードから、データタイプなどと推測できます。 は定義の一部だからです。
...
* **コンテンツフラグメントモデルは現在サポートされていません**：読み取りも作成もできません。コンテンツフラグメントを作成または既存のフラグメントを更新するには、コンテンツフラグメントモデルの正しいパスを開発者が知っておく必要があります。 現在のところ、これらの概要を取得するには、管理 UI を使用するしかありません。
