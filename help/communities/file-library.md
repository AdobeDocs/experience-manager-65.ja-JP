...
を追加するには、以下を実行します。 `File Library` コンポーネントをオーサリングモードでページに追加する場合は、次の場所にコンポーネントを配置します。
...
配置した `File Library` コンポーネントを使用して、 `Configure` アイコンをクリックすると、編集ダイアログボックスが開きます。
...
* **許可された名前空間**
  ...
  「タグ付けを許可」をオンにした場合、使用可能なタグは、チェックする名前空間に制限されます。 名前空間が選択されていない場合は、すべての名前空間が許可されます。 初期設定は、すべての名前空間です。
...
の下 **ユーザーモデレート** コメントが許可されている場合は、「コメントのモデレート」タブを設定します。
...
* **モデレートのしきい値**
  ...
  訪問者がコメントに何回フラグを設定したらモデレーターに通知するかを指定します。 デフォルトは 1 回です (**1**) をクリックします。
