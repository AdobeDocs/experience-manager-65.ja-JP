...
* **[!UICONTROL 画像を追加]**
   ...
   割り当てビューでメンバに対するリソースを表す画像を追加します。
...
リソースを追加パネルで、以下の操作を実行します。

* 選択 `+ Add Resources` をクリックして、学習パスに追加する 2 つのスキーレッスンリソースを選択します。
...
* **[!UICONTROL 割り当て先を追加]**
   ...
   プルダウンメニューを使用して、 `Community Ski Class` メンバーを含むグループ `Riley Taylor` および `Sidney Croft.`
