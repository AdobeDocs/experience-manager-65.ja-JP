...
1. **承認済みユーザー権限の確認**
   ...
   これを検証するには、{ 作成された承認済みユーザー [ の代わりに ](/help/sites-administering/sync.md#createauthuser) オーサーインスタンスの [AdobeGranite 配布 – 暗号化されたパスワードトランスポート秘密鍵プロバイダー ](/help/sites-administering/sync.md#adobegraniteencpasswrd) 設定を、管理者ユーザーの資格情報を使用するように変更します。 次に、パッケージを再度インストールしてみてください。 管理者の資格情報でユーザーの同期が正常に機能する場合は、作成された公開ユーザーに適切な ACL がなかったことを意味します。
