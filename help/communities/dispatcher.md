...
description: AEM Communities用Dispatcherを設定して、コミュニティサイトを適切に機能させる方法について説明します。
...
AEM Communities用Dispatcher キャッシュ機能は、Dispatcherがコミュニティサイトのページの完全にキャッシュされたバージョンを提供する機能です。

現在は、検索の結果としてコミュニティサイトを閲覧したり、コミュニティページにアクセスしたりしたユーザーなどの匿名サイト訪問者と、インデックスページを表示する検索エンジンのみがサポートされています。 この利点は、匿名のユーザーと検索エンジンのパフォーマンスが向上することです。

サインイン済みメンバーの場合、Dispatcherはキャッシュをバイパスし、リクエストをパブリッシャーに直接中継します。これにより、すべてのページが動的に生成および配信されます。

Dispatcher キャッシュをサポートするように設定されている場合、Dispatcher キャッシュされたページが最新であることを確認するために、TTL ベースの「最大有効期限」がヘッダーに追加されます。
...
フィルタープロパティ名は、フィルターパターンを適用する順序を示すために4桁の数字を使用するという規則に従っています。 複数のフィルターパターンがリクエストに適用される場合、最後に適用されるフィルターパターンが効果的になります。 したがって、最初のフィルターパターンは、すべてを否定するために使用されることが多く、次のパターンは制御された方法でアクセスを復元するのに役立ちます。

次のサンプルでは、特定の`dispatcher.any` ファイルに適合するように変更する必要があるプロパティ名を使用しています。
...
`dispatcher.any`のルールセクションでは、リクエストされたURLに基づいてキャッシュする応答を定義します。 コミュニティの場合、「ルール」セクションは、キャッシュすべきではないものを定義するために使用されます。
...
問題の主な原因は、特にアクセスを拒否するルールを追加する際に、以前のルールへの影響に注意を払わずにフィルタールールを挿入することです。

最初のフィルターパターンは、多くの場合、すべてを拒否するために使用され、次のフィルターによって制御された方法でアクセスが復元されます。 複数のフィルターがリクエストに適用される場合、最後に適用されるフィルターは有効なフィルターです。

## サンプル Dispatcher.any {#sample-dispatcher-any}

次に、Communities /filtersと/rulesを含むサンプル `dispatcher.any` ファイルを示します。
