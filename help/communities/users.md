...
新しいコミュニティサイトを作成すると、オーサー環境（{ オーサーグループの役割 ](#author-group-roles) を参照）またはパブリッシュ環境（{2[Publishのグループの役割 ](#publish-group-roles) を参照）でコミュニティサイトを管理するために必要な様々な管理機能に適した一意の ID （uid）と権限で、新しいユーザーグループが動的に作成されます。[
