...
* ドラフトモード
* 公開の予定
* 代理で作成 (a) [特権を持つ人](/help/communities/users.md#privileged-members-group) 別のコミュニティメンバーに代わってコンテンツを作成できます )
...
次の場合に [必要なクライアント側ライブラリ](/help/communities/blog-developer-basics.md#essentials-for-client-side) が含まれる場合、この方法で `Blog` コンポーネントが表示されます。
...
以下 **ユーザーモデレート** 「 」タブで、モデレート設定を指定します。
...
以下 **タグフィールド** 」タブで、次の場合に適用できるタグを指定します。 **タグ付けを許可** が **設定** タブ：
...
以下 **ジャーナルのサイドバー設定** タブで、アーカイブの日付形式と、サイドバーに表示するエントリの種類を指定します。
...
サインインしているユーザーがコミュニティメンバーまたは [特権を持つ人](/help/communities/users.md#privileged-members-group) （設定に応じて）、 `New Article` 新しいブログ記事を作成して投稿するには、以下を実行します。
