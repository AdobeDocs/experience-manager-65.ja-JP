...
* アダプティブフォームの翻訳を生成する際、生成された XLIFF ファイルには、アダプティブフォームと同じ一連のテキストが含まれていません。 場合によっては、テキストのコンテキストを確認する必要があります。 XLIFF のシーケンスがアルファベットの場合は、これはできません。 （NPR-37435）。

* アダプティブフォームが翻訳されると、HTMLタグが翻訳の一部になります。 ユーザーが誤ってタグが有効でない場合は、レコードのドキュメントにテキスト全体が表示されません。 (NPR-37499)

* アダプティブフォームがベース言語で作成され確定されると、外部チームが翻訳を行い、読み込みます。 追加や欠落したドット (.) などのテキストの小さな変更がある場合 がベース言語用にでおこなわれると、他のすべての言語では完全な翻訳が失われます。 (NPR-37189)
