...
* AEM 6.5.18 またはAEM 6.5.19 にアップグレードした後、ネストされた設定フォルダーがサポートされなくなり、コンテンツフラグメントモデルフォルダーが表示されなくなりました。 （SITES-18110）
* 一部のサブフォルダーでは、継承されたコンテンツフラグメントモデルから選択できません。 ユーザーインターフェイス経由で作成された DAM フォルダーにそのようなノードがある場合でも、`jcr:content` プロパティを持たないフォルダーをサポートする必要があります。 （SITES-17943）
...
* Jackson エクスポーターでは、コンテンツフラグメントはシリアル化できません。ページで参照されるコンテンツフラグメント（Jackson エクスポーターコードを使用）があり、コンテンツフラグメントにタグが追加されると、ページの読み込みが中断します。 （SITES-18096）
...
* 言語コピーを作成すると、エラーメッセージが表示されます。これは、コンテンツフラグメントモデルを使用して、ページプロパティで参照されたコンテンツフラグメントを持つページで発生します。 （NPR-41441）
