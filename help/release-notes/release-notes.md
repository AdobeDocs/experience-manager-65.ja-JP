...
* [XFA ベースのPDFの階層ブックマーク ](https://helpx.adobe.com/content/dam/help/ja/experience-manager/6-5/forms/pdf/using-designer.pdf):Output ServiceとAEM Forms Designerは、静的なインタラクティブおよびフラットなXFA ベースのPDFで構造化ブックマーク階層を生成するようになりました。 ブックマークは、テキストボックスのアクセシビリティプロパティで設定された見出しレベル（H1～H6）に従うため、H2～H6のエントリは並行して表示される代わりに正しい親の下にネストされます。
...
* インタラクティブ通信のプレビューで、AEM Forms サービスパック 6.5.24.0の後にコンテンツが正しく読み込まれるようになりました。 スペースが足りない状態でテキストの読み込みが遅くなったため、プレビューは作成したコンテンツと一致し、読みやすさを維持できます。 （FORMS-25346）
* 検証パターンを設定してフォームを保存すると、アダプティブ Forms コアコンポーネントに検証の詳細が表示されるようになりました。 パターンはオーサリングインターフェイスに表示されたままになります。 （FORMS-25236）
* AEM Forms 6.5 サービスパック 23およびAEM Forms サービスパック 6.5.24.0環境で、ネストされたExtensible Forms Description Language （XDP）フラグメントを正しく処理できるようになりました。 （FORMS-25234）
* フォームフラグメント外のローカライズされたテキストが、Rhino エンジンを使用したアダプティブ Formsのサーバーサイドの再検証中に、正しい言語で表示されるようになりました。 送信後にテキストがデフォルトの言語に戻らなくなりました。 （FORMS-25224）
* AEM Forms サービスパック 6.5.24.0にアップグレードした後、Red Hat Enterprise Linux （RHEL） 8で無効な命令エラーが発生してOutput Serviceがクラッシュしなくなりました。 ドキュメントの生成とフォーム出力の処理が、サービスを突然停止することなく完了します。 （FORMS-25192）
* アダプティブFormsのaddInstance （）関数を使用して動的に追加されたパネルとコンテンツが、最初のインスタンス数が0に設定されたときに表示されるようになりました。 （FORMS25169、FORMS25124）
* AEM Forms サービスパック 6.5.24.0にアップグレードすると、オーサー環境と公開環境で中国語（香港）の繁体字翻訳が正しく表示されるようになりました。 ローカライズされたzh-HK コンテンツが間違った言語で表示されたり、デフォルトの文字列に予期せずフォールバックしたりすることがなくなりました。 （FORMS-25042）
* アダプティブFormsの「手書き署名」フィールドのキーボードナビゲーションで、フォームをタブで移動しながら、一貫して署名領域にフォーカスを移動できるようになりました。 （FORMS-25011）
* Web サービス記述言語（WSDL）ファイルが、設定および更新操作中に、「Web サービスを呼び出す」ステップで正しく読み込まれるようになりました。 （FORMS24992、FORMS24789、FORMS24188）
* テキストフラグメントに条件が適用されている場合、文字のドラフトに改行が保持されるようになりました。 複数行のコンテンツが、1つの連続行として表示されなくなりました。 （FORMS-24602）
* Adobe Sign ステップに到達した後に署名ステータスの応答が返されない場合、Adobe Managed Services（AMS）のAEM FormsのAdobe Sign ワークフローが停止しなくなりました。 （FORMS-24514）
* HTMLからポータブル文書形式（PDF）への変換が、大規模または複雑な変換ジョブを含むAEM Forms サービスパック 6.5.24.0で断続的にタイムアウトしなくなりました。 （FORMS-23978）
* AEMForms-6.5.0-0112 パッチをインストールした後、管理者ユーザーインターフェイスのセーフバックアップモードを正しく管理できるようになりました。 オプションが使用可能な場合、ユーザーはセーフバックアップモードを無効にするか、オフにすることができます。 （FORMS23976、FORMS23718）
* フォームデータモデルのデータソース検索が失敗したり、検索結果に生のHyperText Markup Language （HTML）タグが表示されたりすることがなくなりました。 結果は読みやすいテキストを表示します。 （FORMS-23875）
* AEM Forms サービスパック 6.5.24.0でサポートされているコアコンポーネントバージョンを使用してAdobe FormsをSites ページに埋め込むと、実行時にカスタム関数が読み込まれるようになりました。 （FORMS-23802）
* 更新されたSling Commons JavaScript Object Notation （JSON）ライブラリで解析する際に、Interactive Communicationsの数値ルールが値を正しく評価するようになりました。 数値フィールドでは、BigDecimal解析の変更を一貫して処理します。 （FORMS-23733）
* バージョン 6.5.0以降のリリースで作業する際に、レコードのドキュメントの動作が一貫するようになりました。 フォームの出力と関連する処理は、新しい環境で期待される動作と一致します。 （FORMS-23338）

#### Forms JEE {#forms-jee-6525}

* org.owasp.esapi.reference.JavaLogFactory エラーのクラスが見つからないログコンポーネントを読み込む際に、アプリケーションの起動が失敗しなくなりました。 影響を受ける環境は、サービスの再起動や再構成を必要とせずに正しく初期化されます。 （FORMS-25348）
* Service Pack 25を適用すると、起動が正常に完了します。 システムのブートストラッププロセスは、初期化が完了する前に失敗しなくなります。 （FORMS-25347）
* コア Portable Document Format （PDF）（CPDF）コンポーネントが、Linux環境で正常にビルドおよび実行されるようになりました。実行中に予期せず停止することはありません。 （FORMS-25115）
* バーコード抽出中に特定のPortable Document Format （PDF）ファイルを処理する際に、DecodingException エラーが発生してバーコードのデコードが失敗する問題が発生しなくなりました。 （FORMS23534、FORMS23449）

#### Forms Designer {#forms-designer-6525}

* バージョン 6.5でExportDataを使用する際に、ExportData出力がフォームデータと一致するようになりました。 書き出された拡張可能マークアップ言語（XML）フィールドは、元のフォームに表示される値と一致します。 （LC-3922791）
* Output Serviceは、AEM Forms Service Pack 6.5.22.0で作成されたPortable Document Format （PDF） ファイルで正しいアクセシビリティタグを生成するようになりました。 支援テクノロジーでは、要素をスキップすることなくコンテンツを正しい順序で読み取ります。 （LC-3922756）
* タグ付けを有効にしたポータブルドキュメント形式（PDF）フォームを統合すると、Output サービスで非表示または無効なフィールドの状態が正しく反映されるようになりました。 （LC-3922708）
* Workbenchの一番下のキャプションを持つフィールドに正しいタグ付けが適用されるようになりました。これにより、生成されるドキュメントの一貫性とアクセシビリティが向上します。 （LC-3922619）
* AEM Forms サービスパック 6.5.20.0にアップグレードした後も、フォームのQR コードは引き続きスキャンできます。 標準スキャナーは、生成されたQR コードを確実に読み取ることができます。 （LC-3922551）
* 異なるサービスパックで同じ入力を使用する場合に、Forms Service renderで一貫性のある出力が生成されるようになりました。 AEM Forms サービスパック 6.5.17.0とAEM Forms サービスパック 6.5.18.0の間でレンダリングされた値が異なっていなくなりました。 （LC-3922461）
* XML データパッケージ（XDP）テンプレートから生成されたPDF/A ドキュメントで、くぼんだ正方形の境界線スタイルが正しくレンダリングされるようになりました。 フォームフィールドのアピアランスは、デザインされたレイアウトと一致します。 （LC-3922180）
* 統合されたポータブルドキュメント形式（PDF）の送信では、すべてのセクションにデータが保持されるようになりました。 入力された値は、最終的に統合された文書に残ります。 （LC-3922008）
* 変換された静的なPortable Document Format （PDF）ファイルでは、元のXDP テンプレート内の1つのリンクから複数のリンクタグが生成されなくなりました。 （LC-3921997）
* AEM Formsからの送信の書き出しに、書き出された出力ファイルのすべてのフィールドが含まれるようになりました。 （LC-3921983）
