...
* **FORMS-23802** （フォームが Sites ページにある場合にカスタム関数が読み込まれない）: アダプティブフォームが Sites ページに埋め込まれており、aem-forms-core-component ライブラリのバージョンが 1.1.76 未満の場合、カスタム関数はプレビューまたは公開で機能しません。ログに `InvalidFormContainerException: No form container found` などのエラーが表示される場合があります。 この問題を解決するには、AEM Forms SP24 （AddOn 6.0.1454[&#x200B; の &#x200B;](/help/release-notes/aem-forms-hotfix.md) ホットフィックスをダウンロードしてインストールしてください。
