...
description: ' [!DNL Adobe Experience Manager] タッチ操作対応UI固有のリリースノート。'
...
AEM 6.4以降の[クラシックUIは非推奨(](../release-notes/deprecated-removed-features.md))となりました。 Adobeでは、クラシックUIの機能がさらに強化されることはありません。タッチ操作対応UIの強力な新機能を利用することをお勧めします。

バージョン6.0以降、AEMでは、[!DNL Adobe Experience Cloud]とAdobeのユーザーインターフェイスの全体的なガイドラインに合わせた、「タッチ操作対応UI」（単に「タッチUI」と呼ばれる）と呼ばれる新しいユーザーインターフェイスが導入されました。 ほぼ機能パリティに達したので、これは、従来のデスクトップ指向インターフェイス（「クラシックUI」と呼ばれる）を備えたAEMの標準UIになりました。

ほとんどの機能はタッチ操作対応 UI に搭載されていますが、まだ完成せず、今後のリリースで追加される機能もあります。

次のリストは、AEM 6.5で実装されている機能の現在のステータスを示しています。

AEM 6.5にアップグレードするお客様向けの推奨事項については、[お客様向けのユーザーインターフェイスの推奨事項](/help/sites-deploying/ui-recommendations.md)を参照してください。

>[!NOTE]
>
>このページで扱うのは、クラシックUIと同等の機能のみです。 タッチ操作対応UIに追加され、クラシックUIに存在しない固有の機能は一覧に表示されません。

>[!NOTE]
>
>このリストは、完成を目指していますが、すべてを網羅したものではありません。
...
これは、クラシックUIサイト管理(`/siteadmin`)が持つ機能とタッチ操作対応UIのステータス(`/sites.html`)の一覧です。

| 機能 | ステータス | コメント |
|--- |--- |--- |
| サイト階層を移動 | 完了 | AEM 6.4では、[コンテンツツリービュー](/help/sites-authoring/basic-handling.md#content-tree)が導入されました。 |
| ワークフローを開始 | 完了 |  |
| ページを新規作成 | 完全 |  |
| サイトを新規作成 | 完全 |  |
| ローンチを新規作成 | 完全 |  |
| 新しいライブコピーの作成 | 完了 |  |
| フォルダーを作成 | 完全 |  |
| 公開ステータスを表示 | 完了 | AEM 6.5以降は、ワークフローステータスがリスト表示に表示されます。 |
| 検索 | 完了 |  |
| ページのコピー&amp;ペースト（複製） | 完了 |  |
| ページを移動 | 完全 |  |
| ページを公開 | 完全 |  |
...
| 参照 | 完了 | AEM 6.5に追加された受信ページリンクの表示。 |
...
| ロールアウトページ | 完了 |  |
| ワークフローの開始と表示 | 完了 |  |
| ワークフローパッケージの処理 | ほぼ完全 | タッチ操作対応UIで完全にアクセス可能。 クラシックUIでは、複数のワークフローペイロードが引き続き表示されます。 |
| ページをロック／ロック解除 | 完全 |  |
| ページを発行 | 完了 |  |
| ページを非公開 | 完全 |  |
...
| 参照を表示 | 削除 | Site Adminを使用して、[詳細な参照リスト](/help/sites-authoring/author-environment-tools.md#references)を確認します。 |
...
| 取り消し／やり直し | 完全 |  |
| コンテンツをコンポーネントプレースホルダーにドラッグする | 完了 |  |
| コンテンツをparsysプレースホルダーに直接ドラッグし、コンポーネントを自動作成 | 完了 |  |
...
| 機能 | ステータス | コメント |
|--- |--- |--- |
| リッチテキストエディター | 完了 | インプレース、ダイアログ、フルスクリーンで使用できます。 |
...
| RTEプラグイン：検索と置換 | 完了 |  |
| RTEプラグイン：テキストフォーマット（太字、...） | 完了 |  |
| RTEプラグイン：下付き文字と上付き文字 | 完了 |  |
| RTEプラグイン：均等配置 | 完了 |  |
| RTEプラグイン：リスト（箇条書き/数値） | 完了 |  |
| RTEプラグイン：段落書式 | 完了 |  |
| RTEプラグイン：テキストスタイル | 完了 |  |
| RTEプラグイン：ソースエディター（HTMLを編集） | 完了 | ダイアログと全画面表示でのみ使用できます。 |
| RTEプラグイン：スペルチェッカー | 完了 |  |
| RTEプラグイン：表（埋め込み表エディタ） | 完了 |  |
| RTEプラグイン：取り消し/やり直し | 完了 |  |
| RTEプラグイン：インライン画像を許可する | 完了 |  |
| テーブルエディター | 完了 | インプレース、ダイアログ、フルスクリーンで使用できます。 |
| 画像を表のセルにドラッグ | 完了 | インラインで使用可能 |
| 画像エディター | 完了 | インプレース、ダイアログ、フルスクリーンで使用できます。 |
| IPEプラグインの有効化/無効化 | 完了 | AEM 6.3では、[テンプレートエディター](/help/sites-authoring/templates.md)にUIが導入されました。 |
| IPEプラグイン：切り抜き | 完了 |  |
| IPEプラグイン：反転 | 完了 |  |
...
| 機能 | ステータス | コメント |
|--- |--- |--- |
| タスク管理 | 置換 | 6.0では、プロジェクトとタスクが導入されました。 |
| ワークフローインボックス | 完了 |  |
| ページテンプレート設定へのワークフロー(`/etc/workflow/wcm/templates.html`) | 消失 | クラシック UI を使用します。 |
| 管理UIのタグ付け | 完了 |  |
| MSM/ブループリントコントロールセンター | 完了 |  |
| ブループリントマネージャーUI | 完了 |  |
| ロールアウト設定UI | 消失 | クラシック UI を使用します。 |
| ユーザー、グループ、権限のUI | ほぼ完全 | 高度な権限編集の場合は、クラシックUIを使用します。 |
| バージョンのパージ(`/etc/versioning/purge.html`) | 消失 | クラシック UI を使用します。 |
