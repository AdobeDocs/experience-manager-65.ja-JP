...
2020年に、Adobe Experience Manager (AEM) 6.5向けに、四半期ごとのService Packに新機能と改善点が提供されました。 お客様は、この新しいアプローチを利用して、より迅速に革新的な技術を導入できるようになります。
...
* サイト内のContextHubの読み込みと初期化にかかる時間を短縮します(contexthub.kernel.js)。 サイト訪問中の最初のページの読み込みが高速化されます。

* ページエディターで、エクスペリエンスフラグメントをページキャンバスにドラッグ&amp;ドロップした後にページが更新される時間を短縮します。

* ライブコピーの概要で、サイトに多数のライブコピーがある場合にエントリの読み込みに要する時間を短縮します(+200)。

* テンプレートエディターで、テンプレートエディターの動作が遅くなる可能性のある不完全なURLまたは無効なURLの処理を改善します。

さらに、AEM 6.5 SP4以降、スタイルシステムが強化され、コンポーネントダイアログ内でスタイルを選択できるようになりました。
