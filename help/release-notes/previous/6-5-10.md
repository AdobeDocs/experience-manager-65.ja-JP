...
[!DNL Adobe Experience Manager] 6.5.10.0 に導入された主な機能および機能強化は次のとおりです。
...
* **[!DNL Content Fragments]**のGraphQL API: GraphQL APIをサポートするために、コンテンツフラグメントモデルプロパティフィールドでハイフンを使用できなくなりました。コンテンツフラグメントモデルのプロパティ名にハイフンが含まれている場合、GraphQL クエリは不要な結果を返す可能性があります。
プロパティ名には次の文字のみが使用できます：A-Za-z0-9_。数字は最初の位置に置くことができません。
...
* **Camera Raw サポート**: [!DNL Adobe Camera Raw] v10.4をサポートする新しい[!DNL Camera Raw] パッケージを利用できます。  [!DNL Camera Raw]](/help/assets/camera-raw.md)を使用して画像を[処理するを参照してください。

* ビルトインのリポジトリ（Apache Jackrabbit Oak）を 1.22.8 に更新。
...
* [プラットフォームのアップデート](/help/forms/using/aem-forms-jee-supported-platforms.md)：JEE 上の [!DNL Adobe Experience Manager Forms] は、以下のプラットフォームをサポートするようになりました。
...
以下は、[!DNL Experience Manager] 6.5.10.0リリースで提供される修正のリストです。
...
* Web ページのリンクをカスタムサーブレットフィルターバンドルなどで変更すると、web ページのレンダリングに失敗します。 （NPR-36857） <!-- Note updated as per CQDOC-19203, April 07, 2022 -->
...
* サブページが多いサイト構造がある場合、Experience Manager 6.4.8.2と比較して、Experience Manager 6.5.8では、列ビューでのサブページの読み込みに時間がかかります（CQ-4322766）。
...
* スクリーンリーダーが、「[!UICONTROL インタラクティブビデオを作成]」ページの「[!UICONTROL コンテンツ]」タブで、状態（選択または未選択）と画像へのリンクの切替スイッチを正しく読み上げるようになりました（CQ-4290707）。
...
* 視覚的な問題があるユーザーにとって、一部のテキストおよびコントロール名は、輝度比が4.5:1の最小必要比率を満たしているため、読みやすくなります（NPR-36503）。
...
* テキストドキュメントフラグメントの編集後にインタラクティブなコミュニケーションの印刷プレビューを実行するたびに、タブ文字が 2 つの単語間で重複する（NPR-37021）。

* 最大サイズ制限を超えるテキストドキュメントフラグメントを保存すると、[!DNL AEM Forms] にエラーが表示されます（NPR-36874）。

* インタラクティブなコミュニケーションに画像を追加すると、その画像の後に追加の空のブロックが表示される （NPR-36659）。
...
* AEM Forms Service Pack 8 にアップグレードした後、ワークベンチプロセスにフォームを送信できません（CQ-4325846）。
...
UberJar for Experience Manager 6.5.10.0 は、[Maven Central リポジトリー](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.10/)で入手できます。
...
## Experience Manager 6.5.10.0の既知の問題 {#known-issues}
