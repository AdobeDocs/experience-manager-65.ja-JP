...
[!DNL Experience Manager] 6.5.14.0 には、2019年4月の 6.5 の初公開以降にリリースされた新しい機能、お客様から要望のあった主な機能強化、バグ修正およびパフォーマンスや安定性、セキュリティの向上が含まれています。 [このサービスパック](#install)を [!DNL Experience Manager] 6.5 にインストール<!-- UPDATE FOR EACH NEW RELEASE -->

主な機能と改善点は次のとおりです。

* **[AEM Forms Designerでのリストのアクセシビリティのサポート &#x200B;](https://helpx.adobe.com/content/dam/help/ja/experience-manager/6-5/forms/pdf/using-designer.pdf)**: AEM Forms Designerでは、作成者が「アクセシビリティ」タブを使用して、リスト、リスト項目、およびリスト ラベルの役割をサブフォームに割り当てることで、アクセス可能なリスト構造を作成できるようになりました。 この機能は、スクリーンリーダーの互換性のために、シンプル、番号付き、箇条書き、ネストされたリストをサポートしており、適切な`<L>`、`<LI>`、`<Lbl>`、および`<Lbody>` タグ生成が可能です。 この機能強化により、フォームリストがPDF/UAのアクセシビリティ標準に準拠するようになり、支援テクノロジーユーザーのナビゲーションとコンテンツの理解が向上します。
...
* ユーザーがインタラクティブなコミュニケーション（IC）で `&` 記号を入力すると、対応する IC のドラフトが読み込まれません。 （CQ-4343969）
...
* AEM Formsでは、log4j 1.xを使用します。 log4j 1.xのサポートが終了しました。 （NPR-38273）
...
* Forms DesignerのハイパーリンクにスクリーンReaderテキストを指定する機能を追加しました。（NPR-36221）
...
* コンテンツフラグメントモデルを作成し、ドロップダウンタイプの列挙フィールドを追加すると、_`is required`_の正しい検証ができません。 （NPR-38237）
...
* [!DNL Experience Manager] 6.5.14.0には[!DNL Experience Manager] 6.5が必要です。 詳しい手順については、[&#x200B; アップグレードドキュメント &#x200B;](/help/sites-deploying/upgrade.md)を参照してください。<!-- UPDATE FOR EACH NEW RELEASE -->
...
* 非推奨の `/oak:index/lucene` インデックスは `FileNotFoundException` を発生させ、インデックスを無限ループに陥れます。回避策として、AEM 起動時に、システムプロパティ `oak.indexTracker.autoRefresh` を次のように 3153600000000 に追加します。
