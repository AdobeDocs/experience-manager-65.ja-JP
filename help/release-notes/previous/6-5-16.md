...
* [!DNL Experience Manager] のユーザインターフェイスが、次の機能を含む DASH ストリーミング用に変更されました。
   ...
   * オプション **[!UICONTROL auto]**、**[!UICONTROL hls]**&#x200B;および&#x200B;**[!UICONTROL dash]**&#x200B;が、ビューアプリセットエディターの「**[!UICONTROL ビヘイビアー]**」タブの再生ドロップダウンリストで使用できるようになりました。
（ASSETS-17428）
...
* OutputServiceとAssemblerService APIは、PDF フォームをPDF/Aに変換できません。 （NPR-39990）
...
* ユーザーがforms-users グループ内のユーザーとアダプティブフォームを作成しようとすると、テンプレートを選択するオプションが表示されず、次のようなエラーが発生します。
...
* Experience Manager 6.5からAdobe Search&amp;Promote コードと依存関係を削除します。 Adobe Search&amp;Promote、2022年9月に提供終了。 詳しくは、「[Adobe Search&amp;Promote のサービス終了に関するお知らせ](https://experienceleague.adobe.com/docs/discontinued/using/search-promote.html?lang=ja)」を参照してください。 （NPR-39706）
...
* [!DNL Experience Manager] 6.5.16.0には[!DNL Experience Manager] 6.5が必要です。 詳しい手順については、[&#x200B; アップグレードドキュメント &#x200B;](/help/sites-deploying/upgrade.md)を参照してください。<!-- UPDATE FOR EACH NEW RELEASE -->
...
* コンテンツフラグメントまたはサイト/ページを移動/削除/公開しようとすると、バックグラウンドクエリが失敗したため、コンテンツフラグメントの参照を取得する際に問題が発生します。つまり、機能は機能しません。
