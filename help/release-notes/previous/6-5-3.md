...
* ビルトインのリポジトリ（Apache Jackrabbit Oak）をバージョン 1.10.6 に更新しました。
...
* [バッチモードでインタラクティブなコミュニケーションを生成](/help/forms/using/generate-multiple-interactive-communication-using-batch-api.md)できます。
...
* アセットのジョブの詳細をポーリングすると、[!DNL Experience Manager] で例外が表示されます（CQ-4283569）。
