...
* AEM 6.2、6.3、および6.4を実行しているお客様は、AEM 6.5への直接インプレースアップグレードをサポートしています。 5.xまたは6.0/6.1を使用しているお客様で、インプレースアップグレードを使用する場合は、まず6.4にアップグレードする必要があります。 その後、6.5 にアップグレードするか、インスタンス間のコンテンツの転送を通じて AEM 6.5 に直接アップグレードします。
* アップグレード手順は 6.5 でもほぼ変わりません。
* 私たちは、6.4で導入された「後方互換性」、「アップグレード複雑性評価」および「持続可能なアップグレード」機能を引き続きサポートします。 必要に応じて、これらの領域にバージョン固有の更新が行われました。
...
Commerce integration frameworkの機能強化は、AEM 6.4以降のリリースサイクルが短縮されました。 [AEMとAdobe Commerceの連携に関する詳細は、Commerce integration framework](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/content-and-commerce/integrations/magento.html?lang=ja)を参照してください。
...
**@Mentions サポート**
AEM Communitiesでは、登録ユーザーがUser Generated Contentで、他の登録メンバーにタグ付け（メンション）して関心を引き出せるようになりました。 タグ付け（メンション）されたメンバーには、対応するユーザー生成コンテンツへのディープリンクが付いた通知が届きます。 ただし、ユーザーは web とメールでの通知を無効にしたり有効にしたりすることができます。
...
[!DNL Adobe Experience Manager]では、[!DNL Adobe Creative Cloud]と統合し、クリエイティブ チームとマーケティング チームまたはビジネス チームが緊密に連携するワークフローで使用するアセットを共有するための様々な方法を提供しています。[!DNL Experience Manager] 6.5では、統合の改善を引き続き行い、さらに合理化して、より多くの機会を公開し、既存の方法を合理化します。
