...
* **Document Services**
   ...
   * OSGI および JEE 環境で AEM 6.5.15.0 サービスパックにアップグレードすると、特定のテンプレートを使用するアセンブラーサービスが動作しなくなります。 （FORMS-9355、FORMS-9445、FORMS-9408）
...
* **アダプティブフォーム**
   ...
   * ユーザーが複雑なJSON スキーマを使用してアダプティブフォームを作成しようとすると、失敗します。エラーは次のように発生します。
...
* **インタラクティブなコミュニケーション**
   ...
   * Experience Manager サービスパック 16 をサーバーにインストールした後、すべてのインタラクティブなコミュニケーションレターで、これらのレターを編集しようとするとクロックを開始します。 プロパティページをプレビューまたは表示／編集するためのサンプルペイロードが提供されている場合は、機能します。 ただし、レターを編集することはできません。 （FORMS-9067）
...
* [!DNL Experience Manager] 6.5.18.0には[!DNL Experience Manager] 6.5が必要です。 詳しい手順については、[ アップグレードドキュメント ](/help/sites-deploying/upgrade.md)を参照してください。<!-- UPDATE FOR EACH NEW RELEASE -->
...
1. OSGi バンドル `org.apache.jackrabbit.oak-core`はバージョン 1.22.16以降です（Web コンソールを使用：`/system/console/bundles`）。
...
* コンテンツフラグメント、サイト、ページのいずれかを移動、削除、または公開しようとすると、背景クエリが失敗したため、コンテンツフラグメントの参照が取得される際に問題が発生します。つまり、機能は機能しません。
...
* JEE インストーラーにAEM Formsをインストールする場合、JDK 11.0.20はサポートされていません。JEE インストーラーにAEM Formsをインストールできるのは、JDK 11.0.19以前のバージョンのみです。（FORMS-10659）
...
* AEM Service Pack 6.5.18.0のフルインストーラーをインストールした後、JBoss® Turnkeyを使用してJEEでEAR デプロイメントが失敗します。
この問題を解決するには、設定マネージャーを実行する前に、`<AEM_Forms_Installation_dir>\jboss\bin\standalone.bat` ファイルを見つけ、すべての出現箇所について`Adobe_Adobe_JAVA_HOME`から`Adobe_JAVA_HOME`に更新します。（CQDOC-20803）
...
* AEM サービスパック 18 にアップグレードした後は、インタラクティブなコミュニケーションレターを編集することはできません。 （FORMS-10578）。 この問題のホットフィックスが入手可能です。 ホットフィックスをダウンロードしてインストールするには、[Adobe Experience Manager Forms のホットフィックス](/help/release-notes/aem-forms-hotfix.md)を参照してください。
