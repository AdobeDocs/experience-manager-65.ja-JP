...
* 非推奨 `/oak:index/lucene` インデックスは、 `FileNotFoundException` これはインデックスを無限ループに送ります。回避策は、システムプロパティを追加することです。 `oak.indexTracker.autoRefresh` を3153600000000(AEMの起動時 ) に次のように変更します。
