...
[!DNL Experience Manager] 6.5.15.0 には、2019年4月の 6.5 の初公開以降にリリースされた新しい機能、お客様から要望のあった主な機能強化、バグ修正およびパフォーマンスや安定性、セキュリティの向上が含まれています。 [このサービスパック](#install)を [!DNL Experience Manager] 6.5 にインストールします。<!-- UPDATE FOR EACH NEW RELEASE -->
...
* アダプティブフォームのチェックボックス、ドロップダウンリスト、またはラジオボタンフィールドで`=`の署名を使用し、レコードのドキュメントが生成された場合、生成されたレコードのドキュメントに`=`の署名が表示されません（NPR-38859）。
...
* IBM® WebSphere®を使用してデプロイされたAEM FormsからAEM Forms Service Pack 14 （SP14）を適用すると、データベースの初期化中にブートストラップが失敗し、`java.lang.NoClassDefFoundError:org/apache/log4j/Logger` エラーが発生します（NPR-39414）。
...
* ユーザーがConfiguration Managerを使用してコネクタ設定を更新しようとすると、失敗します。（CQ-4347077）
...
* AEM Designerでは、キャプションのないフォームフィールドがチェックボックスを含むテーブルセルに配置されます。（LC-3920410）
...
* リスト表示でのタイトルの順序の並べ替え（昇順と降順の両方）。タイトルの順序がA、C、Bの順である方法。 （SITES-7585）
...
* [!DNL Experience Manager] 6.5.15.0には[!DNL Experience Manager] 6.5が必要です。 詳しい手順については、[ アップグレードドキュメント ](/help/sites-deploying/upgrade.md)を参照してください。<!-- UPDATE FOR EACH NEW RELEASE -->
...
1. OSGi バンドル `org.apache.jackrabbit.oak-core`はバージョン 1.22.13以降です（Web コンソールを使用：`/system/console/bundles`）。
...
* [GraphQL インデックスパッケージ 1.0.5を使用したAEM コンテンツフラグメント](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Fadobe%2Fpackages%2Fcq650%2Ffeaturepack%2Fcfm-graphql-index-def-1.0.5.zip)
...
* 非推奨の `/oak:index/lucene` インデックスは `FileNotFoundException` を発生させ、インデックス作成を無限ループに陥れます。回避策として、（AEM 起動時に）システムプロパティ `oak.indexTracker.autoRefresh` を次のように 3153600000000 に追加します。
...
* AEM Forms JEE 6.5.15.0 サービスパックが JBoss Linux 環境に正しくインストールされていません。 `PatchInstallerProcessing[1-9*].log` ファイルでログエントリ `[AEM_Forms_JEE_DIR]/patch/AEMForms-6.5.0-0057/xml/RUP_BOM.xml not found! Assuming this component is not in the installation. Skipping Processing` を確認して、AEM Forms JEE 6.5.15.0 サービスパックが適切にインストールされていることを検証します。 このエントリは、AEM Forms JEE 6.5.15.0 サービスパックのインストールが成功しなかったことを示しています。 AEM Forms JEE 6.5.15.0 サービスパックのインストールに関する問題を修正するには、XML ディレクトリに手動で [RUP_BOM.xml ファイルを追加](/help/forms/using/linux-jboss-installation-issue-on-sp15.md)します。
