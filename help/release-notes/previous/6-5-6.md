...
Adobe Experience Manager 6.5.6.0は、2019年4月&#x200B;**の6.5 リリースの一般公開以降にリリースされた、新機能、お客様から要求された主な機能強化、パフォーマンス、安定性、セキュリティの機能強化を含む重要なアップデートです。** Adobe Experience Manager 6.5 にインストールできます。

Adobe Experience Manager 6.5.6.0で導入された主な機能と機能強化には、次のものが含まれます。
...
* ビルトインのリポジトリ（Apache Jackrabbit Oak）をバージョン 1.22.4 に更新しました。

以下は、[!DNL Experience Manager] 6.5.6.0リリースで提供される修正のリストです。
...
[!DNL Adobe Experience Manager] 6.5.6.0 [!DNL Assets]は、次の問題を修正します。
...
* テキスト抽出と、アップロードされた PDF ファイルのフルテキスト検索が機能しません（NPR-34164）。 この問題を修正するには、Service Pack 6 をインストールした後、[!DNL sAdobe Experience Manager] のデプロイメントを再起動します。
...
* [!DNL Experience Manager] 6.5 サービスパック 5または以前のバージョンを[!DNL Experience Manager] 6.5にインストールすると、アセットカスタムワークフローモデルの実行時コピー（`/var/workflow/models/dam`で作成）が削除される（NPR-34532）。 実行時コピーを取得するには、HTTP APIを使用して、ワークフローモデルのデザイン時コピーを実行時コピーと同期します。
...
* ページの読み込み時に、アセットの詳細ページでユーザーがサイドレールオプション（「[!UICONTROL 概要]」、「[!UICONTROL タイムライン]」、「[!UICONTROL ビューア]」など）を切り替えると、アセットの表示がちらつく（NPR-34235）。
...
* [!UICONTROL ビューアプリセットエディター]の「[!UICONTROL 動作]」タブにあるシンボルがローカライズされていません（CQ-4299026）。
...
* ネストされた繰り返し可能なコンポーネントをテキストドキュメントフラグメントに含めると、インタラクティブなコミュニケーションの保存に失敗する（NPR-34095）。
...
### 翻訳済みアダプティブフォームページ （6.5.6.0）のキャッシュが強化されました {#enhanced-caching-translated-adaptive-forms}
...
### フォームデータモデルサービスの出力を変数（6.5.6.0）に保存します {#save-fdm-service-to-variable}
...
### ファイル添付コンポーネント （6.5.6.0）に複数のファイルを添付 {#attach-multiple-files}

アダプティブフォームの [!UICONTROL 添付ファイル] コンポーネントに、[複数のファイルを添付](/help/forms/using/introduction-forms-authoring.md)できるようになりました。

## UberJar {#uber-jar}

UberJar for Experience Manager 6.5.6.0 は、[Maven Central リポジトリー](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.6/)で入手できます。
