...
>[!NOTE]
>
>UberJar およびその他の関連アーティファクトは、アドビの Maven 公開リポジトリー（`repo.maven.apache.org`）ではなく Maven Central リポジトリーで入手できます。メインの UberJar ファイルの名前は `uber-jar-<version>.jar` に変更されています。そのため、`dependency` タグには、値として `apis` を使用する `classifier` はありません。
