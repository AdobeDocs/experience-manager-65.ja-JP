...
* pageinfo.json サーブレットが LaunchManagerImpl.getLaunchStream(SITES-3489) で停止するので、we-retail サイトと wknd サイトのテンプレートページは壊れています。コンポーネントが読み込まれず、構造オプションを使用できません。
...
* [Platform] commons-httpclient の metatype.xml ファイルの xmlns:metatype のバージョン番号が、「https://www.osgi.org/xmlns/metatype/v1.2.0」ではなく「https://www.osgi.org/xmlns/metatype/v1.0.0」です（NPR-37865）。
...
* クラシックユーザーインターフェイスエディターが、タッチユーザーインターフェイスの比較ビューでマークアップを表示しない (NPR-37588)。
...
* [コマース] ルートフォルダーのアクティベートを解除しても、 [!DNL Experience Manager Commerce] コンソール。また、非アクティブ化時のルートフォルダーの子フォルダーの数が、ユーザーインターフェイスに誤って表示される (CQ-4338261)。
