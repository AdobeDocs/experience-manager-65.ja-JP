...
Adobe Experience Manager 6.5.4.0 は、**2019 年 4 月** に 6.5 リリースが一般リリースされた後にリリースされた、新機能、お客様からリクエストされた主な機能強化、パフォーマンス、安定性、セキュリティの改善を含む重要なアップデートです。 Adobe Experience Manager 6.5 にインストールできます。

Adobe Experience Manager 6.5.4.0 で導入された主な機能および機能強化は次のとおりです。
...
### [!DNL Experience Manager Assets] を使用して [!DNL Brand Portal] を構成する（6.5.4.0） {#configure-assets-bp}
