...
Adobe Experience Manager 6.5.4.0は、2019年4月&#x200B;**の6.5 リリースの一般公開以降にリリースされた、新機能、主なお客様から要求された機能強化とパフォーマンス、安定性、セキュリティの機能強化を含む重要なアップデートです。** Adobe Experience Manager 6.5 にインストールできます。

Adobe Experience Manager 6.5.4.0で導入された主な機能と機能強化には、次のようなものがあります。
...
* [複数列のサポート](/help/forms/using/resize-using-layout-mode.md) アダプティブフォームとインタラクティブなコミュニケーションのレイアウトモード用。
...
* ビルトインのリポジトリ（Apache Jackrabbit Oak）をバージョン 1.10.8 に更新しました。
...
* ${extension}を含むアセットのレンディション生成が失敗する（NPR-32294）。
...
* 日付と時刻の形式の制約は、日付フィールドのフィールドラベルで提供され、ユーザーがキーボードを使用して正しい形式で日付を入力できるようにします。
例：`On Time (MM-DD-YYYY HH:mm)`。ここで、MMは2桁の形式で月、YYYYは年、DDは2桁の形式で日、HHは24時間の軍事形式で時間、mmは分です。
...
### [!DNL Experience Manager Assets]を[!DNL Brand Portal]で設定（6.5.4.0） {#configure-assets-bp}
...
* ワークベンチ：Experience Utilities サービスの「GetPDF のプロパティ」操作を使用すると、PDF メッセージが表示される（NPR-32150）。
...
UberJar for Experience Manager 6.5.4.0 は、[Maven Central リポジトリー](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.4/)で入手できます。
