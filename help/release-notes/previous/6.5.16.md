...
* GraphQL クエリでは、`fragments` インデックスの代わりに `damAssetLucene` インデックスを使用する場合があります。その結果、GraphQL クエリが失敗するか、実行に長い時間がかかる可能性があります。
...
* AEM 6.5.15 以降、```org.apache.servicemix.bundles.rhino``` バンドルで提供される Rhino JavaScript エンジンには、新しく巻き上げ動作が追加されました。strict モード（```use strict;```）を使用するスクリプトでは、変数を正しく宣言する必要があります。誤って宣言した場合、スクリプトは実行されず、ランタイムエラーが発生します。
