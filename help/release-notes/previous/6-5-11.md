...
* [!DNL Experience Manager] 6.5.8.0 からバージョン 6.5.9.0 にアップグレードすると、インストールによって `/content/dam/projects` のプロパティが上書きされます。 フォルダーに割り当て済みのメタデータスキーマとプロパティはデフォルトにリセットされます（NPR-37124）。
...
* [!DNL Experience Manager] 6.5.9.0 にアップグレード後、翻訳プロジェクトにページを追加すると、空のエラーメッセージが表示されます（NPR-37105）。
...
* AEM 6.5.10.0 をインストールすると、XDP フォームのHTML プレビューが機能しない（NPR-37503、CQ-4331926）。
...
**AEM Forms6.5.11.1** で修正された問題

>[!NOTE]
>
>AEM 6.5.11.0 Formsにアップグレードしていない場合は、AEM Forms 6.5.11.1 アドオンパッケージを直接インストールします。 AEMをForms6.5.11.0 インストール済みの場合、AdobeはAEM 6.5.11.1 Formsにアップグレードすることをお勧めします。

* 「送信アクション」、「メールを送信」および「AEMを起動ワークフロー」は、Forms 6.5.11.0 アドオンパッケージのインストール後、機能しなくなります。
* Forms 6.5.11.0 アドオンパッケージをインストールした後、Microsoft Word ドキュメントのPDF ドキュメントへの変換を停止します。
* （JEE のみ）Apache Log4j2 に対して報告された重大なセキュリティ脆弱性（CVE-2021-44228 および CVE-2021-45046）。
* （JEE のみ） 6.5.11.0 のパッチの Assembler DSC に、仕様バージョンや impl バージョンなどの誤った metainfo が含まれています。
...
* コンテンツフラグメントとGraphQLを使用している場合は、6.5.12.0 にアップグレードして、適切なパッケージをインストールすることをお勧めします。
   * 6.5.12.0 へのアップグレードが選択肢にない場合は、サポートに連絡して、対応策が実装可能かどうかを評価してください。
