...
[!DNL Experience Manager] 6.5.19.0 には、2019 年 4 月の 6.5 リリースの公開当初にリリースされた新機能、お客様からリクエストされた主な機能強化、バグ修正およびパフォーマンス、安定性、セキュリティの向上が含まれています。 [!DNL Experience Manager] 6.5 で[このサービスパックをインストール](#install)します。
...
* AEM Forms サービスパック 6.5.15.0 に forms アドオンパッケージをインストールした後、PDF ドキュメントを生成しようとすると、断続的に失敗して次のエラーが表示されます。
...
* JBoss® 7.1.4 プラットフォームでExperience Manager 6.5.16.0 以降のサービスパックをインストールすると、デプロイ `adobe-livecycle-jboss.ear` 失敗します。 （CQ-4351522、CQDOC-20159）
...
#### サーブレットフラグメントのインストール（AEM サービスパック 6.5.14.0 以前）

* AEM Service Pack 6.5.15.0*以降にアップグレードし、AEM インスタンスが Tomcat 8.5.88 で動作している場合は、（事前に* サーブレットフラグメントをインストールする必要があります。サービスパック 6.5.15.0 以降のインストールに進みます。
...
* AEM サービスパック 6.5.14.0 以降をインストールすると、フォントリストが空なので、`Home`/`Services`/`PDF Generator`/`Adobe PDF Settings` に移動しても、PDF ドキュメント用の JEE 管理 UI からフォントを選択できません。 （FORMS-12095）
...
* Java EE web アプリケーションを開発するための一般的なオープンソース web アプリケーションフレームワークである Struts 2 RCE には、重大なセキュリティ脆弱性が報告されています。Adobeは、JEE 上のAEMの脆弱性に対処するために、[AEM Forms 6.5 サービスパック 19.1 （6.5.19.1） ](/help/forms/using/mitigating-struts-2-rce-vulnerabilities-for-experience-manager-manager-form.md) をリリースしました。
...
* 顧客が多数のフィールドを含むメタデータスキーマを保存すると、フォームビルダーのマルチパート/フォームデータ要求が maxFileCount 制限を超え、サイレントエラーが発生します。 （CQDOC-22465）
