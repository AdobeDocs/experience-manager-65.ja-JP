...
* アダプティブフォームを送信すると、間違った値がチェックボックスに保存されます。 各「,」に「/」を追加します。 （FORMS-12093）
* コアコンポーネントに基づくアダプティブフォームを送信すると、フォームが送信できません。 機能の切り替え – ft-cq-4324864 を適用して問題を修正します。 機能の切り替えを適用するには：
   1. 次の URL を開きます。 `http://[server]:[port]/system/console/configMgr`
   1. を検索して開きます。 `com.adobe.granite.toggle.impl.dev.DynamicToggleProviderImpl` 設定
   1. フィーチャーのツールナンバーを指定します。 例：ft-cq-4324864
   1. 設定を保存します。これで、この機能が有効になります。 （FORMS-12789）
...
* PDF Generatorサービスは、サーバーで使用可能なフォントを列挙できません。 その結果、PDFG 管理 UI の Adobe PDF 設定ページのフォント選択パネルには何も表示されず、選択したフォントの埋め込み（解除）を効果的に防ぐことができます。（FORMS-12095）
