...
* **[AEM Forms Designerのアクセシビリティの機能強化](https://helpx.adobe.com/content/dam/help/ja/experience-manager/6-5/forms/pdf/using-designer.pdf)**: AEM Forms Designerでは、次のアクセシビリティの機能強化が導入されています。

   * AEM Forms Designerでは、新しい`<TOC>`および`<TOCI>` タグを使用してアクセス可能な目次がサポートされるようになりました。作成者は、「アクセシビリティ」タブを使用して、これらのロールをサブフォームに割り当てることができます。 この機能は、マルチレベル目次構造、ラベル付きエントリ、番号付き参照、スパンサブ構造をサポートしており、スクリーンリーダーが目次コンテンツを適切に識別して移動できるようにします。

   * AEM Forms Designerでは、スコープ、行スパン、列スパンなどのテーブルセルに対するアクセシビリティプロパティの設定がサポートされるようになりました。これにより、スクリーンリーダーやアシスタントデバイスは、生成されたPDFのテーブル構造を正しく解釈できるようになりました。 作成者は、セルのプロパティ ダイアログから直接これらのプロパティを設定し、XML Source ビューで確認できます。 設定されたプロパティは、タグ付きのPDF出力に正確に反映され、Acrobat テーブルエディターを使用して検証できます。
...
* 署名コンポーネント（`/apps/fpl/components/campaign/signature`）では、Link Externalizer が機能していませんでした。 画像タグの上の HTML コメントを削除した場合、ドメインが画像ソースに追加されませんでした。 この問題は、ステージング環境ではなく、本番環境の署名コンポーネントでのみ発生しました。 （SITES-16120）
...
* ユーザーがワークベンチプロセスを使用して XML 出力を生成する場合、一部のファイルで失敗します。 （FORMS-11314）
...
* Forms Designerのタグ付けが正しい場合でも、PDFでタグの順序がランダムに変更されます。（LC-3921313） 
...
* [!DNL Experience Manager] 6.5.19.0には[!DNL Experience Manager] 6.5が必要です。 詳しい手順については、[&#x200B; アップグレードドキュメント &#x200B;](/help/sites-deploying/upgrade.md)を参照してください。<!-- UPDATE FOR EACH NEW RELEASE -->
* サービスパックは、アドビの[ソフトウェア配布](https://experience.adobe.com/#/downloads/content/software-distribution/jp/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.19.0.zip)からダウンロードできます。
...
1. OSGi バンドル `org.apache.jackrabbit.oak-core`はバージョン 1.22.17以降です（Web コンソールを使用：`/system/console/bundles`）。
