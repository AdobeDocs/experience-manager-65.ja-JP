...
>[!NOTE]
>
>UberJar およびその他の関連アーティファクトは、アドビの Maven 公開リポジトリ （`repo.maven.apache.org`） ではなく Maven 中央リポジトリで入手できます。メインの UberJar ファイルの名前が `uber-jar-<version>.jar` に変更されます。そのため、`dependency` タグには、値として `apis` を使用する `classifier` はありません。