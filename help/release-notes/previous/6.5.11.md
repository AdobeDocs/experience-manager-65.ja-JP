---
title: リリースノート： [!DNL Adobe Experience Manager] 6.5.11.0
...
* ユーザーが XDP フォームをPDFからHTML5 フォームに変換しようとしたとき。 カンボジアフォント用のHTML5 フォームのレンダリング中に発生する問題を以下に示します。
   * 改行は、Forms5 でレンダリングされます。HTML5 では、 （CQ-4322508）
   * リストがHTML5 のフォームで正しくレンダリングされない。 （CQ-4322317）
   * Forms のレンダリング中に余分なスペースがHTML5 フォームのレンダリング中に表示されます。 （CQ-4322509）
...
* ユーザーが XDP フォームをPDFからHTML5 フォームに変換しようとすると、修正された問題の一部は次のようになります。
   * LTR としてレンダリングする Farsi のサポート。
   * LTR テキストと混在する場合の RTL のサポート。
   * 画像の回転のサポート。
   * カンボジア語のサポートが追加されました。 サポートのコンボディアンサポートを有効にするには、次のようにフラグを有効にする必要があります。 `CTL=true` HTML5 フォームのレンダリング中
