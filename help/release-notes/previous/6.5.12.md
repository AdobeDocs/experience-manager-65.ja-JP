...
* 編集したテンプレートを使用してエクスペリエンスフラグメントを作成しようとしても、初期ページのプロパティに対して行った編集が表示されない（NPR-37962）。
...
* [Commerce] ルートフォルダーを無効にしても、[!DNL Experience Manager Commerce] コンソールで子製品の無効ステータスが変更されない。さらに、無効化した時のルートフォルダーの子フォルダーの数が、ユーザーインターフェイスに誤って表示される（CQ-4338261）。
