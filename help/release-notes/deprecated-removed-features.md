...
| Adobe Target | AEM 6.5の[!DNL Adobe I/O]ベースのAdobe Target標準API(Rest API)を使用してAEMがAdobe Targetサービスに接続する機能を追加し、ターゲットクラシックAPI(XML)の使用は廃止されました。 | 統合を[新しいAPI](https://helpx.adobe.com/jp/experience-manager/kt/sites/using/aem-sites-target-standard-technical-video-understand.html)を使用するように再設定します。 |
