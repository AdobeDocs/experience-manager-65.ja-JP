...
| アセット | ユーザーが`/content/dam/collections`に対する十分な（読み取りと書き込みの）権限を持っていない場合、ユーザーはコレクションを作成できません。 | ユーザーのアクセス制御設定を保持し、適切な権限を確保します。 |
...
| Adobe Target | AEM 6.5 では、Adobe I/O ベースの Adobe Target Standard API（Rest API）を使用して AEM が Adobe Target サービスに接続できるようになったので、Target Classic API（XML）方式は廃止されました。 | 統合を[新しいAPI](https://helpx.adobe.com/jp/experience-manager/kt/sites/using/aem-sites-target-standard-technical-video-understand.html)を使用するように再設定します。 |
| Adobe Target | AEMでの`mbox.js`ベースの統合のAdobe Targetとの使用は推奨されません。 | `at.js` 1.xを使用するように切り替えます。 |
...
| フォーム | JEE上のAEM FormsからUPDベースのクラスタリングのサポートを削除しました。 | JEE上のAEM Formsでは、TCPベースのクラスタリングのみを使用できます。 UDPマルチキャストサーバーを以前のバージョンからAEM 5.5Forms（JEE上）にアップグレードする場合は、手動設定を実行してTCPベースのGemfireクラスタリングに切り替えます。 詳細な手順については、「[JEE上のAEM 6.5 formsへのアップグレード](../forms/using/upgrade-forms-jee.md)」を参照してください。 |
...
| キャッシュ | `system/console/slingjsp` は、AEM 6.5では使用できなくなりました。 | クラスとスライトリーキャッシュは、Apache Sling Commons FileSystem ClassLoaderバンドルの下に格納されます。 AEM Webコンソールでバンドル番号を確認し、キャッシュフォルダーをファイルシステム(`crx-quickstart/launchpad/felix/bundle<ID>`)から直接削除できます。 |
