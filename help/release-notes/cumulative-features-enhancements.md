...
* **Dynamic Media _スナップショット_**を使用すると、テスト画像またはDynamic Media URLを使用して、画像修飾子やスマートイメージングの最適化（WebPまたはAVIF出力、帯域幅に応じた圧縮、デバイスピクセルレシオの拡大など）をプレビューできます。次に、各設定が品質とファイルサイズに与える影響をすぐに比較できます。
[Dynamic Media スナップショット ](https://experienceleague.adobe.com/ja/docs/experience-manager-learn/assets/dynamic-media/images/dynamic-media-snapshot)を参照してください。
...
* **Experience Manager SitesとコンテンツフラグメントをAssets Next-Generation Dynamic Mediaと統合** - ユーザーは、Experience Manager Sites 6.5でクラウドホスティングされたアセットを使用できるようになりました。 オンプレミスまたはManaged Servicesインスタンスでアセットをオーサリングし、配信することができます。
