...
で説明されているように [AEM ページテンプレート](/help/mobile/apps-architecture.md)を選択します。各コンポーネントは JavaScript フラグメントを出力し、 `data` 約束。 angular規則に従って、コントローラはスコープに変数を割り当てる場合にのみ使用してください。
...
このディレクトリにはが含まれます。 [CLI フック](https://gist.github.com/jlcarvalho/22402d013bc72f795d45a01836ce735c). hooks ディレクトリのフォルダーには、ビルド中の正確なポイントで実行される node.js スクリプトが含まれています。
