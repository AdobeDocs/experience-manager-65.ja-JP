...
ルートページは、アプリケーションの作成時に指定されたアプリケーションの宛先パスプロパティの下に格納されます（デフォルトパスは/content/phonegap/apps です）。 ページ名は、アプリケーションの Name プロパティです。 例えば、`myphonegapapp` という名前のサイトのルートページのデフォルト URL は `http://localhost:4502/content/phonegap/apps/myphonegapapp.html` です。
...
**メモ：** デフォルトの PhoneGap ブループリントは、英語のアプリケーションのみを作成します。 開発者は、ブループリントを変更して、より多くの言語バリエーションを作成できます。
...
* **名前：** デフォルト値は、スペース文字を削除したタイトルプロパティの値です。 名前は、CQ 内でアプリケーションを参照するために使用されます。例えば、アプリケーションを表すリポジトリーノード用に使用します。
* **説明：** アプリケーションの説明。
...
1. 作成アイコンをクリックします。
   ...
   ![ 正方形の内側にプラス記号が表示された「作成」アイコン ](do-not-localize/chlimage_1-7.png)
...
1. 設定するアプリケーションを選択し、「ページプロパティを表示」アイコンをクリックします。
   ...
   ![ 円の中のレター I で示される「ページプロパティを表示」アイコン ](do-not-localize/chlimage_1-8.png)

1. プロパティ値を変更するには、「編集」アイコンをクリックします。

   ![ 編集アイコンが鉛筆で示されます。](do-not-localize/chlimage_1-9.png)

1. 基本プロパティと詳細プロパティを設定し、完了アイコンをクリックします。

   ![ 完了アイコンにチェックマークが付きます。](do-not-localize/chlimage_1-10.png)
...
1. アプリAdmin Console内で編集するモバイルアプリをクリックしてドリルインします。 構成するアプリケーションの言語バージョンを選択し、[ アプリケーション プロパティの表示 ] アイコンをクリックします。
   ...
   ![ 円の中の文字 I で示される「アプリケーションのプロパティを表示」アイコン ](do-not-localize/chlimage_1-11.png)

1. プロパティ値を変更するには、「編集」アイコンをクリックします。

   ![ 編集アイコンが鉛筆で示されます。](do-not-localize/chlimage_1-12.png)

1. 「基本」、「詳細」、「ビルド」、「Analytics」の各タブでプロパティを設定し、完了アイコンをクリックします。

   ![ 完了アイコンにチェックマークが付きます。](do-not-localize/chlimage_1-13.png)
...
1. スプラッシュページを選択し、「キャッシュを更新」アイコンをクリックします。
   ...
   ![ ストライプのバレーで示される「キャッシュを更新」アイコンの上にリサイクル記号が表示されます。](do-not-localize/chlimage_1-14.png)
