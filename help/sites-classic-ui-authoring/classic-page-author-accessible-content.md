...
>[!NOTE]
>
>画像が装飾的なものである場合(「適切なテキストの代替オプションの [作成](#creating-good-text-alternatives)」を参照 **)は、スペースバーを使用して「** 代替テキスト」フィールドにスペースを入力できます。 空の `alt` 属性が作成され、スクリーンリーダーが画像を無視するように促されます。
