...
* 長い説明（100 文字を超える）は避ける必要があります。 代替テキストに詳細が必要な場合は、次のようにします。
...
* 一時停止、停止、非表示：情報の移動、点滅、スクロールまたは自動更新について、以下が該当する。
   ...
   * 動き、点滅、スクロール：動く、点滅、またはスクロールする情報に対して、
      * (a) が自動的に始まる。
      * (b) 五秒を超える持続時間
      * (c) 他のコンテンツと並行して提示され、動き、点滅、スクロールが必要な活動の一部でない限り、ユーザーが一時停止、停止、または非表示にする仕組みがある。
   * 自動更新：すべての自動更新情報に対して
      * (a) 自動的に開始し、
      * (b) 他のコンテンツと並行して提示され、自動更新が必要なアクティビティの一部でない限り、ユーザーが一時停止、停止または非表示にしたり、更新の頻度を制御したりするメカニズムがある。
