...
* 長い説明（100 文字以上）は避ける必要があります。 代替テキストに詳細が必要な場合は、次のようにします。
...
* 一時停止、停止、非表示：情報の移動、点滅、スクロールまたは自動更新について、以下が該当する。
   ...
   * 移動、点滅、スクロール：移動、点滅、スクロールに関する情報
      * （a）は自動的に開始し、
      * ロ五秒以上継続すること。
      * （c）他のコンテンツと並行して提示され、移動、点滅又はスクロールが必須の活動の一部でない限り、利用者が当該コンテンツを一時停止、停止又は非表示にする仕組みがあること。
   * 自動更新：以下を行う自動更新情報
      * （a）自動的に開始し、
      * （b）他のコンテンツと並行して提示される場合、自動更新が必須のアクティビティの一部である場合を除き、ユーザーが一時停止、停止、非表示にしたり、更新の頻度を制御したりする仕組みがあります。
