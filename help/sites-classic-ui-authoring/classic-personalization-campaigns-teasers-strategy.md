...
>[!NOTE]
>
>ティーザーコンポーネントは、AEM 6.2 では非推奨（廃止予定）となりました。代わりに、 [ターゲットコンポーネント](/help/sites-authoring/content-targeting-touch.md).
...
* この **ティーザーコンポーネント** は初期設定の状態で使用でき、コンテンツページに特定のティーザー段落のインスタンスを作成できます。 サイドキックからティーザーコンポーネントをドラッグし、ティーザー定義を指定して独自のティーザー段落を作成できます。 **注意：** ティーザーコンポーネントは、AEM 6.2 では非推奨（廃止予定）となりました。代わりに、 [ターゲットコンポーネント](/help/sites-authoring/content-targeting-touch.md).
