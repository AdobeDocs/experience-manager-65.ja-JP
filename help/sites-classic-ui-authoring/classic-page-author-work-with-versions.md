---
title: ページバージョンの処理
description: バージョン管理では、特定の時点でのページの「スナップショット」を作成します。
...
* ページのバージョンを作成します。
* ページを以前のバージョンに戻して、ページに対して行った変更を取り消します。
...
次の例では、赤い線は最初の緑のバージョンの期間にページが編集されたことを示し、黄色い線はページが赤のバージョンの間に編集されたことを示しています。
...
* **タイムワープはページの内容にのみ基づく** - Web サイトをレンダリングする要素（コード、CSS、画像アセットなど）が変更された場合、これらの項目はリポジトリ内でバージョン管理されないので、ビューは元のビューとは異なります。
