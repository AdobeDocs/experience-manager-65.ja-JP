...
そのためには、アセットが表示されるページ用のクラウド設定を最初に指定します。次に、Scene7 コンポーネントをページに追加して、そのコンポーネント上にアセットをドラッグ＆ドロップします。そのページのページプロパティが設定されると、選択したトリガーがScene7にアップロードする際に、**Scene7に公開**&#x200B;リンクが表示されます。
