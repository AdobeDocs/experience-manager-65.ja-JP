...
* この下で、 **Geometrixx**.
* の下 **Geometrixx**&#x200B;見つかった *基礎モードページ* 呼び出し **ニュース**. ダブルクリックしてこのページを開きます。
...
* **ターゲットテンプレート**:これは、この基礎モードがページの作成時に使用するテンプレートです。 この例では、 *Geometrixxコンテンツページ* テンプレート。
* **ターゲットパス**:これは、この基礎モードがページを作成する親ページのパスです。 この例では、パスは */content/geometrixx/en/news*.
...
新しい基礎モードを作成するには、 **ツール** コンソール、次に **デフォルトのページ基礎モード** ページを作成します。 単一のページテンプレートタイプを使用できます。 *基礎モードテンプレート*
...
* ロック記号（ほとんどのコンポーネントに対して）（例：テキストとタイトル）
