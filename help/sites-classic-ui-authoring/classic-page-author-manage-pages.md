...
ページのみを指定する場合 **タイトル** ページを作成する場合、AEMはページを派生します **名前** この文字列から、および [規則に従って名前を検証する](/help/sites-developing/naming-conventions.md) AEMと JCR によって課せられます。 両方の UI とも、「**タイトル**」フィールドに無効な文字を含めることはできますが、派生した名前の無効な文字は別の文字に置き換えられます。例：
...
このテンプレートは、サムネール画像やその他のプロパティなど、ページの構造を定義します。例えば、製品ページ、サイトマップ、連絡先情報用に別々のテンプレートを用意することができます。 テンプレートは、 [コンポーネント](#components).
...
1. 次から： **Web サイト** コンソールで、ページを作成するレベルを選択します。
...
1. **ページを作成**&#x200B;ダイアログでは次の操作を実行できます。
   ...
   * 「**名前**」を入力します。この項目は URI の生成に使用されます。指定しない場合、名前はタイトルから派生します。
      ...
      * ページを指定する場合 **名前** ページを作成する際に、AEM [の規則に従って名前を検証します。](/help/sites-developing/naming-conventions.md) AEMと JCR によってインポジトされます。
      * クラシック UI では、「**名前**」フィールドに&#x200B;**無効な文字は入力できません**。
   ...
   * 新しいページの作成に使用するテンプレートをクリックします。
   ...
   ページの作成に最低限必要な情報は、 **タイトル** および必要なテンプレート。
...
1. 「**作成**」をクリックしてページを作成します。**Web サイト**&#x200B;コンソールに戻ると、新しいページのエントリを確認できます。
   ...
   コンソールには、必要に応じて更新されるページに関する情報（最終編集日時やユーザーなど）が表示されます。

   >[!NOTE]
   >
   >また、既存のページの編集中にページを作成することもできます。使用 **子ページを作成** から **ページ** サイドキックの「 」タブで、編集するページのすぐ下にページが作成されます。
...
1. 新しい場所に移動して、次のいずれかをクリックします。
   ...
   * **Shift キーを押しながら「ペースト」**：選択したページのみをペーストします。
   ...
   >[!NOTE]
   >
   >サイドキックの「**ページ**」タブから「**ページをコピー**」を使用することもできます。これにより、宛先を指定できるダイアログボックスが開きます。
...
1. The **フォルダーを作成** ダイアログボックスが開きます。 ここで「**名前**」と「**タイトル**」を入力できます。
