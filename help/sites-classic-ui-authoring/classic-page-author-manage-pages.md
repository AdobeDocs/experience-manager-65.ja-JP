...
ページのみを指定する場合 **タイトル** ページを作成する場合、AEMはページを派生します **名前** この文字列から、および [規則に従って名前を検証する](/help/sites-developing/naming-conventions.md) AEMと JCR によって課せられます。 両方の UI とも、「**タイトル**」フィールドに無効な文字を含めることはできますが、派生した名前の無効な文字は別の文字に置き換えられます。次に例を示します。
...
1. The **ページを作成** ダイアログでは、次の操作を実行できます。
