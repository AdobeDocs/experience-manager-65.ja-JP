...
>
>Internet Explorer を使用する場合、アップロードした画像ファイルをプレビューに表示して変更（切り抜き）を実行できるようにするには、画像をアップロードし「**OK**」をクリックしてから画像を再度開く必要があります。
>
...
例えば、`Geometrixxe` と入力すると、「もしかして : Geometrixx」
...
**タイトル**&#x200B;コンポーネントの表示例を以下に示します。デザインはサイト固有の CSS で決まります。
...
* ビデオ
   ...
   * **サイズ**
      ...
      ビデオのネイティブサイズ（ピクセル単位の幅 × 高さ）が「サイズ」の横のボックスに表示されます（上の図を参照してください）。ビデオのネイティブ寸法を上書きする場合は、幅と高さの寸法をここに手動で入力します。「**OK**」を選択するとダイアログが閉じます。
...
フォームの例を以下に示します。 **フォーム**&#x200B;コンポーネント（開始および終了）、入力に使用する 2 つの&#x200B;**フォーム****テキスト**&#x200B;フィールド、導入部のテキストに使用する 1 つの&#x200B;**一般****テキスト**&#x200B;フィールド、および「**送信**」ボタンから構成されます。
...
**読み込み元パス**&#x200B;を使用して、フォームの必須フィールドに値をプリロードすることができます。[フォーム値のプリロード](/help/sites-developing/developing-forms.md#preloading-form-values)を参照してください。
   ...
   * **アクションの設定**
      ...
      「**アクション設定**」で選択できるオプションは、選択した「**アクションタイプ**」によって異なります。
...
>[!NOTE]
>
>**項目読み込みパス**&#x200B;を使用すると、チェックボックスグループのリストと値を事前に読み込むことができます。
>
