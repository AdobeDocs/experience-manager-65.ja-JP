...
は、 `mybrand.com/products/product-detail.1234.html`
...
作成者が、プロモーション目的で別の場所からアクセス可能なページを作成する場合、ページごとに定義される AEM のバニティー URL が役立つことがあります。To add a vanity URL for a page, navigate to it in the **Sites** console and edit the page properties. At the bottom of the **Basic** tab, you see a section where vanity URLs can be added. 複数のURLを介してページにアクセスできるように設定すると、ページのSEO値がフラグメント化されるので、この問題を回避するために、正規のURLタグをページに追加する必要があります。
...
>[!NOTE]
>
>このプロ `sling:alias` パティは、Page Propertiesを編集する際に [Aliasプロパティを使用して設定できます](/help/sites-authoring/editing-page-properties.md#advanced)。
