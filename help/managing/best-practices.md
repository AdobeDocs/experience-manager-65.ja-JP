...
* **フェーズのヘルス**
   ...
   * プロジェクトの高レベルのステータスインジケーター。危険な領域を強調するのに役立ちます。
...
この **ロール別ステータス** ワークシートは詳細な内訳を示します [**ヘルス**、**画質、 **完全性**](#projectheartbeat) 作成者 **[フェーズ](#phases-and-milestones)** および **[ペルソナ](#persona)**.
...
* **検証**
   ...
   * 誰が関わってるの？
   * 進行状況をどのように測定しますか？
   * プロジェクトの期間中、進捗状況をどのくらいの頻度でレビューしますか？
...
* **システム統合**
   ...
   * サードパーティ製システムの統合方法と共に、オフライン/オンライン、クライアント側/ブラウザー側、サードパーティシステムがダウンした場合のフォールオーバー処理などの特別な考慮事項も含まれます
...
* **テストの概念**
   ...
   * テストに必要なコンテンツの準備
   * 使用するテストツールの情報
   * テストに関与するユーザーの概要特に QA チーム外のグループ
...
* **パフォーマンステストと負荷テスト**
  ...
  >[!NOTE]
  >
  >このプロセスは、AEMの通常の使用中も継続する必要がありますが、これらの初期段階は最も重要です。
...
各 **必須ドキュメント**&#x200B;に設定する場合、ペルソナは次の内容を示す必要があります。
...
各 **成果物**&#x200B;に設定する場合、ペルソナは次を示す必要があります。
