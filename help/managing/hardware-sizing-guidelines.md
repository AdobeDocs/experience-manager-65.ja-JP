...
* **開発環境** 新しい機能の開発や重要な変更をおこなうために使用します。ベストプラクティスは、開発者ごとに開発環境を使用することです（通常は、個人用システムにローカルでインストール）。
...
* スケーラビリティの面から考えると、基本的には書き込み操作数が最も重要です。オーサー環境の場合は[同時操作している作成者](/help/managing/hardware-sizing-guidelines.md#authors-working-in-parallel)を、パブリッシュ環境の場合は[ソーシャルコラボレーション](/help/managing/hardware-sizing-guidelines.md#socialcollaborationspecificconsiderations)を参照してください。読み取り操作を処理する目的でのみシステムにアクセスする操作に対して、ロードバランシングを確立できます。詳しくは、[Dispatcher](https://helpx.adobe.com/experience-manager/dispatcher/user-guide.html)を参照してください。
...
* **ベンチマークテスト2**
b2オーサーインスタンスのアクティブ — アクティブ共有なしクラスターで、新しいページの作成(10%)、既存のページの変更(80%)、ページの作成と変更(10%)が混在する場合の最大スループットを計算します。ページの複雑度はベンチマークテスト 1 のプロファイルの場合と同じです。ページの基本的な変更として、画像を追加し、テキストコンテンツを変更します。この場合も、ベンチマークテスト 1 に定義されたものと同じ複雑度を持つ 300 個のページを基本負荷として、作業をおこないました。
...
少なくとも16 GBのヒープを割り当て、生の画像の取り込みに[Camera Rawパッケージ](/help/assets/camera-raw.md)を使用するように[!UICONTROL DAMアセットの更新]ワークフローを設定します。
