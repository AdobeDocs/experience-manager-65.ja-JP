...
フォルダーを作成できる初期フォルダーがあらかじめ用意されているはずです。これは、一部の設定詳細を（通常は開発者またはシステム管理者が）ルートフォルダーに適用する必要があるからです。これはおそらく興味のないものですが、必要に応じて、 **設定** が含まれる **Cloud Service** フォルダーの **プロパティ**:
