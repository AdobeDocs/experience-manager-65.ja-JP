...
AEM ヘッドレスジャーニーの前のドキュメント [AEM Headless  - はじめに](getting-started.md)では、ヘッドレス CMS の基本事項を説明し、以下を達成できました。
...
このドキュメントに進む前に、AEM ヘッドレスデベロッパージャーニーの前のドキュメント [AEM Headless  - はじめに](getting-started.md)の内容に再度目を通し、以下を確認してください。
...
AEMとは別に、クライアント側のアプリケーションを構築する場合は、AEMへの API 呼び出しに対してクライアントをテストできるよう、適切な開発環境が設定されていることを確認する必要があります。
