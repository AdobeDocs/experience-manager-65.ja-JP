...
翻訳されたコンテンツの状態に満足したら、そのコンテンツを公開して、ヘッドレスサービスで消費できるようにする必要があります。このタスクは翻訳スペシャリストの責任ではありませんが、完全なワークフローを説明するためにここに記載されています。
...
特定のプロジェクト要件は、翻訳を更新する頻度と、更新を実行する前に実行される決定プロセスを定義します。 翻訳の更新を決定すると、AEMでの処理は簡単になります。 初回翻訳が翻訳プロジェクトに基づいていたように、更新の場合も翻訳プロジェクトに基づいています。
...
お気づきかもしれませんが、新しいカードに表示される統計情報（アセットとコンテンツフラグメントの数）が異なります。これは、AEMが最後の翻訳以降に変更された内容を認識し、翻訳が必要なコンテンツのみを含むからです。 これには、更新されたコンテンツの再翻訳と、新しいコンテンツの初回翻訳が含まれます。
...
1. に移動します。 **ナビゲーション** > **プロジェクト**.
1. 更新する必要があるプロジェクトをクリックします。
1. 次をクリック： **追加** ボタンをクリックします。
1. Adobe Analytics の **タイルを追加** ウィンドウ、クリック **翻訳ジョブ** その後 **送信**.

   ![タイルの追加](assets/add-translation-job-tile.png)

1. 新しい翻訳ジョブのカードで、カードの上部にある山形のボタンをクリックし、「 」を選択します。 **ターゲットを更新** ：新しいジョブのターゲット言語を定義します。

   ![ターゲットの更新](assets/update-target.png)

1. Adobe Analytics の **ターゲット言語を選択** ダイアログが表示されたら、ドロップダウンを使用して言語を選択し、 **完了**.

   ![ターゲット言語の選択](assets/select-target-language.png)

1. 新しい翻訳ジョブのターゲット言語を設定したら、ジョブカードの下部にある省略記号ボタンをクリックして、ジョブの詳細を表示します。
