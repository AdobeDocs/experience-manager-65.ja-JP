...
| `cq:commerceProvider = geometrixx` | 標準のAEM インストールでは、特定の実装が必要です。 例えば、Geometrixxの例では、一般的なAPIの拡張機能が最小限に抑えられています |
|--- |--- |
...
* Maven を呼び出す場合は、次のコマンドライン引数をコマンドに追加します。
  ...
  事前設定済みの Hybris 4 ディストリビューションをダウンロードして、バンドル `cq-commerce-hybris-server` に埋め込みます。
