...
* e コマースシステムをプラグインし、製品データをAdobe Experience Manager(AEM) に取り込む
...
* Maven を呼び出す際に、次のコマンドライン引数をコマンドに追加します。
...
* その応答からセッション cookie が抽出され、新しい cookie 内でエンコードされ（例えば `hybris-session-rest`）、買い物客への応答に設定されます。元の cookie は特定のパスに対してのみ有効で、それ以外の場合は後続のリクエストでブラウザーから返送されないので、新しい cookie でのエンコードが必要です。 パス情報を Cookie の値に追加する必要があります。
...
* hybris は、フィード (AEMがポーリングする ) を介して更新情報を提供します。
...
製品/バリアントリソースには、必ずしも実際の製品データが含まれるとは限りません。 これは、別のシステム（hybris など）で保持されているデータの表現の場合があります。 例えば、製品の説明と価格はAEMには格納されず、e コマースエンジンからリアルタイムで取得されます。
...
* 実装者は、（選択された支払い処理サービスに対して）特定の呼び出しを `CommerceSession` 実装。

**注文のフルフィルメント**

* `CommerceSession` は、フルフィルメントの接続も管理します。
* 実装者は、（選択した支払い処理サービスに）特定の呼び出しを `CommerceSession` 実装。
...
1. `CommerceQuery`
   ...
   検索クエリを表します（クエリテキスト、現在のページ、ページサイズ、並べ替え、選択されたファセットに関する情報を含みます）。 検索 API を実装するすべての e コマースサービスは、このクラスのインスタンスを受け取って検索を実行します。 `CommerceQuery` は、リクエストオブジェクト（`HttpServletRequest`）からインスタンス化できます。

1. `FacetParamHelper`

    1 つの静的メソッド（`toParams`）を提供するユーティリティクラス。このメソッドを使用して、ファセットのリストと 1 つのトグル値から `GET` パラメーター文字列を生成します。これは UI 側で役立ち、各ファセットの値ごとにハイパーリンクを表示し、ユーザーがハイパーリンクをクリックしたときにそれぞれの値が切り替えられるようにする必要があります。 つまり、選択した場合はクエリから削除され、それ以外の場合は追加されます。 これは、複数値または単一値のファセットの処理、値の上書きなどのすべてのロジックを処理します。
...
* 認証
  ...
  AEMは *のみ* web フロントエンド、したがって実行 *すべて* 認証。
...
* AEM -> hybris
   ...
   * hybris にログインする際に、AEMユーザーが存在しない場合は、次の手順を実行します。
      ...
      * 暗号的にランダムなパスワードを使用して hybris ユーザーを作成する
...
* hybris -> AEM の場合
   ...
   * AEM へのログイン時に、システムがユーザーを認識していない場合は、
      ...
      * 指定されたユーザー名/pwd を使用して hybris にログインしようとします。
      * 成功した場合は、同じパスワードを使用してAEMでユーザーを作成します (AEM固有の salt はAEM固有のハッシュになります )。
