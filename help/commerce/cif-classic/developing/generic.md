...
各製品やバリエーションはリソースで表され、1:1をリポジトリノードにマッピングします。 当然のことながら、特定の製品やバリアントはそのパスによって一意に識別できます。
...
製品バリエーションと製品データノードの間には、一対一のマップが必要です。
...
* ストレージ
  ...
  * AEM 汎用の場合、買い物かごは [ClientContext](/help/sites-administering/client-context.md) に保存されます。
...
* 注文フォームには、多くの場合、複数の発送オプション（および価格）を提示する必要があります。
...
>
>そのため、標準で提供される汎用実装ではこの API は実装されませんが、拡張してこの検索機能を追加することができます。
...
このように、`CommerceSession` は、割引券が存在するかどうか、割引券を適用できるかどうかをチェックします。 これは、特定の条件を満たした場合にのみ適用できる割引券に適している場合があります。 例えば、買い物かごの合計価格が100 ドルを超える場合です。 何らかの理由で割引券を適用できない場合は、`addVoucher` メソッドが例外をスローします。 また、`CommerceSession` によって、割引券の追加または削除後に買い物かごの価格の更新もおこないます。
...
提供される `AbstractJcrCommerceSession` によって割引券を適用できます。 クラス `getVouchers()`から返される割引券は、次のプロパティを持つjcr:content ノードを含む`cq:Page`のインスタンスです（特に）。
...
プロモーションハンドラーは、買い物かごを変更する OSGi サービスです。 買い物かごは、`PromotionHandler` インターフェイスで定義される複数のフックをサポートします。
