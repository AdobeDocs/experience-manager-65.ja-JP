---
title: 概念
description: Adobe Experience Managerとの e コマースの一般的な概念。
...
>[!NOTE]
>
>Geometrixxインポーターは CSV ファイルを使用します。実装の上にあるコメントに、受け入れられるスキーマの説明（カスタムプロパティも許可されています）が含まれています。
...
* e コマースエンジンに保持して、AEM で利用できます。
  ...
  データタイプに応じて、 [同期済み](#catalog-maintenance-data-synchronization) 必要に応じて、または直接アクセスする。例えば、製品価格などの非常に揮発性で重要なデータは、常に最新の状態に保つために、ページリクエストごとに e コマースエンジンから取得されます。
...
* できるだけ重複を避ける（ローカライゼーションを参照）か、またはその場合は、複製が生じるノード数を考慮します。
...
* 技術スタックで、コンテンツアクセスモデルとサービスを組み込んで計画します。 これは一般的なベストプラクティスですが、最適化フェーズでは、頻繁に読み取られるデータのアプリケーションキャッシュを追加する（およびバンドルキャッシュにを埋め込まない）ので、ここではさらに重要です。
...
* テキストや画像を含む、一般的な商品情報。
* 価格これは、ページが表示/更新されるたびに e コマースエンジンから取得されます。
* カラーやサイズなどの製品バリアント情報。
...
この **製品** コンポーネントは、必要なメタデータ ( つまり、 `cartPage` および `cartObject`) をクリックします。 これは、デモサイト Geometrixx Outdoors では `UserInfo.jsp` によって提供されています。
...
* エクスペリエンス内に存在しない（キャンペーン内にのみ存在する）プロモーションの場合は、自動的にはオーディエンスに適用されません。ただし、買い物客が割引券を買い物かごに入力し、その割引券がプロモーションを参照している場合には、プロモーションが呼び出されます。
  ...
  がエクスペリエンス外にあるので、自動的には実行されません ( つまり、セグメント化に基づいて )。 ただし、このプロモーションを参照する割引券が、記事のキャンペーンに含まれるいくつかのエクスペリエンスにあります。これらの割引券コードを買い物かごに入力すると、プロモーションが実行されます。
...
アカウント情報を管理しやすくなるページが用意されています。これには、 **マイアカウント** Geometrixxページの上部に表示されるか、 `/content/geometrixx-outdoors/en/user/account.html`.

![chlimage_1-13](/help/sites-administering/assets/chlimage_1-13.png)

### アドレス帳 {#address-book}

サイトには、選択した複数のアドレスを保存する必要があります。配信、請求、代替アドレスを含みます。 これは、デフォルトのアドレス形式に基づくフォームを使用して実装するか、AEMが提供するアドレス帳コンポーネントを使用して実装できます。
...
* サインイン `Allison Parker` - Allison&#39;s cart is empty
* 製品を Allison の買い物かごに追加
* ログアウト — 買い物かごに `anonymous`

* 再度サインイン `Allison Parker` - Allison の製品は復元されました
...
チェックアウト前に、価格の変更が（両方のシステムで）反映されます。
