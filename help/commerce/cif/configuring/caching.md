...
他のコンポーネントのキャッシュは、通常は Dispatcher レベルで設定されたキャッシュと連携して、プロジェクト単位で定義する必要があります。これらのキャッシュにはアクティブな無効化はないので、キャッシュ時間は慎重に設定する必要があります。 考えられるすべてのプロジェクトとユースケースに一致する「1 つのサイズですべてに適合」の値はありません。 プロジェクトの要件に最も適したプロジェクトレベルでキャッシュ方法を定義してください。
