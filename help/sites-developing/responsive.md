...
* 画像：クライアントビューポートに適した画像レンディションを提供します。 選択することができます。
...
メディアクエリを使用すると、ページレンダリングにCSS スタイルを選択的に使用できます。 AEMの開発ツールと機能を使用すると、メディアクエリをアプリケーションに効果的かつ効率的に実装できます。
...
CSS ファイルで、ターゲットにするデバイスのプロパティに基づいてメディアクエリを定義します。 次の実装戦略は、各メディアクエリのスタイルを管理するのに効果的です。
...
* ClientLibraryFolder の css.txt ファイルで、組み立てられた CSS ファイル内で必要とされる順序で CSS ファイルを並べます。
...
>[!NOTE]
>
>`apps.weretail.all` クライアントライブラリフォルダーに clientlibs ライブラリが埋め込まれています。
...
* 型：`String[]`
* 値：web ページをレンダリングするページコンポーネントへのパス。 例えば、geometrixx-media アプリは次の値を使用します。
...
picturefill.js コードが期待する必要なdiv要素を生成するコンポーネントを作成します。 AEM ページでは、data-src属性の値は、リポジトリ内のリソースへのパスです。 例えば、ページコンポーネントは、DAMでメディアクエリと画像レンディションに関連するパスをハードコーディングできます。 または、作成者が画像レンディションを選択したり、ランタイムレンダリングオプションを指定したりできるカスタム画像コンポーネントを作成します。
...
AdaptiveImageComponentServlet クラスもwriteLayer メソッドをオーバーライドします。 この方法では、JPEGの画質を画像に適用します。
...
3つの追加のセレクターで、要求された画像の幅、高さ、および（オプションで）画質を指定します。 次の例では、幅770 ピクセル、高さ360 ピクセル、中画質の画像を要求します。
...
メインページコンポーネントを`cq:resourceSuperType`として使用する他のページコンポーネントを作成します。 これらのコンポーネントには、必要に応じてメインページのスクリプトを上書きするスクリプトが含まれています。
...
ターゲットとするビューポート幅の範囲ごとに、固定ページ幅と一定の幅のコンテンツブロックを使用します。 ブラウザーウィンドウのサイズを手動で変更する場合、コンテンツサイズの変更は、個別のウィンドウ幅（ブレークポイントとも呼ばれます）で行われます。 したがって、ページデザインはより密接に結び付き、ユーザーエクスペリエンスを最大限に高めます。
...
1. コンテンツブロック要素のクラス名を定義します。 クラスは、列のスパンに関して、可能なすべての幅に必要です。 たとえば、3 列にまたがる `div` 要素には `span3` クラスを使用し、4 列にまたがる場合は `span4` クラスを使用します。 グリッドにある列と同じ数のクラスを定義します。

1. ターゲットとするビューポートサイズごとに、対応するメディアクエリをCSS ファイルに追加します。 各メディアクエリに次の項目を追加します。
...
ターゲットにしているそれぞれのビューポートサイズのページレイアウトに対して、各コンテンツブロックがまたがる列数を決定します。 次に、それぞれのコンテンツブロックの div 要素で使用するクラスを決定します。
