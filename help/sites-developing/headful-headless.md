...
description: AEMプロジェクトはヘッドフルおよびヘッドレスモデルで実装できますが、選択肢はバイナリではありません。 AEM は、1 つのプロジェクトで両方のモデルのメリットを活用できる柔軟性を備えています。
...
Adobe Experience Managerプロジェクトは、ヘッドフルモデルとヘッドレスモデルの両方で実装できますが、選択肢はバイナリではありません。 AEM は、1 つのプロジェクトで両方のモデルのメリットを活用できる柔軟性を備えています。このドキュメントでは、様々なモデルの概要とSPA統合のレベルについて説明します。
