...
フロントエンド開発者は、アプリの構築に使用されるSPAコンポーネントのライブラリの作成を自分たちが担当していると考える必要があります。 フロントエンド開発者は、コンポーネントの内部構造を完全に制御できます。 [ただし、AEMは常にサイトの構造を所有しています。](/help/sites-developing/spa-overview.md)
...
* **Dispatcher／CDN**
   ...
   * リクエストは、AEM オーサーでの処理と同様に処理されますが、ページ情報のリクエストはありません。ページ情報はエディターでのみ必要とされるからです。
