...
AEMは、タッチ対応UIと、タッチ対応UIを備え [た作成者環境向けのレスポンシブデザインを備え](/help/sites-authoring/responsive-layout.md) 、タッチデバイスとデスクトップデバイスの両方で動作するように設計されています。

>[!NOTE]
>
>タッチ対応UIは、AEMの標準的なUIです。 従来のUIはAEM 6.4で非推奨となりました。
...
[Get File](assets/coralui.pdf)Coral UI(CUI)は、複数の製品にわたってユーザーエクスペリエンスの一貫性を保つように設計された、タッチ対応UIのアドビの視覚スタイルの実装です。 Coral UI は、オーサー環境で使用されるビジュアルスタイルを採用するために必要となるすべてのものを備えています。
