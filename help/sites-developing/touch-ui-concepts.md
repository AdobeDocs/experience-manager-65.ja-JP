...
AEMは、タッチデバイスとデスクトップデバイスの両方で動作するように設計されたオーサー環境向けの[レスポンシブデザイン](/help/sites-authoring/responsive-layout.md)を備えたタッチ操作対応UIを備えています。

>[!NOTE]
>
>タッチ操作対応UIはAEMの標準UIです。 クラシックUIはAEM 6.4で非推奨（廃止予定）となりました。
...
[Granite UI](assets/graniteui.pdf)
でファイルを取得：
...
[Get FileCoral ](assets/coralui.pdf)
UI(CUI)は、複数の製品間でユーザーエクスペリエンスの一貫性を保つために設計された、タッチ操作対応UI用Adobeのビジュアルスタイルの実装です。Coral UI は、オーサー環境で使用されるビジュアルスタイルを採用するために必要となるすべてのものを備えています。
