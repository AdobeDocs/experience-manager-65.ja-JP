...
技術的な観点から見ると、表示された要素の各段落*は独自のparsysに存在し、段落間に追加する各コンポーネントは、parsysに挿入されます（フードの下）。
