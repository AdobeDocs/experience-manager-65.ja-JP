...
![テンプレートがベースのシンプルなコンテンツフラグメントは、メインアセットとサブアセット（オプション）の複合にマッピングされます。](assets/chlimage_1-90.png)
...
#### コンテンツフラグメント管理（CFM）機能は、Assets のコアを基盤として構築されていますが、できる限り独立させる必要があります。{#feature-integration}
...
>[コンテンツフラグメントコアコンポーネント[#$tu68]が推奨されます。詳しくは、[コアコンポーネントの開発[#$tu69]を参照してください。
>
>
...
* ここで、このコンポーネントを使用して、他のアセット（画像など）を参照されているフラグメントの段落の中間に配置できます。[](/help/sites-developing/components-content-fragments.md#in-between-content)
...
#### コンテンツフラグメントのバックエンド実装は、ページ上で使用されるフラグメントのインスタンスを検索可能にしたり、複合メディアコンテンツを管理したりする役割を担います。この実装では、フラグメントのレンダリングに使用されるコンポーネントと、レンダリングがパラメーター化される方法を把握する必要があります。{#configuration-using-osgi-console}
...
**Resource types** `sling:resourceTypes` のリストを使用して、コンテンツフラグメントのレンダリングに使用するコンポーネントと、バックグラウンド処理を適用する場所を定義できます。**
...
>[!NOTE]AEM では、段落内で最初に検出されたプロパティを利用します。そのため、プロパティは慎重に選択する必要があります。
>
...
* If the value is `singleText` (and there is only one element configured) then the element is rendered as a text with in-between content, layout support, etc. レンダリングされる要素が 1 つのみのフラグメントでは、これがデフォルトです。`variationName`

* それ以外の場合は、よりシンプルな「フォーム表示」と呼ばれる方法が使用されます。この方法では中間コンテンツがサポートされず、フラグメントコンテンツが「そのまま」表示されます。`elementNames``displayMode`
...
* if `paragraphScope` == `range` then the property `paragraphRange` defines the range of paragraphs to be rendered
   ...
   * その他のフレームワークとの統合 {#integration-with-other-frameworks}`all``range`

   * コンテンツフラグメントは、次のフレームワークと統合できます。`paragraphScope``range``paragraphRange`
...
* **コンテンツフラグメントの個々の翻訳は、実際には別々のフラグメントです。例えば、以下のようになります。**
   ...
   言語のルートが異なります。[](/help/sites-administering/tc-manage.md)
   ...
   * AEM 翻訳ワークフローでは、`/content` が使用されます。
   ...
   >
   >テンプレートをコピーしてフラグメントを作成するので、これは暗黙的になります。`/content`
   >
   >    * **メタデータスキーマ**](/help/sites-developing/i18n-dev.md)
      >
      >    
...
* **CFM には、次のような独自の固有のスキーマがあります。**
   ...
   * 必要に応じて拡張できます。
      ...
      `/libs/dam/content/schemaeditors/forms/contentfragment`各スキーマフォームは、フラグメントエディターと統合されています。
...
* **コンテンツフラグメント**（[ContentFragment[#$tu159]）
   ...
   >[!CAUTION]このインターフェイスでは、次のことを実行できます。
   >
   ...
   * **コンテンツ要素**（[ContentElement[#$tu180]）
   ...
   要素のバージョンを一覧表示する`ContentFragment``ContentElement``ContentVariation``Versionable`
...
* `Asset`  — コンテンツフラグメントを表すDAM `Asset` 抽象。 更新を `Asset` 直接行うには、オブジェクトを再構築する必要があり `ContentFragment` ます。
   ...
   * `ContentElement` は、次のものに適応させることができます。`Resource``ContentFragment`

   * `ElementTemplate` - 要素の構造に関する情報にアクセスするためのものです。`Asset``Asset``ContentFragment`
...
* `Resource` を使用した変更は、`FragmentTemplate` に自動的に反映されません。
   ...
   * `Resource` は、次のものに適応させることができます。`Resource`
...
* `FragmentTemplate`
   ...
   * 注意事項 {#caveats}
   * `FragmentTemplate`次のことに注意してください。

### API は、UI がサポートする機能を提供するために実装されています。{#caveats}
...
* `ContentElement` から新しいバリエーションを作成してもデータ構造は更新されません（ただし、`ContentFragment` からグローバルに新しいバリエーションを作成すると更新されます）。
   ...
   * コンテンツフラグメント管理 API - クライアント側 {#the-content-fragment-management-api-client-side}`ContentFragment`

   * [!CAUTION]

## AEM 6.5 では、クライアント側 API は内部用になります。{#the-content-fragment-management-api-client-side}
...
### 複数の表示（= HTMLページ）にまたがるコンテンツフラグメントの編集は、アトミックで行う必要があります。{#requirements}
...
* ページからの移動
   ...
   * 編集セッションが存在していて、変更が保持されていない場合は、コンテンツが失われる可能性があることをユーザーに通知し、ページに留まるかどうかを尋ねるモーダル確認ダイアログが表示されます。[](#processes)

* 例 {#examples}

   * 例：既存のコンテンツフラグメントへのアクセス {#example-accessing-an-existing-content-fragment}

## 既存のコンテンツフラグメントにアクセスするには、API を表すリソースを以下に適応させます。{#examples}
...
* 自動保存間隔を 5 分に設定する場合は、次のようにノードにプロパティを定義する必要があります。`Long`

* ノード：`/conf/global/settings/dam/cfm/jcr:content``/libs/settings/dam/cfm/jcr:content`
...
詳しくは、次を参照してください。[](/help/sites-developing/content-fragment-templates.md)

## [コアコンポーネント - コンテンツフラグメントコンポーネント[#$tu291]（推奨）
