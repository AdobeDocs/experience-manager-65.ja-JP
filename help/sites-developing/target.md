...
Adobe targetとやり取りするページでターゲットコンポーネントを使用するには、&lt;head>要素に特定のクライアント側コードを含めます。
...
>[!NOTE]
>
>製品に付属ののバージ `at.js` ョンのみがサポートされます。 製品に付属して `at.js` いるのバージョンは、次の場所のファイルを参照するこ `at.js` とで取得できます。
>
