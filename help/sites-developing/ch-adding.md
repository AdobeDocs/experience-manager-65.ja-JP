...
ストアを JavaScript オブジェクトとして取得するには、[`ContextHub`](contexthub-api.md#ui-event-constants) JavaScript オブジェクトを使用します。 ストアオブジェクトを取得したら、そのストアオブジェクトに含まれるデータを操作できます。 ストアを取得するには、[`getAllStores`](contexthub-api.md#getallstores) 関数または [`getStore`](contexthub-api.md#getstore-name) 関数を使用します。
...
個々のデータ項目は、一連のキーと値のペアとして保存されます。 値を保存して取得するには、対応するキーを指定します。
...
>[!NOTE]
>
>ContextHubは、デフォルトではパブリッシュサーバー上の現在ログインしているユーザーを認識せず、そのようなユーザーは「匿名」と見なされます。
>
...
* 最初に、[位置情報](ch-samplestores.md#contexthub-geolocation-sample-store-candidate) ストアを使用して、ユーザーの緯度を決定します。
