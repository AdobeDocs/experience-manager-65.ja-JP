...
* イネーブルメントリソース（[イネーブルメントリソースのタグ付け](/help/communities/functions.md#catalog-function)を参照）

## タグの機能 {#tag-features}

AEM 内のタグには以下のような機能があります。

* タグは様々な名前空間にグループ化できます。 このような階層を使用すると、分類を作成できます。 これらの分類は、AEM 全体で使用されます。
...
* タグは、作成者とサイト訪問者が適用できます。 作成者に関係なく、ページに割り当てたとき、または検索時に、すべての形式のタグが使用可能になります。
...
* **名前をに変更**
最初に、タグの現在の`name`を表示します。 新しい `name` を入力できます。
...
最初に、**Stock Photography**&#x200B;名前空間の下にあるタグを選択し、**`Edit`** アイコンを選択します（[ タグの編集](#editing-tags) セクションを参照）。
...
>[!NOTE]
>
>標準ページコンポーネントのタグクラウドとメタキーワードでは、ページ言語に基づいてローカライズされたタグ `titles`が使用されます（使用可能な場合）。
