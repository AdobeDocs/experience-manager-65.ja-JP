...
この画面では、特定のパスの権限のリストと権限タイプに加えて、次に示すように、きめ細かいアクセス制御の制限を追加することもできます。
...
次のように、新しい ACE がアクセス制御リストに反映されます。 `jcr:write`は、上記で追加された`jcr:removeNode`を含む集計権限ですが、`jcr:write`で取り上げられているため、以下には示されません。
...
右上の「**保存**」ボタンをクリックして変更を保存すると、次に示すように、**dam-users**&#x200B;の新しい権限に反映されます。
