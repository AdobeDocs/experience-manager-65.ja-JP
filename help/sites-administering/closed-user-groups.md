...
* 通常のログインの場合、は次の順序でログインパスを取得できるかどうかをテストします。
...
新しい承認関連パーツは、 **Oak CUG 認証** バンドル ( `org.apache.jackrabbit.oak-authorization-cug`) です。これは、AEMのデフォルトインストールの一部です。 このバンドルは、読み取りアクセスを管理する追加の方法としてデプロイすることを目的とした、個別の認証モデルを定義します。
