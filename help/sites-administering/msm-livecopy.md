...
1. ライブコピーの宛先パスを指定し（ライブコピーの親フォルダーまたはページを開く）、 **次へ**.
...
ロールアウトは非同期ジョブとして処理され、 [**非同期ジョブステータス** dashboard](asynchronous-jobs.md#monitor-the-status-of-asynchronous-operations) 時刻 **グローバルナビゲーション** > **ツール** > **運用** > **ジョブ**
...
1. Adobe Analytics の **継承をキャンセル** 確認ダイアログで、「 **はい**.
