...
1. 「**サブページを除外**」チェックボックスを次のように使用します。
   ...
   * 未選択：選択したページのすべての子孫を含むライブコピーを作成する（ディープライブコピー）
...
ロールアウトは非同期ジョブとして処理され、**グローバルナビゲーション** -> **ツール** -> **操作** -> **ジョブ**&#x200B;の&#x200B;[**非同期ジョブステータス**&#x200B;ダッシュボードで確認できます。](asynchronous-jobs.md#monitor-the-status-of-asynchronous-operations)

>[!NOTE]
>
>非同期ロールアウト処理にはAEM 6.5.3.0以降が必要です。 以前のバージョンでは、ページは即座に同期的に処理されていました。

#### 参照レールからのブループリントのロールアウト {#roll-out-a-blueprint-from-the-reference-rail}

1. **サイト**&#x200B;コンソールで、ライブコピーのページを選択し、**[参照](/help/sites-authoring/basic-handling.md#references)**&#x200B;パネルを（ツールバーから）開きます。
...
>[!NOTE]
>
>非同期ロールアウト処理にはAEM 6.5.3.0以降が必要です。 以前のバージョンでは、「**バックグラウンドロールアウト**」オプションがオンになっていない限り、ページは即座に同期的に処理されていました。
