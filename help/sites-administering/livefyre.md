...
1. AEMのホームページで、 **ツール** アイコンをクリックします。
1. に移動します。 **デプロイメント/パッケージ**.
...
1. から Livefyre パッケージをダウンロード [ソフトウェア配布](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html).

1. パッケージマネージャーから、ダウンロードしたパッケージをインストールします。 詳しくは、 [パッケージの使用方法](/help/sites-administering/package-manager.md) AEMでのソフトウェア配布とパッケージの使用に関する詳細

   ![livefyre-aem4-6-4](assets/livefyre-aem4-6-4.png)

   Livefyre-AEM パッケージがインストールされます。統合機能を使い始める前に、Livefyre を使用するように AEM を設定する必要があります。

   機能パックの詳細およびリリースノートについては、 [機能パック](https://experienceleague.adobe.com/docs/experience-manager-65/release-notes/home.html).

### Livefyre を使用するようにAEMを設定します。設定フォルダーの作成 {#configure-aem-to-use-livefyre-create-a-configuration-folder}

1. AEMのホームページで、 **ツール** アイコンをクリックし、 **一般/設定ブラウザー**.
...
1. 設定に名前を付けて、 **クラウド設定** チェックボックス
   ...
   これにより、の下にフォルダーが作成されます。 **ツール/デプロイメント/ Livefyre 設定** を指定します。
...
1. AEMのホームページで、 **ツール** アイコンをクリックし、 **デプロイメント/Livefyre 設定**.
1. 新しい Livefyre 設定を作成する設定フォルダーを選択して、「**作成**」をクリックします。
   ...
   >[!NOTE]
   >
   >Livefyre 設定を追加する前に、フォルダーのプロパティでクラウド設定を有効にしておく必要があります。 設定フォルダーは、 [設定ブラウザー。](/help/sites-administering/configurations.md)
   >
...
1. CRXDE Liteの使用、コピー */libs/social/integrations/livefyre/components/authorizablecomponent/authclientlib* を */apps/social/integrations/livefyre/components/authorizablecomponent/authclientlib*.
1. 編集して保存 */apps/social/integrations/livefyre/components/authorizablecomponent/authclientlib/auth.js* を使用して、ニーズに合った Livefyre Auth Delegate を実装します。

   Auth Delegate のカスタマイズについて詳しくは、 [ID 統合](https://answers.livefyre.com/developers/identity-integration/).

   AEM Clientlibs について詳しくは、 [クライアント側ライブラリの使用](https://experienceleague.adobe.com/docs/experience-manager-65/developing/introduction/clientlibs.html).
...
ページ用に Livefyre が有効になったら、Livefyre コンポーネントを許可するようにコンテナを設定する必要があります。詳しくは、 [デザインモードでのコンポーネントの設定](https://experienceleague.adobe.com/docs/experience-manager-65/authoring/siteandpage/default-components-designmode.html) 様々なコンポーネントを有効にする方法の説明

>[!NOTE]
>
>POST 認証が必要なアプリは、シングルサインオン統合のカスタマイズで認証が設定されるまで機能しません。

1. から **コンポーネント** デザインモードのサイドパネルで、「 **Livefyre** メニューから選択して、使用可能な Livefyre コンポーネントのリストに限定します。
...
1. 新しい Livefyre アプリを作成するか、既存のアプリを埋め込むかを選択します。
   ...
   コンポーネントの挿入について詳しくは、 [ページのコンテンツの編集](https://experienceleague.adobe.com/docs/experience-manager-65/authoring/authoring/editing-content.html).
...
1. AEMホームページから、に移動します。 **アセット/ファイル**.
1. クリック **作成**&#x200B;をクリックし、 **UGC を読み込みます。**
...
1. 読み込むアセットを選択します。選択したアセットは自動的にカウントされ、 **選択** タブに表示されます。
1. **オプション**:「 **選択** 」タブに移動し、読み込む選択した UGC コンテンツを確認します。
1. 「**次へ**」をクリックします。
...
1. 権限リクエストについて、各アセットに対して次のいずれかのオプションを選択します。
   ...
   * **手動での権限のリクエスト** をクリックして、コピー&amp;ペーストできるメッセージを取得し、Instagram経由でコンテンツの所有者に手動で送信します。
   * **手動によるコンテンツ権限の属性設定** 個々のアセットの権限を上書きする場合。

   >[!NOTE]
   >
   >ビジネス以外のユーザーアカウントからのコンテンツの集計に影響を与える更新により、お客様に代わってコメントを投稿したり、作成者からの返信を自動的に確認したりできなくなりました。 [詳しくはここをクリック](https://developers.facebook.com/blog/post/2018/04/04/facebook-api-platform-product-changes/).
...
1. クリック **完了** をクリックして、権限リクエストワークフローを終了します。
...
1. AEMホームページから、に移動します。 **AEM Commerce**.
1. 新しいコレクションを作成するか、既存のコレクションを使用します。
1. コレクションの上にマウスポインターを置いて、「 **コレクションのプロパティ** （鉛筆アイコン）。
1. チェック **Livefyre と同期**.
1. 入力 **Livefyre ページプレフィックス** をクリックして、このコレクションをAEMの特定のページにリンクします。
