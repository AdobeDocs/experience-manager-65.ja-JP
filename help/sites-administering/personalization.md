...
* Web 上の多くの機能と同じように、パーソナライゼーションは慎重に実行する必要があります。この機能は本当にユーザーの役に立つのか、あるいは、より少ない労力で目的のページへ到達できる方法が他にないか、常に最初に考慮するべきです。ユーザーがパーソナライゼーションを（試しに）一度設定しても、価値を見出さなければそれ以降使用されないというリスクがあります。
