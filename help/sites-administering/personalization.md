...
* ユーザーが表示したい内容です。 好ましくは、興味を持った内容を、それらのタスクを実行するためにそれらに使用します。
* 分類できるので、事前定義されたルールに従ってユーザーが使用できます。
* 動的である必要があります。

つまり、コンテンツは、何らかの方法で、ユーザーに依存する必要があります。 すべてのユーザーに同じコンテンツが表示される場合、パーソナライゼーションは冗長になります。

### ルール {#rules}

* パーソナライゼーションの実際の動作（ユーザーに表示されるコンテンツとタイミング）を定義します。
...
* キャッシュとは、パフォーマンスと正確性の面で、Web サイトがパーソナライズされたコンテンツをどの程度迅速に提供し、常に最新の状態で提供するかを指します。
...
* したがって、ルールは十分に考慮する必要があります。ユーザーの要件がフォアグラウンドにある必要があります。 この作業は手間がかかりますが、軽視すべきではありません。パーソナライゼーションの実装では多くの場合、テクニカルな作業よりビジネスルールを定義することが優先されます。

#### 用途 {#when-to-use}

* Web 上の多くの機能と同じように、パーソナライゼーションは慎重に実行する必要があります。この機能は本当にユーザーの役に立つのか、あるいは、より少ない労力で目的のページへ到達できる方法が他にないか、常に最初に考慮するべきです。パーソナライゼーションは、実際にはメリットがないので、1 回だけ設定する機能（動作の仕組みを確認するため）のリスクが高まります。
* パーソナライゼーションは、コンテンツが動的な場合にのみ意味があり、ユーザーによって何らかの形で依存します。 すべてのユーザーに同じコンテンツが表示される場合、パーソナライゼーションは不必要です。
...
ブログ、フォーラム、カレンダーなど、結果としてコミュニティのコンテンツが作成されるコミュニティの機能は、一般的にユーザー生成コンテンツ（UGC）と呼ばれます。UGC を複数のAEMインスタンス ( [パブリッシュファーム](/help/communities/topologies.md)) を使用する場合、主な問題の 1 つは、すべてのインスタンスに UGC を同期する方法です。

を使用 [AEM Communities 6.1](/help/communities/overview.md) 拡張機能を使用すると、この問題は [UGC 用の共通ストア](/help/communities/working-with-srp.md). パーソナライゼーションに関しては、Communities には以下が含まれます。 [ソーシャルログイン](/help/communities/social-login.md) ：サイト訪問者がFacebookおよびTwitterを使用してログインするためのオプションを提供する機能。

Communities の拡張機能がない場合、UGC の整合性の問題に対処する方法は次のとおりです。

* 必要に応じて、複数のパブリッシュインスタンスを同期
* パブリッシュインスタンスからオーサー環境に UGC を送信し、そこからページコンテンツの公開と同様の方法で UGC を公開できます。
