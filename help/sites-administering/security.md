...
>[!NOTE]
>
>AEMは、[コレクション](/help/assets/manage-collections.md)の役割割り当て（所有者、編集者、閲覧者）のユーザーグループを自動的に生成します。 ただし、それらのグループに ACL を手動で追加すると、AEM 内にセキュリティ上の脆弱性をもたらす恐れがあります。ACL を手動で追加することはお勧めしません。
...
>
>JEEでAEM Formsを使用している場合は、以下の手順を使用してパスワードを変更しないでください。JEE上のAEM Forms Admin Console(/adminui)を使用してパスワードを変更します。
