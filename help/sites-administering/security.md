...
>[!NOTE]
>
>AEM は[コレクション](/help/assets/manage-collections.md)で role-assignment のユーザーグループ（所有者、編集者、閲覧者）を自動的に生成します。ただし、それらのグループに ACL を手動で追加すると、AEM 内にセキュリティ上の脆弱性をもたらす恐れがあります。ACL を手動で追加することはお勧めしません。
...
>
>JEE で AEM Forms を使用している場合は、以下の手順を使用してパスワードを変更しないでください。JEE 上の AEM Forms Admin Console (/adminui) を使用してパスワードを変更します。
