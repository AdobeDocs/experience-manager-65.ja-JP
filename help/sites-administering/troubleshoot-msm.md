...
* 次を確認します。 [MSM のベストプラクティスに関する記事](msm-best-practices.md) いくつかの誤解の説明と共に、そこでいくつかのヒントが提供されているので、
...
* `cq:LiveSync` mixin タイプ
   ...
   * これらのページには、 `cq:LiveSyncConfig` タイプの子ノード `cq:LiveCopy` 次のプロパティを使用して、ライブコピーの基本情報と必須情報を含める。
...
詳しくは、 [この記事](msm-best-practices.md) を参照してください。
...
これを避けるには、ライブコピーを使用します。 **休止** の代わりに機能 **分離**. 詳細は、 **分離** アクションは、 [この記事を参照してください。](msm-livecopy.md)
