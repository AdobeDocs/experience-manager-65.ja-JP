...
* **ページエディター（ドキュメント検索）：**
  ...
  この設定では、アセットブラウザーでドキュメントを検索する際（ページを編集する際）に使用できるオプションを定義します。
...
* **プロパティ名**
  ...
  `jcr:content` ノードの`x` プロパティを持つリソースの第1 レベルの子ノードでのみ検索する場合は、`*/jcr:content/x`を使用します

* **プロパティの深さ**

  リソース内でそのプロパティを検索する最大の深さです。 そのため、そのプロパティの検索は、子のレベルが指定された深さに等しいまで、リソースと再帰的な子で実行できます。
...
次の例（ページの作成に使用するテンプレートに従って検索する場合）は、関連する手順を示しています。
