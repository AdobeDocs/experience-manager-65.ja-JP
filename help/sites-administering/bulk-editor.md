...
1. **編集** の **フォームの開始** 段落を選択します。 ダイアログボックスで、 **詳細** タブ、展開 **アクションの設定**&#x200B;を選択し、次に **データの表示…**.
...
* **制限：** は、間隔を使用して結果の数を制限します。 例えば、limit:10..20 間隔は 0 から始まり、start は含み、end は排他的です。 開いた間隔を指定することもできます（例：:limit:10..または制限：..20 ドットを省略し、値を 1 つだけ指定した場合、GQL は最大でこの数の結果を返します。 例えば、limit:10 と指定します（最初の 10 件の結果を返します）。
...
1. クリック **書き出し** タブ区切りの Excel スプレッドシートに変更内容を書き出すことができます。 AEM WCM が、ファイルをダウンロードする場所を尋ねます。
