...
>[!NOTE]
>
>事前設定済みのレポートは、パフォーマンスを集中的に消費するわけではありませんが、実稼働環境では毎日のスナップショットを使用することをお勧めします。 可能であれば、Webサイトにあまりアクティビティがない時間帯に毎日のスナップショットを実行します。これは、**Day CQレポート設定**&#x200B;の`Daily snapshots (repconf.hourofday)`パラメーターを使用して定義できます。この設定方法の詳細は、[OSGI設定](/help/sites-deploying/configuring-osgi.md)を参照してください。
