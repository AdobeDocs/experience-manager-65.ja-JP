...
>[!NOTE]
>
>事前設定済みのレポートは、パフォーマンスを集中的に消費するわけではありませんが、実稼動環境では毎日のスナップショットを使用することをお勧めします。 可能であれば、Webサイト上でのアクティビティが少ない時間帯に、これらの日別のスナップショットを実行します。これは、**Day CQ Reporting Configuration**&#x200B;の`Daily snapshots (repconf.hourofday)`パラメーターを使用して定義できます。この設定方法について詳しくは、[OSGI設定](/help/sites-deploying/configuring-osgi.md)を参照してください。
