...
* 列を削除するには、列のタイトルをクリックし、レポートのヘッダー領域に長押ししてドラッグします（赤いマイナス記号は位置が有効でないことを示します）。マウスボタンを放すと、コンポーネントを削除ダイアログが表示されます。
...
* を使用します。 [列のドロップダウンメニュー](#column-drop-down-menu) 具体的に選択する **昇順に並べ替え** または **降順に並べ替え**&#x200B;繰り返しになりますが、タイトルテキストのすぐ横に矢印が付きます。
...
1. **編集** スナップショットの作成頻度（1 時間ごとまたは 1 日ごと）を定義する設定。
