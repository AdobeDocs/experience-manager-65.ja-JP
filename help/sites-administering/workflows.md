...
* **休止**:ワークフローを中断としてマークします。ただし、この状態に関する既知の問題については、以下の注意事項を参照してください。
...
* **休止**:一時停止中にワークフローの状態が「休止」に変更されました。以下の「注意」を参照してください。

>[!CAUTION]
>
>ワークフロー状態を「休止」にマークすると、既知の問題が発生します。 この状態では、受信トレイ内の中断されたワークフローアイテムに対して操作を実行できます。
