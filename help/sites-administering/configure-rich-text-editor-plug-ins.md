...
1. スタイルノードの下に新しいノードを作成します。 詳しくは、 [新しいスタイルを指定する](#stylesindropdown).
   * 名前：`jpn-word-wrap`
   * 型：`nt:unstructure`
...
1. スタイルシートを作成し、そのパスを指定します。 詳しくは、 [スタイルシートの場所を指定](#locationofstylesheet). 次の内容をスタイルシートに追加します。 必要に応じて背景色を変更してください。
...
>[!CAUTION]
>
RTE の段落書式を設定する際に、書式オプションとしての段落タグ &lt;p> を削除しないでください。次の場合、 `<p>` タグが削除された場合、コンテンツ作成者は **段落書式** オプションを使用できます。
