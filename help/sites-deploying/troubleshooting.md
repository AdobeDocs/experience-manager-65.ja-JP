...
ヒープサイズを既に 512 MB に定義している場合は、ヒープダンプを作成してメモリの問題をさらに分析する必要があります。
...
多くの場合、一定期間に収集された 3 つのヒープダンプファイルは、問題の分析に必要です。

* エラーが発生する前に
* 失敗 1 時
* 失敗 2 時
* *イベントの解決後に情報を収集することも理想的です。*

これらを比較して、変更やオブジェクトがメモリをどのように使用しているかを確認できます。

>[!NOTE]
>
>このような情報を定期的に収集する場合や、ヒープダンプの読み取りに経験がある場合は、1 つのヒープダンプファイルで問題を分析するのに十分です。
