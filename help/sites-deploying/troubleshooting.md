...
既にヒープサイズを 512 MB として定義している場合、ヒープダンプを作成してメモリの問題を詳細に分析することができます。
...
多くの場合、問題を分析するには、一定期間にわたって収集された 3 つのヒープダンプファイルが必要です。

* エラーが発生する前
* エラー 1 の間
* エラー 2 の間
* *イベントが解決した後に情報を収集することも理想的です*

これらを比較することで、オブジェクトがメモリをどのように使用しているかの変化を確認できます。

>[!NOTE]
>
>このような情報を定期的に収集する場合、またはヒープダンプの読み取り経験がある場合は、1 つのヒープダンプファイルで問題を分析するだけで十分です。
