...
不適切に長い時間を要するインデックス再作成と、大量のコンテンツのインデックス作成を行うので、長い時間を要するインデックス再作成を区別することが重要です。 例えば、コンテンツのインデックス作成に要する時間はコンテンツの量に応じて増えるので、大規模な実稼動リポジトリのインデックス再作成には、小規模な開発リポジトリよりも時間がかかります。
