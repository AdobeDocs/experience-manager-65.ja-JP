...
* この **type** プロパティは、インデックスのタイプを指定します。この場合、 **プロパティ**
...
* この **declaringNodeTypes** プロパティを使用すると、インデックスの適用先となる特定のノードタイプを指定できます。
* この **再インデックス** に設定されている場合はフラグを設定 **true**、コンテンツの完全なインデックス再作成をトリガーします。
...
* この **非同期** に設定する必要があるプロパティ **非同期**. これにより、インデックス更新プロセスがバックグラウンドスレッドに送られます。
* この **includePropertyTypes** プロパティ。インデックスに含めるプロパティタイプのサブセットを定義します。
* この **excludePropertyNames** プロパティ名のリストを定義する — インデックスから除外する必要のあるプロパティ。
* この **再インデックス** に設定する際にフラグを設定 **true**、コンテンツの完全なインデックス再作成をトリガーします。
...
上記のクエリの Lucene プロパティインデックスを定義するには、以下の定義を追加します。それには、以下にノードを作成します。 **oak:index:**
...
1. default ノードに次のプロパティを追加します。
   ...
   * **値：** `LUCENE_47`
   ...
   If `luceneMatchVersion` が指定されていない場合、Oak は付属の Lucene のバージョンを使用します。
...
1. 選択 **リモート Solr** を、 **Oak Solr** サーバプロバイダ。
