...
AEM バージョン 6.0から6.5まで直接アップグレードできます。 5.6.x以前を実行しているお客様は、まずバージョン 6.0以降にアップグレードする必要があり、6.0 （SP3）が推奨されます。 また、6.3 以降では新しい OAK Segment Tar 形式がセグメントノードストアに使用されるようになり、この新しい形式へのリポジトリ移行は 6.0、6.1 および 6.2 でも必須です。

>[!CAUTION]
>
>AEM 6.2 から 6.3 にアップグレードする場合は、バージョン（**6.2-SP1-CFP1 - -6.2SP1-CFP12.1**）または&#x200B;**6.2SP1-CFP15** 以降からアップグレードする必要があります。 そうでない場合は、**6.2SP1-CFP13/6.2SP1CFP14**&#x200B;からAEM 6.3にアップグレードする場合は、少なくともバージョン **6.3.2.2**&#x200B;にアップグレードする必要があります。 そうしないと、AEM Sites はアップグレード後に失敗します。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>小さな影響～大きな影響</td>
   <td>Oak ではインデックスが自動的に生成されないため、アプリケーションとカスタマイズによっては、<br />JVM、オペレーティングシステムのバージョンおよびインデックス作成関連<br />の変更に依存する場合があります。</td>
  </tr>
  <tr>
   <td>カスタムアプリケーションコンテンツ</td>
   <td>小さな影響～大きな影響</td>
   <td>アップグレードを通じて処理されないコンテンツは、アップグレードが行われる前に<br /> バックアップしてから、リポジトリに戻すことができます。<br /> ほとんどのコンテンツは、移行ツールを通じて処理できます。</td>
  </tr>
 </tbody>
</table>
...
アップグレード時に、オペレーティングシステムや JVM など、技術スタックの他のコンポーネントのアップグレードも必要になる可能性があります。 また、リポジトリ構成の変更により、追加のハードウェアが必要になる場合もあります。 これは 6.x よりも前のインスタンスから移行する顧客にのみ発生しますが、考慮しておくことが重要です。 最後に、モニタリング、メンテナンスなどの運用作業やバックアップおよび障害回復プロセスに対する変更が必要になる場合があります。
...
[Assets のモニタリングのベストプラクティス](/help/assets/assets-monitoring-best-practices.md)
...
初期評価後、より包括的な次の手順として、テストインスタンスでアップグレードを実行し、基本的なスモークテストを行うことができます。 アドビは一部も提供しています。 また、アップグレード先のバージョンだけでなくソースバージョンとターゲットバージョンの間のバージョンについても、[廃止および削除された機能](/help/release-notes/deprecated-removed-features.md)のリストを確認する必要があります。 例えば、AEM 6.2 から 6.5 にアップグレードする場合、AEM 6.5 以外に AEM 6.3 の廃止および削除された機能を確認することが重要です。
