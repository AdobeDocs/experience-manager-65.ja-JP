...
AEMバージョン6.0以降から6.5に直接アップグレードできます。5.6.x以下を実行しているお客様は、まず6.0(SP3)を推奨して、バージョン6.0以降にアップグレードする必要があります。 また、6.3 以降は新しい OAK Segment Tar 形式がセグメントノードストアに対して使用されており、この新しい形式へのリポジトリ移行は 6.0、6.1 および 6.2 でも必須です。
...
AEM 6.5 と互換性を確保するために[コードのアップグレードとカスタマイズ](/help/sites-deploying/upgrading-code-and-customizations.md)手順を提供します。この反復プロセスが実行されると、必要に応じてランブックに変更を加える必要があります。ほとんどの場合、アップグレード後すぐに開発を必要とせずに、カスタマイズの後方互換性を維持する方法について詳しくは、 AEM 6.5の](/help/sites-deploying/backward-compatibility.md)の[後方互換性も参照してください。
