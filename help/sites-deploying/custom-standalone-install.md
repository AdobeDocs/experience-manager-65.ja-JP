...
* `stdout.log`内の関連するリフレクションアクセスの警告メッセージを防ぐためには、`-add-opens`スイッチを追加する必要があります。
...
1. Windowsは、サービスが実行中であることを示します。AEM開始とprunsrv実行可能ファイルがタスクマネージャーに表示されます。 Webブラウザーで、AEM(例えば、AEMを使用している開始ーに`https://localhost:4502`)に移動します。
