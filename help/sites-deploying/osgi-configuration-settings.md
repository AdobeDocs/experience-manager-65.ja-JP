...
設定を作成する際には、工場出荷時の設定に変更を加えないでください。代わりに、設定マネージャー（**http://localhost:4502/system/console/configMgr/**）を使用して、このコンポーネントの工場出荷時の設定を作成してください。このプロキシ設定は、**org.apache.http.proxyconfigurator** で利用できます。
