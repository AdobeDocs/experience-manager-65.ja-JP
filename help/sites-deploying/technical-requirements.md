...
1. Oracleは、Oracle Java™ SE製品の「長期サポート」（LTS）モデルに移行しました。Java™ 9、Java™ 10およびJava™ 12は、Oracleによる非LTS リリースです（[Oracle Java™ SE サポートロードマップ ](https://www.oracle.com/jp/technetwork/java/eol-135779.html)を参照）。 実稼動環境にAEMをデプロイするために、Adobeでは、Java™のLTS リリースのみをサポートしています。LTS リリースのすべてのメンテナンスアップデートを含むOracle Java™ SE JDKのサポートと配布は、Adobe Java™ SE テクノロジを使用するすべてのAEMのお客様に対して、Oracleで直接サポートされます。Adobe Experience Manager](assets/Java_Policy_for_Adobe_Experience_Manager.pdf)の[Java™ サポートポリシーを参照してください。
   **重要：Oracle Java™ 17および21は[AEM 6.5 LTS](https://experienceleague.adobe.com/ja/docs/experience-manager-65-lts/content/implementing/deploying/introduction/technical-requirements)でサポートされています。**
...
2. MongoDB Enterprise バージョン 4.2 および 4.4 には、最低でも AEM 6.5 SP9 が必要です。
3. MongoDB Sharding は AEM ではサポートしていません。
4. MongoDB Storage Engine WiredTiger のみがサポートされています。
...
| IBM® WebSphere® Application Server 9.0.0.10 | R：新規契約向けの制限サポート `[1]` `[2]` |
...
| JBoss® EAP 7.4 と JBoss® Application Server <sup>[2] [3] [7] | A：サポート対象 |

1. AEM Forms を使用したデプロイメントに推奨されます。
2. アプリケーションサーバーで AEM 6.5 デプロイメントを開始すると、制限付きサポートに移行します。 既存のお客様は AEM 6.5 にアップグレードして、引き続きアプリケーションサーバーを使用することができます。 新規のお客様には、前述のレベル R の説明にあるサポート基準とサポートプログラムが適用されます。
3. 適用可能 AEM Forms のみ：
   * JBoss® EAP 7.1.4 のサポートを削除し、JBoss® EAP 7.4.10 のサポートを追加しました。
   * AEM Forms サービスパック 6.5.25.0以降では、JBoss® EAP 7.4.23のサポートが追加されています。 この[ リンク ](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/featurepack/jboss-eap-7.4.23-1.0.16.zip)を使用すると、ソフトウェア配布ポータルからJBoss® EAP 7.4.23をダウンロードできます。
...
| Linux® （Ubuntuを含むDebian ディストリビューションに基づく） | A：サポート対象 `[1]` `[2]` |
...
* Microsoft® Windows® 2016 Server、Microsoft® Windows® 2019 Server、Microsoft® Windows® 10、Windows® 11またはMicrosoft® Windows Terminal Server 2025
