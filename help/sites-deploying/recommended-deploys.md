...
>[!NOTE]
>
>上の図では、データセンター 2 のAEM サーバーとデータセンター 1 の MongoDB プライマリノードとのネットワーク遅延が、[MongoDB のAEM - Checklists](/help/sites-deploying/aem-with-mongodb.md#checklists) に記載されている要件よりも大きいと仮定して、AEM サーバー 3 とAdobe Experience Manager サーバー 4 のステータスが非アクティブになっています。 例えば、可用性ゾーンの使用などにより、最大遅延が要件に反しない場合は、データセンター 2 の AEM サーバーもアクティブになることができ、結果として、複数のデータセンターにまたがるアクティブ-アクティブ構成の AEM クラスターとなります。
...
>[!NOTE]
>
>[Tough Day](/help/sites-developing/tough-day.md) を使用すると、デプロイされたハードウェア設定のコンテキストで、顧客のアプリケーションのパフォーマンスを評価できます。
...
* [Adobe Experience Managerに関する MongoDB デプロイメントレビュー ](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager) を参照してください。
* [MongoDB 操作チェックリスト ](https://docs.mongodb.org/manual/administration/production-checklist/) を確認してください。
* [MongoDB の認定クラス（オンラインで利用可能 ](https://university.mongodb.com/)）に参加します。
