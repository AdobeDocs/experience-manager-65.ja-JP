...
6.5では、すべての新機能を下位互換性に保つために大きな注目が集まりましたが、下位互換性の問題が引き続き発生する場合は、互換性モードを使用すると、カスタムコードを6.5に準拠させておくために開発を一時的に延期できます。 このアプローチは、アップグレード直後の開発作業を回避するのに役立ちます（[AEM 6.5](/help/sites-deploying/backward-compatibility.md)の後方互換性を参照）。
