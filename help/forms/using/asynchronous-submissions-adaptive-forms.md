...
非同期送信では、ユーザーがフォームを送信すると、フォーム開発者は、他のフォームやWebサイトの別のセクションへのリダイレクトなど、別のエクスペリエンスにプラグインします。 作成者は、別のデータストアへのデータの送信やカスタム解析エンジンの追加など、別のサービスをプラグインすることもできます。非同期送信の場合、アダプティブフォームは単一ページアプリのように動作します。
