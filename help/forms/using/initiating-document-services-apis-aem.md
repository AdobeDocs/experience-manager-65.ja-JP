...
1. *出力ドキュメントのマップ*：各出力ドキュメントの保存場所を明示的に指定します。出力ドキュメントごとに 1 つのエントリを追加します。 各エントリでは、ドキュメントと保存場所を指定します。出力ドキュメントは、ペイロードを上書きしたり、ペイロードフォルダーの下に保存したりできます。複数の出力ドキュメントがあると役に立ちます。
