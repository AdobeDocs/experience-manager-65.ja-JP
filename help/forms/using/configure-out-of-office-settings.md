---
title: 不在時の設定
description: 不在設定をAdobe Experience Manager Formsインスタンスに設定する方法を説明します。
...
>[!NOTE]
>
>* [ 不在 ] 設定を有効にすると、設定を有効にする前にインボックスで使用できる項目はすべてインボックスに残ります。 設定を有効にした後に受信した項目のみが委任されます。
...
1. 「**[!UICONTROL この期間のタスクを転送]**」チェックボックスを選択します。このオプションを選択せず、担当者を指定しない場合、項目はどのユーザーにも転送されません。 設定は有効になっていますが、項目はインボックスに残ります。
1. 「**[!UICONTROL 担当者を追加]**」をタップします。ユーザーを **[!UICONTROL 割り当て先]** フィールドに値を入力して、項目を委任できます。 次を指定します。 **[!UICONTROL ワークフローモデル]** 指定したユーザーに委任できます。 複数のワークフローモデルを選択できます。
...
>[!NOTE]
>
>（フォーム中心のワークフロー項目のみ）ワークフローの「**タスクを割り当て**」ステップの「**担当者が「不在」設定を使用して委任することを許可する**」オプションを有効にします。前述のオプションが有効になっている項目のみが他のユーザーに委任されます。
