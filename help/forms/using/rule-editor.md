...
* 「When」のルール内に記述した条件は、その条件が適用されるオブジェクトの値をユーザーが変更した際に評価されます。ただし、値がサーバー側で変更されたときにアクションをトリガーにする場合は、値の事前入力と同様に、フィールドの初期化時にアクションをトリガーする When ルールを作成することをお勧めします。
...
ラジオボタンやリストなどの複数値コンポーネントに対してルールを作成する場合、オプションが自動的に取得され、それらのオプションを使用してルールを作成できるようになりました。オプションの値を再度入力する必要はありません。
...
* **並べ替え**：新しく作成したルールはすべて、ルールリストの一番下に追加されます。ルールは上から下に順に実行されます。最上部のルールが最初に実行され、その後に同じタイプの他のルールが実行されます。 例えば、上からそれぞれ 1 番目、2 番目、3 番目、4 番目の位置に「When」、「Show」、「Enable」、「When」の各ルールがある場合、一番上の「When」のルールが最初に実行され、次に 4 番目の位置に「When」のルールが実行されます。 そして、「Show」ルールおよび「Enable」ルールが実行されます。
