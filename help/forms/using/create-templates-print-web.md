...
1. **作成**／**ファイルのアップロード**&#x200B;をタップします。
   ...
   に移動して選択します。 **create_first_ic_print_template** template (XDP) をタップし、 **開く** をクリックして、XDP テンプレートをAEM Forms Server に読み込みます。
...
1. タップ **+** の隣の左側のウィンドウで **テーブル** 1.右クリック **セル 1** を選択し、 **オブジェクト名を変更** から **日付**.
...
1. [ プロパティ ] ペインで、次の操作を行います。
   ...
   1. プロパティを保存するには、 ![保存](/help/forms/using/assets/done_icon.png).
...
1. プロパティを保存するには、 ![保存](assets/done_icon.png).
