...
1. Log to AEM [!DNL Forms] author instance in as an administrator. [!UICONTROL CRXDE Liteを開きます]。 ロゴ/ **[!UICONTROL ツール]** / **[!UICONTROL 一般]** /CRXDE Liteをタップして、CRXDE Liteを開くにはブラウザでhttps://localhost:4502/crx/de/index.jspAEM **[!UICONTROL URLを開く]**[](https://localhost:4502/crx/de/index.jsp) か、 urlを開きます。

1. [!UICONTROL CRXDE Liteの/etc/clientlibsに移動します]。 /etc/clientlibs サブフォルダーを右クリックして、**[!UICONTROL 作成]**／**[!UICONTROL ノードを作成]**&#x200B;をクリックします。「 **[!UICONTROL 名前]** 」フィールドにWeRetailFormTestCasesと入力 **します**。 タイプを **cq:ClientLibraryFolderとして選択し** 、「 **[!UICONTROL OK]**」をクリックします。 ノードを作成します。 の代わりに任意の名前を使用でき `WeRetailFormTestCases`ます。
...
1. AEM Testing UI(AEM/ **[!UICONTROL Tools]** / **[!UICONTROL Operations]** / **[!UICONTROL Testing]**)を開きます。 テストスイート( **市販用 — テスト** )がUIに表示されます。
...
1. CRXDE [!UICONTROL Lite]で、 `/content/forms/af/create-first-adaptive-form` フォルダーに移動します。 「 **[!UICONTROL create-first-adaptive-form]** 」フォルダーノードを右クリックし、 **[!UICONTROL 作成]**/ファイルを **[!UICONTROL 作成をクリックします]**。 In the **[!UICONTROL Name]** field, type `prefill.xml` and click **[!UICONTROL OK]**. 次のコードを ファイルに追加します。
...
1. テストスイートのすべてのテストを実行するには：
   ...
   1. Tests [!UICONTROL パネルで、「] We retail - Tests (1) ****」をタップします。 スイートが展開され、テストのリストが表示されます。
...
1. テスト名「Prefill test」をタップして、テストケースの結果を確認します。 「 [!UICONTROL Result] 」パネルが開きます。 「 [!UICONTROL Result] 」パネルでテストケースの名前をタップし、テストのすべての詳細を表示します。
