...
* **[クリック式](../../forms/using/adaptive-form-expressions.md#p-click-expression-p)**:を使用して、ボタンのクリックイベント上の操作を処理します。
...
* **[](../../forms/using/adaptive-form-expressions.md#p-value-commit-script-p)値コミットスクリプト&#x200B;**:フィールドの値が変更された後にフォームのコンポーネントを変更する場合。
* **[視認性の数式](../../forms/using/adaptive-form-expressions.md#p-visibility-expression-p)**：フィールドおよびパネルの視認性をコントロールします。
* **[ステップ完了の式](../../forms/using/adaptive-form-expressions.md#p-step-completion-expression-p)**:ユーザがウィザードの次のステップに進むのを防ぐ。
...
**例**:ボタンのクリックアクシ **ョンで** textbox1に値 **** AEM Formsを設定する場合、ボタンのクリック式は次のようになります。 `textbox1.value="AEM Forms"`
