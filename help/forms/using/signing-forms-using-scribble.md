---
title: 手書き署名（非推奨）を使用したフォームへの電子署名の適用
seo-title: 手書き署名（非推奨）を使用したフォームへの電子署名の適用
...
You can use the (Deprecated) **Scribble Signature** component and **Signature Step** component to draw (Scribble) signature on an adaptive form. 署名ステップコンポーネントでは、アダプティブフォームのPDF バージョンが表示されます。「レコードのドキュメント」オプションを有効にするか、フォームテンプレートベースのアダプティブフォームで署名ステップコンポーネントを使用する必要があります。
...
1. コンテンツブラウザーで「**フォームコンテナ**」をタップし、**設定**&#x200B;アイコン ![](/help/forms/using/assets/configure.png) をタップします。この操作により、アダプティブフォームのコンテナプロパティを表示するプロパティブラウザーが開きます。**アダプティブフォームコンテナ**／**電子署名**&#x200B;に移動して、「**Adobe Sign を有効にする**」オプションを選択解除します。「 ![aem_6_3_forms_save」アイコンをタップして](assets/aem_6_3_forms_save.png) 、変更を保存します。
