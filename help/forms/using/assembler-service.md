...
フォームデザイナーは、LiveCycleデザイナーを使用してフォームフラグメントを作成します。 このようなフラグメントは、XFA フォーム内では一意の名前を持つサブフォームとなります。また、一意の名前を持つ挿入ポイントを含んだ XFA フォームを作成する場合も、Designer を使用します。プログラマーは、XFA フォームへのフラグメントの挿入する方法を指定する DDX ドキュメントを作成します。
...
* しおりに関する情報（ページ番号、タイトル、設定先、外観など）。この\
   PDFドキュメントのデータを取り込み、PDFドキュメントに読み込みます。

* 添付ファイル情報（ファイル情報を含む）。ページレベルの添付ファイルの場合は、\
   添付ファイル注釈の場所。 このデータをPDFドキュメントから書き出し、\
   PDFドキュメントに読み込みます。
