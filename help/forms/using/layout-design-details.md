...
seo-description: レイアウトデザイン レターやInteractive Communicationsで使用するレイアウトの作成方法を詳しく説明します。
...
XDP は Adobe Forms Designer でデザインできます。この記事では、フォームフィールドやターゲット領域を使用する場所、レイアウトフラグメントを使用するタイミングなど、効果的な通信/インタラクティブ通信を作成するためのXDPのデザイン方法について詳しく説明します。
...
インタラクティブコミュニケーションの作成に必要なレイアウトを分析したら、Forms Designerを使用してXDPテンプレートでサブフォームを作成できます。 XDPテンプレートで使用される空のサブフォームコンポーネントは、Interactive Communicationの印刷チャネルにターゲット領域を表示します。

>[!NOTE]
>
>XDPテ追加ンプレートのサブフォームコンポーネントにコンテンツを追加する代わりに、Interactive Communicationの印刷チャネルにコンテンツを追加する。 追加ターゲットフラグメント、グラフ、画像、フラグメントを使用して、印刷チャネルの [ドキュメント領域に対するコンテンツ](create-interactive-communication.md#step2)を提供します。
...
1. **サブフォーム**&#x200B;コンポーネントを&#x200B;**オブジェクトライブラリ**&#x200B;からフォームにドラッグアンドドロップします。
   ...
   ![コンポーネントデザイナ](assets/subform_component_designer_new.png)
...
1. Select the **Subform** tab and select **Flowed** from the **Content** drop-down list. 「**連結**」タブで、次の手順を実行します。
   ...
   1. 「名前」フィールドにサブフォームの名前を指 **定します** 。
   1. 「**データ連結**」ドロップダウンリストから「**データ連結なし**」を選択します。
   手順2 ～ 5を繰り返して、XDPテンプレートにサブフォームを追加します。 インタ追加ラクティブ・コミュニケーションの作成時にのみ [](create-interactive-communication.md#step2) 、テキスト、ドキュメント・フラグメント、ターゲット、グラフが領域に表示されます。
...
>[!NOTE]
>
>XDPテ追加ンプレートの画像フィールドまたはサブフォームコンポーネントに画像を追加する代わりに、Interactive Communicationの印刷チャネルに画像を追加する。 詳しくは、「インタラクティブ通信 [へのコンテンツの追加」を参照してください](../../forms/using/create-interactive-communication.md#step2)。
...
>[!NOTE]
>
>テーブルのセルにテキストを入力するか、 [Interactive Communicationの作成時にのみフォームデータモデルのオブジェクト](create-interactive-communication.md#step2) との連結を作成します。

次の手順を実行して、Forms Designerを使用してXDPテンプレートで表コンポーネントを使用します。

1. Drag-and-drop the **Table** component from the **Object Library** to the form.
1. In the **Insert Table** dialog:

   1. 表の行数と列数を指定します。
   1. 「テーブルにヘ **ッダー行を含む」チェックボックスを選択して** 、テーブルヘッダーの行を含めます。
   1. 「**OK**」をタップします。

1. テーブル **名** の横の左ペインで、+をタップし、ヘッダーと他の行に含まれるセル名を右クリックし、「オブジェクト名の変更 **** 」を選択してテーブルセルの名前を変更します。
1. デザインヘッダーのテキストフィールドをクリックし **、表示** 、名前を変更します。
1. 「テキストフィールド」コンポーネ **ントを** 「オブジェクトライブラリ」から **「デザイン** 」表示の各テーブルセルにドラッグ&amp;ドロ **ップします**。 この手順を実行すると、インタラクティブ通信の作成時に、テーブルのセルをフォームデータモデルのオブジェクトと連結できます。

   ![テーブルのテキストフィールド](assets/text_fields_table_new.png)

1. Select the name of the row from the left pane and select **Object** > **Binding** > **Repeat Row for Each Data Item**. この手順を実行して、コレクション型のフォームデータモデルオブジェクトを使用して、この行のテーブルセル間に連結が作成された場合、データベースで使用可能な各データ項目に対してテーブル行が自動的に繰り返されるようにします。
