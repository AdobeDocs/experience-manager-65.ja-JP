...
1. 選択 **[!UICONTROL 作成]** を選択し、 **[!UICONTROL インタラクティブ通信]**. インタラクティブ通信を作成ページが表示されます。
...
1. 選択 **[!UICONTROL 次へ]**. 印刷チャネルと web チャネルの詳細を指定するための画面が表示されます。
1. 以下を入力します。
   ...
   * **[!UICONTROL Web チャネルのマスターとして印刷を使用]**：印刷チャネルに同期される web チャネルを作成する場合は、このオプションを選択します。印刷チャネルを Web チャネルのマスターとして使用すると、Web チャネルのコンテンツとデータの連結が印刷チャネルから派生し、印刷チャネルで行った変更が、「同期」を選択したときに Web チャネルに反映されます。 ただし、作成者は、web チャネル内の特定のコンポーネントについて、必要に応じて継承設定を解除することができます。詳しくは、[Web チャネルと印刷チャネルの同期](../../forms/using/create-interactive-communication.md#synchronize)を参照してください。
...
1. 「**[!UICONTROL 作成]**」を選択します。インタラクティブ通信が作成され、警告ボックスが表示されます。選択 **[!UICONTROL 編集]** インタラクティブ通信のコンテンツを作成します。詳しくは、 [インタラクティブ通信のオーサリングユーザーインターフェイスを使用してコンテンツを追加する](#step2). または、 **[!UICONTROL 完了]** 後でインタラクティブ通信を編集することを選択します。
...
1. インタラクティブ通信オーサリングインターフェイスは、「編集」を選択したときに起動します。 [インタラクティブ通信の作成](#createic). または、AEM上の既存のインタラクティブ通信アセットに移動して選択し、「 」を選択することもできます。 **[!UICONTROL 編集]** をクリックして、インタラクティブ通信のオーサリングインターフェイスを起動します。
...
ドキュメントフラグメントを選択し、 ![configure_icon](assets/configure_icon.png) （設定）をクリックし、 **[!UICONTROL プロパティ]** インタラクティブ通信のサイドキックから 「**[!UICONTROL 変数とデータモデルオブジェクト]**」セクションには、非表示の変数を含む変数と、ドキュメントフラグメントで使用されるデータモデルオブジェクトが一覧表示されます。各データモデルオブジェクトまたは変数の横にある「![編集](assets/edit.svg)」（編集）アイコンを使用して、プロパティを編集します。

1. 変数の連結を設定するには、変数を選択し、「 ![configure_icon](assets/configure_icon.png) （設定）を行い、サイドバーのプロパティパネルで連結プロパティを設定します。
...
1. Web チャネルにコンポーネントを追加するには、 Web チャネルを選択した状態で、「 」を選択します。 **[!UICONTROL コンポーネント]**. 必要に応じて、インタラクティブ通信の web チャネルにコンポーネントをドラッグアンドドロップし、そのコンポーネントの設定を行います。
   ...
   | テキスト | インタラクティブ通信の web チャネルにリッチテキストを追加できます。テキストでは、フォームデータモデルオブジェクトを使用して、コンテンツを動的にすることもできます。 |
...
1. プレビューする（印刷または Web）チャネルを選択し、プレビューを選択します。 インタラクティブ通信が表示されます。
...
1. 印刷チャネルで、ドキュメントコンテナをハイライト表示し、「 **プロパティ**.
...
1. 「**添付ファイル**」を展開し、以下のプロパティを指定します。
   ...
   * **[!UICONTROL 添付するファイル]**：を選択します。 **[!UICONTROL 追加]** 添付するファイルを参照して選択し、次の項目を指定します。
...
1. 選択 ![done_icon](assets/done_icon.png).
...
データの表示パターンを設定するには、要素を選択し、 ![configure_icon](assets/configure_icon.png) （設定）を行い、 **[!UICONTROL プロパティ]** パネルを使用して、 **[!UICONTROL タイプ]**&#x200B;のドロップダウンリストから事前定義されたオプションを選択して、選択したタイプに関連付けられたパターンを表示します。リストにないパターンを定義するには、**[!UICONTROL タイプ]**&#x200B;のドロップダウンリストから「**[!UICONTROL カスタム]**」を選択します。「**[!UICONTROL パターン]**」フィールドの値を編集すると、タイプは自動的に「**[!UICONTROL カスタム]**」に変更されます。
...
インタラクティブ通信内のコンポーネントまたはコンテンツに条件を設定するには、コンテンツのコンポーネントまたは部分を選択し、「 」を選択します。 ![createruleicon](assets/createruleicon.png) （ルールを作成）をクリックして、ルールエディターを起動します。
...
1. テーブル内のセルごとに、データ連結を指定します。繰り返し可能な行を作成するには、フォームデータモデルのプロパティを、共通コレクションプロパティに属する行に挿入します。
   ...
   1. テーブル内のセルを選択し、「 」を選択します。 ![configure_icon](assets/configure_icon.png) （設定）を参照してください。
...
1. 任意のテーブル行を選択して、コンポーネントツールバーを表示します。 インタラクティブ通信のサイドキックにあるコンテンツブラウザーを使用して、テーブルの行を選択することもできます。
...
1. テーブルセル内のデフォルトのテキストボックスを選択し、「 」を選択します。 ![編集](assets/edit.png) （編集）。
1. テーブルのセルにテキストを入力し、「 」を選択します。 ![done_icon](assets/done_icon.png) をクリックして保存します。

#### テーブルセルとデータモデルオブジェクト要素の間に連結を作成 {#createbindingtablecells}

1. テーブル行のデフォルトのテキストボックスを選択し、「 」を選択します。 ![編集](assets/edit.png) （編集）。
1. 「データモデルオブジェクト」ドロップダウンリストを選択し、プロパティを選択します。
1. 表のセルとデータモデルオブジェクトのプロパティの間の連結を保存して作成する場合に選択します。

![データ連結を作成](assets/create_data_binding_table_new.png)

#### テーブルセルのテキストにハイパーリンクを作成 {#createhyperlinktable}

1. テーブルセル内のデフォルトのテキストボックスを選択し、「 」を選択します。 ![編集](assets/edit.svg) （編集）。
1. テーブルセル内のテキストを選択し、ハイパーリンクアイコンを選択します。
...
インタラクティブ通信の web チャネルに、ボタンをコンポーネントとして追加できます。を使用してルールを定義する [ルールエディター](../../forms/using/rule-editor.md) を使用すれば、他のインタラクティブ通信、アダプティブフォーム、画像やドキュメントフラグメントなどの他のアセット、または「 」ボタンを選択して外部 URL に移動できます。

ボタンを追加してルールを定義するには：

1. Web テンプレートのルートパネルを選択し、「 」を選択します。 **+** を追加します。 **ボタン** コンポーネントをインタラクティブ通信に追加します。
1. ボタンコンポーネントを選択し、「 」を選択します。 ![edit-rules](assets/edit-rules.png) をクリックして、「 」ボタンの選択に関するルールを定義します。
...
1. 次の場所からパネルを選択します。 **コンテンツ** ブラウザーを使用して、設定、ルールの編集、コピー、削除、コンポーネントの挿入など、追加のアクションをパネルで実行できます。
...
Web チャネルの関連するターゲット領域または変数の上にマウスポインターを置いて、「 」を選択します。 ![cancelinherance](assets/cancelinheritance.png) （継承をキャンセル）を選択し、継承をキャンセルダイアログで、 **[!UICONTROL はい]**.

ターゲット領域内でコンポーネントの継承がキャンセルされ、必要に応じてコンポーネントを編集できるようになります。

### 継承を再度有効にする {#re-enable-inheritance}

Web チャネルでコンポーネントの継承をキャンセルした場合は、その継承を再度有効にすることができます。継承を再度有効にするには、コンポーネントを含む関連するターゲット領域の境界線にカーソルを合わせ、「 」を選択します。 ![reenableinheritance](assets/reenableinheritance.png).

継承を元に戻すためのダイアログが表示されます。

![revertinheritance](assets/revertinheritance.png)

必要に応じて、「 」を選択します。 **[!UICONTROL 継承を元に戻してからページを同期]**. インタラクティブ通信を全体的に同期する場合に、このオプションを選択してください。このオプションを選択しなかった場合は、継承を元に戻したときに、関連するターゲット領域だけが同期されます。
...
1. Web チャネルと印刷チャネルを同期するには、Web チャネルに切り替えて、その他のオプションアイコンを選択します。
...
1. コンポーネントを選択し、グループ ( ![グループ](assets/group.jpg)) 操作を実行します。
1. 複数のコンポーネントを選択して選択 **パネル内のオブジェクトのグループ化**.
