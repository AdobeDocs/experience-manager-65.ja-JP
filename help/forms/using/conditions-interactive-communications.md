...
1. 次の情報を指定します。
   ...
   * **[!UICONTROL タグ]**:（オプション）カスタムタグを作成するには、テキストフィールドに値を入力して、「Enter」を選択します。 この条件を保存すると、新しく追加されたタグも作成されます。
...
1. 条件に含める適切なアセットを選択し、「 」を選択します。 **[!UICONTROL 完了]**.
   ...
   **[C] アセットを複製。** このアイコンを選択すると、適用されたルール（存在する場合）と共に条件にアセットのコピーが作成されます。 その後、複製されたアセットで、ルールとアセットを編集できます。アセットの複製は、特定のコンテキストに基づいて代替アセットを表示する類似のルールを作成する場合に便利です。
...
1. 選択 **[!UICONTROL ルールを作成]** ：必要に応じて、アセットの表示と非表示を切り替えるルールを追加します。 ルールでの変数の使用について詳しくは、[変数の作成](#variables)を参照してください。詳しくは、[条件にルールを追加する](#ruleeditor)を参照してください。
...
1. 評価式、文字列、データモデルプロパティ、変数、または日付を挿入します。
   ...
   * ルールを作成または編集する際に、「 ![icon_resize](assets/icon_resize.png) （サイズ変更）をクリックして、[ ルールを作成/ルールを編集 ] ダイアログを展開します。 ダイアログを拡張して全画面表示にすると、ルールを構成する[変数](#variables)を作成できるようになります。もう一度「サイズ変更」を選択すると、通常の「ルールを作成」ダイアログに戻ります。
...
条件でルールを作成または編集する際に、 ![icon_resize](assets/icon_resize.png) （サイズ変更）をクリックして、[ ルールを作成\ルールを編集 ] ダイアログを展開します。 ダイアログを拡張して全画面表示にすると、以下の操作を実行できるようになります。
