...
**接続** API はハンドラーを引数として受け入れます。XFAベースのHTML5フォームとForm bridgeの間に正常に接続が作成された後、ハンドルが呼び出されます。
...
* **必要情報:**
   ...
   * **** 値：som配列で提供されるSOM式に対応する値を含む配 ****&#x200B;列です。 値のデータ型がfieldTypeと異なる場合、値は変更されません。
...
* **** 出力：結果をArrayとしてdataプロパティに含むオブ *ジェクト*
