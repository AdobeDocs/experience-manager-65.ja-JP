...
**ドキュメントスイープの間隔（秒）** ：不要になったファイルを削除しようとしたときに、サービス間でドキュメントデータを渡すために使用された時間（秒単位）です。
