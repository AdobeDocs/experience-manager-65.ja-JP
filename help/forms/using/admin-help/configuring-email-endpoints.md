...
**ファイルパターン：** プロバイダーが受け入れる受信ファイル添付パターン。これには、特定の拡張子 (&amp;ast;.dat, &amp;ast;.xml) を持つファイル、特定の名前（データ）、名前と拡張子の複合式 (.[dD][aA]&#39;port&#39;) と同じです。デフォルト値は&amp;ast；です。&amp;ast;.
...
**ファイルパターン：** プロバイダーが受け入れる受信ファイル添付パターンを指定します。これには、特定の拡張子 (&amp;ast;.dat, &amp;ast;.xml) を持つファイル、特定の名前（データ）、名前と拡張子の複合式 (&amp;ast;) が含まれます。[dD][aA]&#39;port&#39;) と同じです。
