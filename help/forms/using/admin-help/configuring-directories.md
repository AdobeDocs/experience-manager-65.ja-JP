...
**Base DN:** Set as the starting point for synchronizing users and groups from the LDAP hierarchy. サービスのために同期する必要があるすべてのユーザーおよびグループを含む階層の最下位レベルの BaseDN を指定することをお勧めします。
...
**フルネーム：** （必須）スキーマのフルネームのユーザー属性。

**ログインID:** （必須）スキーマのログインIDのユーザー属性。

**姓：** （必須）スキーマの姓のユーザー属性。

**名前：** （必須）スキーマの名のユーザー属性。

**イニシャル：** スキーマのイニシャルのイニシャル属性。
...
**組織：** スキーマが属する組織の名前のユーザー属性。

**プライマリ電子メール：** スキーマのプライマリ電子メールアドレスのユーザー属性。

**セカンダリ電子メール：** スキーマのセカンダリ電子メールアドレスのユーザー属性。

**電話：** スキーマの電話番号のユーザー属性。

**住所：** スキーマの住所の住所のユーザー属性。

**ロケール：** ISOロケール情報を含むスキーマ属性。 この値は 2 文字の言語コードまたは言語および国コードです。

**タイムゾーン：** スキーマが存在するタイムゾーンを含むユーザー属性。 この値は「市区町村／国」などの文字列です。
...
**フルネーム：** （必須）スキーマ属性（グループのフルネーム）

**メンバDN:** （必須）スキーマ属性。グループ内のメンバーの識別名です。
...
**組織：** スキーマ属性：グループが属する組織の名前

**プライマリ電子メール：** スキーマ属性（グループのプライマリ電子メールアドレス）

**セカンダリ電子メール：** スキーマのグループのセカンダリ電子メールアドレスの属性。
