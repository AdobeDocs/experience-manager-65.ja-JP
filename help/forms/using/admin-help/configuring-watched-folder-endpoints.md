...
**Include File Pattern:** （必須）セミコロン **;** スキャンおよび取得の対象とするフォルダーとファイルを決定するために監視フォルダーで使用されるパターンの区切り形式のリストです。 例えば、「ファイルパターンを含める」がinput&amp;ast；の場合、input&amp;ast；に一致するすべてのファイルとフォルダー拾い上げられます。 input1、input2 などの名前を持つファイルおよびフォルダーが含まれます。
