...
カラー画像とグレースケール画像の解像度設定は、ファイルを出力する際の走査線数の 1.5～2 倍にする必要があります。（この推奨解像度の設定を下回らない場合、直線を含まない画像、または幾何学的パターンや繰り返しパターンを含む画像は、低解像度の影響を受けません）。 モノクロ画像の解像度は、出力デバイスと同じである必要があります。 ただし、1500 dpi を超える解像度でモノクロ画像を保存すると、ファイルサイズだけが大きくなり、画質は大幅には向上されません。
