...
## カスタム ID および Trust キーストアの作成 {#create-the-custom-identity-and-trust-keystores}
...
1. 次のコマンドを入力して、証明書を新しいキーストアファイル（カスタム Trust キーストア）に挿入します。
...
作成したカスタム ID キーストアとカスタム Trust キーストアを使用するように、WebLogic を設定します。また、キーストアファイルの作成に使用する識別名には、WebLogic Server をホストするコンピューターの名前が含まれていないので、WebLogic のホスト名の検証機能を無効にします。

## SSL を使用する WebLogic を設定 {#configure-weblogic-to-use-ssl}
...
1. 「General」の「Configuration」で、「**Listen Port Enabled**」と「**SSL Listen Port Enabled**」が選択されていることを確認します。有効でない場合は、次の手順を実行します。
   ...
   1. 「**Listen Port Enabled**」チェックボックスと「**SSL Listen Port Enabled**」チェックボックスをオンにします。

1. このサーバーが管理対象サーバーである場合は、リッスンポートを未使用のポート番号（8001 など）に、SSL リッスンポートを未使用のポート番号（8002 など）に変更します。スタンドアロンサーバーの場合、デフォルトの SSL ポートは 7002 です。
...
1. 「ID」で、次の値を指定します。
   ...
   **Custom Identity Keystore Passphrase**：*mypassword*（カスタム ID キーストアのパスワード）
...
1. 「Trust」で、次の値を指定します。
   ...
   **Custom Trust Keystore Pass Phrase**: *mypassword*（カスタム信頼キーのパスワード）
...
1. 「詳細」で、ホスト名の検証リストから「なし」を選択します。

   ホスト名の検証が無効になっていない場合、共通名（CN）にサーバーのホスト名が含まれている必要があります。

1. チェンジ センターで「ロックして編集」をクリックし、選択内容と値を変更します。
