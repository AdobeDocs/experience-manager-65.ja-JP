...
**ベースURL:** ベースドキュメントセキュリティURL。サーバー名とポートが含まれます。 ベースに情報が追加されて、接続 URL が作成されます。例えば、/edc/Main.do が追加されて Web ページへのアクセスが行われます。ユーザーは、この URL を使用して、外部ユーザー登録の招待に応答することもできます。
...
**デフォルトのオフライン同期期間：** 最初に保護されたドキュメントがオフラインで使用できる最大時間です。

**クライアントセッションタイムアウト：** クライアントアプリケーションを使用してログインしたドキュメントがドキュメントセキュリティを操作しない場合に、ユーザーセキュリティが切断されるまでの時間（分単位）です。

**匿名ユーザーのアクセスを許可：** 匿名ユーザーがポリシーで保護されたポリシーを開くことを許可する共有ポリシーおよび個人用ポリシーを作成する機能を有効にするには、このドキュメントを選択します。 （アカウントを持たないユーザーでもドキュメントにアクセスできますが、Document Security にログインしたり、他のポリシーで保護されたドキュメントを使用したりすることはできません）。

**バージョン7クライアントへのアクセスの無効化：** ユーザーがAcrobatまたはReader 7.0を使用してサーバーに接続できるかどうかを指定します。 このオプションが選択されている場合、ユーザーは Acrobat または Reader 8.0 以降を使用して、PDF ドキュメントで Document Security の処理を実行する必要があります。ポリシーで保護されたドキュメントを開く際に、Acrobat または Reader 8.0 以降を認証モードで実行することがポリシーによって指定されている場合は、Acrobat または Reader 7 へのアクセスを無効にする必要があります（ユーザーおよびグループのドキュメント権限の指定を参照）。

**[ドキュメントごとにオフラインアクセスを許可** ]このオプションを選択すると、ドキュメントごとにオフラインアクセスを指定します。 この設定が有効な場合、ユーザーは少なくとも 1 回オンラインで開いたドキュメントのみに対し、オフラインアクセス権があります。
...
**拡張認証の優先HTML制御幅** Acrobatで開き、ユーザーの資格情報を入力するための拡張認証ダイアログの幅を指定します。

**拡張認証の推奨HTML制御の高さ** Acrobatで開き、ユーザーの資格情報を入力するための拡張認証ダイアログの高さを指定します。

***注意&#x200B;**:このダイアログボックスの幅と高さの制限は、次のとおりです。*幅：最小= 400、最大= 900
...
**表示ドキュメント:** 受信者表示は、ポリシーで保護されたドキュメントです。

**閉じるドキュメント:** 受信者がポリシーで保護されたドキュメントを閉じる。

**低解像度で印刷** 受信者は、ポリシーで保護されたドキュメントを低解像度で印刷します。

**高解像度で印刷：** 受信者は、ポリシーで保護されたドキュメントを高解像度で印刷します。

**注追加釈のドキュメント:** 受信者がPDF注釈を追加します。ドキュメント

**失効ドキュメント:** ユーザーまたは管理者がポリシーで保護されたユーザーへのアクセス権限を失効させました。ドキュメント

**失効取り消しドキュメント:** ユーザーまたは管理者が、ポリシーで保護されたユーザーへのアクセス権を復元しました。ドキュメント

**フォーム入力：** 受信者が入力可能なフォームであるPDFドキュメントに情報を入力します。

**削除されたポリシー：** 発行者は、セキュリティ保護を取り消すためにドキュメントからポリシーを削除します。

**変更ドキュメントの失効URL:** APIレベルからの呼び出しによって、失効したドキュメントに代わる新しいドキュメントにアクセスするために指定された失効URLが変更されます。

**変更ドキュメント:** 受信者は、ポリシーで保護されたドキュメントの内容を変更します。

**署名ドキュメント:** 受信者がドキュメントに署名

**新しいドキュメントの保護：** ユーザーがポリシーを適用して、ユーザーを保護しているドキュメント。

**ポリシーの切り替えドキュメント:** ユーザーまたは管理者が、ユーザーに割り当てられているポリシーを切り替えました。ドキュメント

**公開ドキュメント:** 新しいドキュメントのdocumentNameとライセンスが既存のドキュメントと同じで、ドキュメントに親子関係がない。 このイベントは、AEM Forms SDK を使用してトリガーされます。

**反復ドキュメント:** 新しいドキュメントのdocumentNameとライセンスが既存のドキュメントと同じである場合、そのドキュメントは親子関係を持ちます。 このイベントは、AEM Forms SDK を使用してトリガーされます。
...
**招待ユーザーの登録：** 外部ユーザーが登録セキュリティにドキュメントします。

**ログイン成功：** 管理者またはユーザーがログインに成功しました。

**招待ユーザー：** ドキュメントセキュリティがユーザーに登録を勧めました。

**アクティブ化されたユーザー：** 外部ユーザーがアカウントのアクティベートを行うには、アクティベーションの電子メール内のURLを使用するか、管理者がアカウントを有効にします。
...
**DirectorySynchronization Complete:** この情報は、「情報」ページからはイベントできません。 現在の同期状態、最後の同期時刻など、現在のディレクトリ同期情報は、ドメインの管理ページに表示されます。管理コンソールで、ドメインの管理ページにアクセスするには、設定／User Management／ドメインの管理をクリックします。

**Client Enable Offline Access:** ユーザーが、ユーザーのコンピューター上のドキュメントに対してセキュリティで保護されたサーバーへのオフラインアクセスを有効にしました。

**同期されたクライアント** ・クライアント・アプリケーションは、オフライン・アクセスを許可するために、サーバと情報を同期する必要があります。

**バージョンの不一致：** サーバーと互換性のないバージョンのAEM forms SDKがサーバーに接続しようとしました。

**ディレクトリ同期情報：** この情報は、「情報」ページからはイベントできません。 現在の同期状態、最後の同期時刻など、現在のディレクトリ同期情報は、ドメインの管理ページに表示されます。管理コンソールで、ドメインの管理ページにアクセスするには、設定／User Management／ドメインの管理をクリックします。

**サーバー設定の変更：** サーバー設定に対する変更。Webページを通じて行うか、config.xmlファイルを読み込んで手動で行います。 これには次の設定に対する変更が含まれます。ベース URL、セッションタイムアウト、ログインロックアウト、ディレクトリ設定、キーのロールオーバー、外部登録に関する SMTP サーバー設定、透かし設定、表示オプションなど。
...
**検索結果に表示する行：** 検索が実行されたときにページに表示される行数。

**クライアントログインダイアログのカスタマイズ**

以下の設定により、ユーザーがクライアントアプリケーションを介して Document Security にログインするときに表示されるログインプロンプトのテキストを指定できます。

**ウェルカムテキスト：** 「ユーザー名とパスワードを使用してログインしてください」など、お知らせメッセージのテキスト。 ウェルカムメッセージのテキストには、Document Security へのログイン方法と、管理者または組織内のサポート担当者への連絡方法が記載されている必要があります。例えば、外部ユーザーがパスワードを忘れた場合や、登録方法やログイン方法に関して質問がある場合は、管理者に問い合わせる必要があります。このウェルカムテキストでは最大 512 文字を使用できます。
...
**ChooseAuthentication Type Text:** ユーザーに認証タイプを選択するよう指示するために表示されるテキスト。

**証明書テキストの選択：** ユーザーに証明書の種類を選択するよう指示するために表示されるテキストです。

**Certificates Not Available Error Text:** 選択した証明書が使用できない場合に表示する512文字までのメッセージです。

**クライアント証明書表示のカスタマイズ**

**Display Only Trusted Credential Issuers:** このオプションを選択すると、AEM formsで信頼するように設定されている秘密鍵証明書発行者からの証明書のみが、クライアントアプリケーションによってユーザーに表示されます（証明書と秘密鍵証明書の管理を参照）。このオプションの選択を解除すると、ユーザーのシステム上の証明書すべてがユーザーに一覧表示されます。
...
**SMTPサーバーのソケットクラス名：** SMTPサーバーのソケットクラス名。 例えば、javax.net.ssl.SSLSocketFactory です。

**電子メールコンテンツタイプ：** text/plainやtext/htmlなど、受け入れられるMIMEタイプ。

**電子メールのエンコーディング：** 電子メールメッセージの送信時に使用するエンコーディング形式。 任意のエンコードを指定できます。例えば、Unicode の UTF-8 や Latin の ISO-8859-1 です。デフォルトは UTF-8 です。

**リダイレクト電子メールアドレス：** この設定の電子メールアドレスを指定すると、新しい招待が指定のアドレスに送信されます。 この設定は、テストを目的とする場合に役立ちます。
...
**差出人：** 招待の電子メールの送信元の電子メールアドレス。 The default format of the From email address is postmaster@[your_installation_domain].com.

**件名：** 招待の電子メールメッセージのデフォルトの件名です。
...
**差出人：** アクティベーションの電子メールの送信元の電子メールアドレス。 この電子メールアドレスに、登録者の電子メールホストから配信失敗通知が届きます。また、受信者が登録電子メールへの応答として送信するメッセージも届きます。The default format of the From email address is postmaster@[your_installation_domain].com.

**件名：** 電子メールメッセージのアクティベーションのデフォルトの件名。

**タイムアウト：** ユーザーがアカウントをアクティベートしない場合にアクティベーションの招待が期限切れになるまでの日数です。 デフォルト値は 30 日です。

**メッセージ：** メッセージの本文に表示されるテキストで、受信者のユーザーアカウントをアクティブにする必要があることを示すメッセージです。 それ以外の情報として、新しいパスワードを取得するために管理者に問い合わせる方法なども含めるとよいでしょう。
...
**差出人：** パスワードリセット用の電子メールの送信元の電子メールアドレス。 The default format of the From email address is postmaster@[your_installation_domain].com.

**件名：** リセット用の電子メールメッセージのデフォルトの件名です。

**メッセージ：** メッセージの本文に表示される、メッセージの外部受信者パスワードがリセットされたことを示すテキスト。
