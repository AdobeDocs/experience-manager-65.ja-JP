...
>[!NOTE]
>
> 外部ブラウザー認証は、JEE サービスパック 6.5.25.0以降のAEM Formsで利用できます。 Service Pack 6.5.24.0以前を使用している場合は、Service Pack 6.5.25.0にアップグレードするか、該当するJEE ホットフィックス パッチをインストールしてください。 ドキュメントのセキュリティについては、[外部ブラウザーからの拡張認証の設定](/help/forms/using/admin-help/configure-external-browser-authentication-document-security.md)を参照してください。
