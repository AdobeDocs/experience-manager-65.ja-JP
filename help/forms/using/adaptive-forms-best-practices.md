...
Forms関連のサンプルテンプレート、テーマ、DOR パッケージのみを使用する場合は、[aemforms-references-* パッケージ ](https://experienceleague.adobe.com/docs/experience-manager-65/forms/publish-process-aem-forms/publishing-unpublishing-forms.html?lang=ja)からダウンロードできます。
...
* すべてのフォームは特定の目的のためにデザインされますが、ほとんどのフォームにはいくつかの共通するセグメントがあります。 例えば、個人の詳細、住所、雇用の詳細などです。 フォームの共通の要素およびセクションに対して[アダプティブフォームフラグメント](/help/forms/using/adaptive-form-fragments.md)を作成し、それらを複数のフォーム間で使用します。 パネルをフラグメントとして既存のフォーム内に保存することもできます。 フラグメント内の変更はすべて、関連付けられたアダプティブフォームに反映されます。 これにより、複数の作成者が同時に、1 つのフォームを構成する別のフラグメントの作業を行えるため、オーサリングの共同作業が促進されます。
   ...
   * フォームのオーサリング中に、再利用可能でないセクションに対してもフォームフラグメントの作成を検討します。 フォームのサイズと複雑さが増すにつれて、フォームをフラグメントに分割すると、オーサリングプロセスが大幅に簡素化され、フォームの保守が簡単になります。 このアプローチにより、フォーム全体を一度に処理するのではなく、フォームのより小さく管理しやすい部分に焦点を当てることができます。
...
### DOM サイズの考慮事項とブラウザーのパフォーマンス

大規模で複雑なアダプティブフォームを作成する際は、DOM サイズがレンダリングとパフォーマンスに与える影響を考慮することが重要です。

* **DOM サイズの影響**：AEM Forms では DOM サイズにハードリミットはありませんが、DOM サイズが大きすぎると、特に遅延読み込みされたフラグメントを処理する際にパフォーマンスに大きな影響を与える場合があります。 大規模な DOM 構造では、レンダリングと操作に多くのメモリと処理時間が必要になります。

* **ブラウザーレンダリングの違い**：レンダリングのパフォーマンスは、ブラウザーやデバイスによって大きく異なる場合があります。 一部のブラウザーレンダリングエンジンでは、スタイルの再計算、リフロー、再ペイントに対する様々なアプローチを使用して、動的な DOM 更新を異なる方法で処理します。 これは、大規模で動的に読み込まれるコンテンツの場合に特に顕著になります。 一部のブラウザーでは、重要な DOM 操作が行われるたびに、完全なレイアウトの再計算とページの再ペイントがトリガーされる場合があります。これにより、大規模なフォームや複雑なフォームでは深刻なパフォーマンスの問題が発生します。

* **パフォーマンス要因**：遅延読み込みのパフォーマンスに影響を与える要因はいくつかあります。
   * フラグメントのサイズと複雑さ
   * 要素に適用される CSS スタイル
   * 動的更新によってトリガーされたリフローの数
   * デバイスとブラウザーの機能

* **実際の影響**：確認されたケースでは、DOM サイズが約 400 KB のフォームで、特定のブラウザーで最大 15 秒の大幅なレンダリング遅延が発生しました。 これらの遅延は、フラグメントサイズだけが原因ではなく、動的コンテンツの挿入中にトリガーされる CSS 処理やページリフローにも関連しています。

**DOM サイズの管理のベストプラクティス：**

* 静的コンテンツの場合は、遅延読み込みによって大きな HTML ブロックを動的に挿入するのではなく、AEM コンテンツフラグメントの使用を考慮します。 このアプローチにより、リフロー、再ペイント、JavaScript 実行時間を短縮し、全体的なページ読み込みパフォーマンスを向上させることができます。

* フラグメントを動的かつ遅延読み込みする必要がある場合は、大きなフラグメントをより小さく管理しやすいフラグメントに分割し、必要に応じて必要なセクションのみを読み込みます。

* 適切な場所にプログレッシブ開示パターンを実装し、ユーザー入力に基づいて必要な場合にのみ追加のフォームフィールドを表示します。

* 特に遅延読み込みフラグメントを使用する場合は、複数のブラウザーとデバイスをまたいでフォームをテストして、様々な環境間で一貫性のあるパフォーマンスを確保します。

* CSS が広範であったり、構造が不十分であると、特に動的コンテンツの更新中にレンダリング時間が大幅に長くなる場合があるので、フォームで使用する CSS を監視して最適化します。

様々なブラウザーレンダリングエンジンで DOM の更新、リフロー、再ペイントを処理する方法に関する技術的な詳細については、様々なブラウザーベンダーが提供するブラウザーエンジンのドキュメントを参照することを検討してください。
