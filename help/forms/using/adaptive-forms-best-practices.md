...
* コンテンツを移動するパッケージを作成する場合は、パッケージのフィルターパスが正しく、必要なパスのみが言及されていることを確認します。

* コンテンツを保存したり、/libsフォルダーに変更を加えたりしないでください。 /appフォルダーにオーバーレイを作成して、デフォルトの機能を拡張または上書きします。

* パッケージに対して正しい依存関係を定義し、事前に決定されたインストール順序/シーケンスを強制します。

* /libsまたは/appsに参照可能なノードを作成しないでください。
...
* アップグレードする際は、非推奨（廃止予定）のAPIと機能をすべて新しいAPIと機能に置き換えてください。
...
* **ドラフトIDの長さ**:アダプティブフォームをドラフトとして保存すると、そのドラフトを一意に識別するためのドラフトIDが生成されます。ドラフトIDフィールドの長さの最小値は26文字です。 Adobeでは、ドラフトIDの長さを26文字以上に設定することをお勧めします。
