...
>[!NOTE]
>
>AEM Forms に、動的なテンプレートが導入されました。AEM サイトのテンプレートエディターを使用して、[動的なテンプレートの作成や編集](../../forms/using/template-editor.md)を行うことができます。この記事で説明するテンプレートは、静的なテンプレートです。デフォルトのインストール環境で静的なテンプレートを使用することはできません。[互換性パッケージをインストールし](../../forms/using/compatibility-package.md) 、お使いの環境にこれらのテンプレートを取得します。
