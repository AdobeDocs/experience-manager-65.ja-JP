...
* XDP ベースのアダプティブフォームでは、別の XDP フラグメントを含むフラグメントとしてパネルを保存すると、生成されるフラグメントは子 XDP フラグメントへと自動的にバインドされます。XSD ベースのアダプティブフォームの場合、結果のフラグメントはスキーマルートにバインドされます。
...
フォームの作成に使用できるアダプティブフォームフラグメントを参照します。
