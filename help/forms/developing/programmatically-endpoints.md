...
* **filePattern**：プロバイダーが受け入れる受信メールの添付ファイルのパターンを指定します。これには、特定のファイル拡張子を持つファイル（&ast;.dat や &ast;.xml）、特定の名前のファイル（data）、名前と拡張子が混在する式に一致するファイル（&ast;.``[dD][aA]``&#39;port&#39;）に含まれます。 デフォルト値は `*` です。
