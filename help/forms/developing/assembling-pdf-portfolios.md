...
DXXドキュメントには、タグが入れ子になっている `Portfolio` 必要があり `Navigator` ます。 タグ `<Resource name="navigator/image.xxx" source="myImage.png"/>` は、onImageレイアウトナビゲーターとして割り当て `myNavigator` られている場合にのみ必要です。 `AdobeOnImage.nav`. このタグを使用すると、Assemblerサービスでポートフォリオの背景として使用する画像を選択できます。 パッケージファイル `PackageFiles``File` のファイル名とMIMEタイプを定義するタグとタグを含めます。
