...
* PDFドキュメント内の電子署名の検証 （「[デジタル署名の確認](digitally-signing-certifying-documents.md#verifying-digital-signatures)」を参照）。
...
各署名を繰り返し処理します。 つまり、各署名について、電子署名を検証し、署名者のIDと各署名のステータスを確認します。 （「[デジタル署名の確認](#verify-digital-signatures-using-the-java-api)」を参照）。
